...
The scan also checks `this`/`super` helper calls and message-source fields
inherited from production source classes. Constant aliases are evaluated in
their declaration scope. Key-carrier exemptions apply only to registered
producers, not every method with a matching name.
