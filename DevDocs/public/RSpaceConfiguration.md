...
* **files.maxUploadSize** The maximum individual file size upload, in bytes. Default is 50MB (52428800). As of the Spring 6 migration the servlet-level limit must be resolved before the Spring context loads, so the value is read in this order: (1) JVM system property `-Dfiles.maxUploadSize=<bytes>` if set, otherwise (2) the `files.maxUploadSize` key in `${propertyFileDir}/deployment.properties` (the same file used for all other deployment overrides — no extra config needed for standard prod installs), otherwise (3) 50MB default. The `max-request-size` (total multipart request) is automatically set to 2x this value. The application-level check in `PropertyHolder.getMaxUploadSize()` continues to read `files.maxUploadSize` via Spring as usual. **Tomcat ops note:** ensure the connector's `maxSwallowSize` is `-1` or larger than the configured upload limit — otherwise oversize uploads result in a connection reset instead of a clean HTTP 413 response.
