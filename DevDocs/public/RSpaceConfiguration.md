...
* **files.maxUploadSize** The maximum individual file size upload, in bytes. Default is 50MB (52428800). The value is read in this order: (1) JVM system property `-Dfiles.maxUploadSize=<bytes>` if set, otherwise (2) the `files.maxUploadSize` key in `${propertyFileDir}/deployment.properties`, otherwise (3) the 50MB default. The total multipart request limit is automatically set to 2x this value. See `DevDocs/DeveloperNotes/UpgradingToSpring6.md` for upgrade notes, including the Tomcat `maxSwallowSize` requirement.
