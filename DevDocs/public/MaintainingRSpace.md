...
* MariaDB connection details for the RSpace database: You can get the connection details for user, password and database name from the properties `jdbc.username`, `jdbc.password` and  the final part of the path defined in `jdbc.url` in `deployment.properties`.
