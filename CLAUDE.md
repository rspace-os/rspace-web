...
7. **Service naming & transactions:** the transaction-proxy pointcut in `applicationContext-service.xml` (`execution(* *..service.*Manager.*(..))`) matches only classes **directly** in a `*.service` package. A `*Manager` in a subpackage (e.g. `service.metadata`) gets **no** transaction unless that subpackage has its own advisor (as `service.inventory` / `service.archive` / `service.fieldmark` do). So a transactional service must be named `*Manager` *and* live in a covered package. Conversely, do not name a non-transactional / coordinating service `*Manager` (use `*Service`, `*Handler`, etc.) — the suffix signals DB-transaction semantics it would not have.
