...
public class SessionTimeZoneUtils {
  ...
  /**
   * Converts a server UTC time into user local time so long as there is a session attribute called
   * {@link SessionAttributeUtils#TIMEZONE} holding a java.util,Timezone object. This will have been
   * set in at user login time.
   *
   * @param serverUtcTime
   ...
   *
   * @param serverUTCdate The server time
   * @return A String in the user's local time in ISO-8601 date format , or UTC if the user's local
   *     time zone was missing or unrecognized.
   */
  ...
  /**
   * As {@link #convertToLocalTimeZone(Date)} but also formats the datetime in a default manner to
   * minute resolution
   *
   * @param serverUTCdate The server time
   * @return A String in the user's local time in ISO-8601 date-time format, or UTC if the user's
   *     local time zone was missing or unrecognized.
   */
   ...
   *     local time zone was missing or unrecognized.
   */
  public String formatDateTimeForClient(
      Date serverUTCdate, boolean showSeconds, boolean showTimezone) {
