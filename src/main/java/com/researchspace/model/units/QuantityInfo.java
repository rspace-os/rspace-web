...
/**
 * Component class to be included by classes implementing {@link Quantifiable}. Stores quantity
 * information with numerical precision up to 3 digital places.
 */
@Embeddable
@Getter
@Setter
...
public class QuantityInfo implements Quantifiable, Serializable {
   ...
   * at 3 decimal places (HALF_UP), and the column is DECIMAL(19,3), so a value with more than
   * {@value #MAX_INTEGER_DIGITS} integer digits does not fit at all: it passes a scale-only check
   * (1E+30 has a NEGATIVE scale) and then fails at the INSERT. Trailing zeros lose nothing, so
   * 0.5000 is storable; null is not.
   */
  ...
  /**
   * Makes a copy. <code>copy.equals(original) == true</code>
   *
   * @return
   */
  ...
  public String toPlainString() {
    return String.format(
        "%s %s", numericValue.toPlainString(), RSUnitDef.getUnitById(unitId).getLabel());
  }
