...
/**
 * Defines a unit for use throughout RSpace. This class is a light wrapper around the javax.measure
 * classes, and wraps instances of these unit classes in the <code>definition</code> field
 *
 * <p>The 'order' field orders the scaling of units and is used to generate an appropriate string
 * representation for quantities in mixed units (the underlying library shows measurements in std
 * units which is sometimes hard to read, e.g. 0.0001g where 100ug would be more easily read.
 */
@ToString
...
public enum RSUnitDef {
  ...
  MICRO_LITRE(
      RSUnits.MICROLITRE,
      2,
      RSUnits.VOLUME_CATEGORY,
      3), // starts with 3 as smaller units defined down in enum
  ...
  public static RSUnitDef getUnitById(Integer id) {
    return getUnitByIdOptional(id)
        .orElseThrow(() -> new IllegalArgumentException("invalid ID for unit"));
  }
  ...
  public static Optional<RSUnitDef> getUnitDefByUnit(Unit<?> unit) {
    return Arrays.stream(values()).filter(def -> def.getDefinition().equals(unit)).findFirst();
  }

  private static Optional<RSUnitDef> getUnitByCategoryAndConversionOrder(
      String category, Integer conversionOrder) {
  ...
  /**
   * Convenience for whether unit is mass, volume or dimensionless for measuring the physical amount
   * of something.
   */
  @JsonIgnore
  ...
  /**
   * An ordering for the units based on increasing unit size within a given category. Units with
   * subsequent display order values (e.g. 8,9,10) are auto-convertible after QuantityUtils
   * operations
   */
  @JsonIgnore
  ...
  /**
   * Given a unit definition returns smaller unit definition from the same conversion family, if
   * such smaller unit exists.
   *
  ...
  public static RSUnitDef getUnitSmallerThan(RSUnitDef initialUnit) {
    Optional<RSUnitDef> prevUnit =
        getUnitByCategoryAndConversionOrder(initialUnit.getCategory(), initialUnit.getOrder() - 1);
  ...
  /**
   * Given a unit definition returns larger unit definition from the same conversion family, if such
   * larger unit exists.
   *
