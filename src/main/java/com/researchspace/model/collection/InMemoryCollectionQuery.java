...
public final class InMemoryCollectionQuery<T> {
  ...
  public long count(Collection<T> resources, ResourceRequest request) {
    ...
    return resources.stream().filter(resource -> matches(resource, request)).count();
  }

  private List<T> matching(Collection<T> resources, ResourceRequest request) {
    Objects.requireNonNull(resources, "Resources");
    Objects.requireNonNull(request, "Resource request");
    List<T> matches = resources.stream().filter(resource -> matches(resource, request)).toList();
  ...
  private boolean matches(T resource, ResourceRequest request) {
    return matches(resource, request.filter()) && matches(resource, request.serverConstraint());
  }

  private boolean matches(T resource, QueryConstraint constraint) {
    if (constraint == null) {
      return true;
    }
    if (constraint instanceof FilterExpression.Comparison comparison) {
      return matchesComparison(resource, comparison);
    }
    if (constraint instanceof FilterExpression.And and) {
      return and.children().stream().allMatch(child -> matches(resource, child));
    }
    if (constraint instanceof FilterExpression.Or or) {
  ...
  private boolean matchesComparison(T resource, FilterExpression.Comparison comparison) {
