...
public class UserPreference implements Serializable {
  ...
  @Setter private Preference preference;
  ...
  /**
   * Convenience constructor. Does fail-fast checking of <code>value</code> which should be:
   *
   * <ul>
   ...
   * @param preference
   * @param user
   * @param value - should be parsable into preference type. Any String should be &lt; 65535
   *     characters.
   * @throws if <code>value</code> is incompatible with its preference type. This ensures that no
   *     invalid data is stored in the database.
   */
  ...
  /**
   * @throws IllegalArgumentException if <code>value</code> is incompatible with its preference
   *     type.
   */
  ...
  public String toString() {
    return "UserPreference [preference="
        + preference
        + ", user="
        + (user == null ? user : user.getUniqueName())
        + ", value="
        + value
        + "]";
  }
   ...
   *
   * @return
   * @throws if is not a {@link Boolean} type-preference
   */
  @Transient
   ...
   *
   * @return
   * @throws IllegalStateException if is not a {@link Boolean} type-preference
   */
  @Transient
