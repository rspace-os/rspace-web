...
public class FileStoreRoot implements Serializable {
  ...
  /**
   * @param fileStoreRoot Must be a path ending in string 'file_store/'
   */
  ...
  /**
   * A file path to the top level folder of an RSpace filestore. Must be unique. Max length = 500 to
   * allow for long file paths.<br>
  ...
  /**
   * Sets the file store root. Performs syntax validation but does not do any checks that the
   * argument is a valid folder.
   *
   * @param fileStoreRoot
   * @throws IllegalArgumentException if <code>fileStoreRoot</code> is empty or too long (see DB
   *     constraint)
   */
  ...
  private void validateRootPath(String fileStoreRoot) {
    ...
    Validate.isTrue(
        fileStoreRoot.length() <= 500,
        String.format(
            "Path is too long [%d] chars, must be <= %d chars", fileStoreRoot.length(), 500));
  ...
  public boolean equals(Object obj) {
    ...
    if (fileStoreRoot == null) {
      if (other.fileStoreRoot != null) return false;
    } else if (!fileStoreRoot.equals(other.fileStoreRoot)) return false;
    return true;
  }

  @Override
  public String toString() {
    return "FileStoreRoot [id="
        + id
        + ", fileStoreRoot="
        + fileStoreRoot
        + ", creationDate="
