...
/** Natural sort order not consistent with equals ( uses column order of template). */
@Entity
...
public abstract class Field extends AbstractField implements Serializable, UniquelyIdentifiable {
  ...
  /**
   * Bidirectional association between media files and fields. This is the owning side of the
   * relationship.
   *
   ...
   * If the FieldAttachment already exists, and was marked deleted, then adding the media file to
   * the same field again will undelete the link. I.e it has the same effect as marking the
   * association as undeleted.
   *
   * @param mediaFile
   * @return <code>Optional<FieldAttachment></code> if mediaFile is successfully added to this field
   */
   ...
   *
   * <p>Usually, it's better to mark the FieldAttachment as deleted rather than call this method.
   *
   * @param mediaFile
   ...
   *
   * @param mediaFile
   * @param deleted Set deleted (true) or not deleted (false).
   * @return A optional {@link FieldAttachment}. Will be <code>null</code> if <code>mediaFile</code>
   *     was not in the set of <code>linkedMediaFiles</code> in this object.
   */
  public Optional<FieldAttachment> setMediaFileLinkDeleted(
      EcatMediaFile mediaFile, boolean deleted) {
    return linkedMediaFiles.stream()
        .filter(fa -> fa.getMediaFile().equals(mediaFile))
  ...
  /**
   * Main public API method to retrieve field data as a String, regardless of subtype or the the
  ...
  /**
   * Main public API method to set data. If this object has <code> isValidating() == true</code>,
   * the argument data should be valid against the template in order to prevent data corruption.<br>
   * E.g., A safe call is :
   *
   * <pre>
   ...
   *
   * @param fieldData
   * @throws IllegalArgumentException if isValidating()== true and fieldData does not validate.
   */
  public void setFieldData(String fieldData) {
    ...
    if (getStructuredDocument() != null) {
      getStructuredDocument()
          .notifyDelta(DeltaType.FIELD_CHG, DeltaType.FIELD_CHG + "-" + getName());
    }
  }
  ...
  /**
   * Iterates through fieldList and for each field tries to get temp field and add it to result
   * list.
   *
   * @param fieldList
   * @param addOrgFieldIfNoTempField decides whether original field should be added to result if it
   *     has no temp field
   * @return List<Field> new list containing temp fields, or mixture of temp and nonTemp
   */
  public static List<Field> getNewListOfTempFields(
      List<Field> fieldList, boolean addOrgFieldIfNoTempField) {
  ...
  /**
   * For each field in fieldList if the field has a temp field attached, and temp field was modified
   * after modificationDate, then add temp field to result;
   *
   * @param fieldList
   * @param modificationDate
   * @return List<Field> temp fields modified after modificationDate
   */
  public static List<Field> getTempFieldsModifiedAfter(
      List<Field> fieldList, Long modificationDate) {
    List<Field> newFieldList = new ArrayList<>();
    for (Field field : fieldList) {
      if (field.getTempField() != null
          && field.getTempField().getModificationDate() > modificationDate) {
        Field tempfield = field.getTempField();
        field.setFieldData(tempfield.getFieldData());
        newFieldList.add(field);
      }
  ...
  /**
   * Checks whether there any field from fieldList has a temporary field attached.
   *
   * @param fieldList
