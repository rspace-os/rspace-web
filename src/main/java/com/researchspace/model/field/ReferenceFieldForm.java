...
/**
 * A Reference to an RSpace GlobalId To be valid, the syntax must be valid GlobalId syntax and also
 * the Prefix must be a known prefix in the Enum {@link GlobalIdPrefix}
 */
@Entity
...
public class ReferenceFieldForm extends FieldForm {
  ...
  static {
    validPrefixes =
        Arrays.stream(GlobalIdPrefix.values())
            .map(GlobalIdPrefix::name)
