 ...
 *
 * <p>User objects natural sort-order is compatible with equals() and is based on username.
 */
@Entity
@Table(name = "User")
@XmlType
...
public class User extends AbstractUserOrGroupImpl
  ...
  public static final String ALLOWED_USERNAME_CHARS_SUBREGEX =
      "[" + ALLOWED_USERNAME_CHARS + "]{" + MIN_UNAME_LENGTH + ",}";

  /** At least 6 chars, alphanumeric preferably, matching a whole line */
  public static final String ALLOWED_USERNAME_CHARS_REGEX =
      "^" + ALLOWED_USERNAME_CHARS_SUBREGEX + "$";

  /**
   * No restriction on length ( other than > 0 characters). For use in SSO environments where we
   * can't control username, matches whole line.
   */
  public static final String ALLOWED_USERNAME_CHARS_RELAXED_LENGTH_REGEX =
      "^" + ALLOWED_USERNAME_CHARS_RELAXED_SUBREGEX + "$";

  public static final String ALLOWED_PWD_CHARS_REGEX =
  ...
  @Setter private byte numConsecutiveLoginFailures;
  @Setter private String username; // required, 6 chars alphanumeric
  private String usernameAlias; // optional
  @Setter private String password; // required
  @Setter private String verificationPassword; // required
  @Setter private String confirmPassword;
  @Setter private SignupSource signupSource = SignupSource.MANUAL; // default
  @Setter private String sid; // optional security identifier

  @Setter private String captcha; // optional signup captcha
  @Setter private boolean picreateGroupOnSignup;
  @Setter private String signupCode = "";
  ...
  @Setter private boolean accountLocked;
  @Setter private boolean tempAccount;
  // optional; mandatory in cloud version
  private String affiliation = "";

  private boolean credentialsExpired;
  @Setter private boolean contentInitialized;
  @Setter private boolean allowedPiRole;
  @Setter private Set<Role> roles = new HashSet<>();
  ...
  @Setter private String tagsJsonString;

  // transient and optional
  @Setter private String token; // used to back signup form in cloud version
  @Setter private UserAuthenticationMethod authenticatedBy;
  @Setter private Set<User> connectedUsers;
  @Setter private Set<Group> connectedGroups;
  ...
  /**
   * Property used only as a temporary property for a new User on the signup page, if enabled. <br>
   * This property is not persisted and calling this method for a persisted user will always return
   * <code>false</code>.
   *
  ...
  /**
   * Getter for verification token; this is used to verify cloud-based signup and is only relevant
   * in cloud-deployment.
   */
  @Transient
  ...
  /**
   * If this object represents the currently logged-in User, this method may store info about
   * authentication method used for logging the user into RSpace.
   */
  @Transient
  ...
  /**
   * Whether user passed as a parameter is a connection of current users. Requires prior call to
   * {@link #setConnectedUsers(Set)}.
   */
  @Transient
  ...
  /**
   * Whether current user is somehow connected to a group passed as a parameter. Requires prior call
   * to {@link #setConnectedGroups(Set)}.
   */
  @Transient
  ...
  /**
   * For cloud deployment, this is mandatory, but is optional for other product variants, so must be
   * allowed to be nullable.
   *
  ...
  /**
   * API method to set affiliation. Performs validation based on the product type
   *
   * @param affiliation
   * @param productType
   * @throws IllegalArgumentException if <code>productType</code> is <code>null</code> or if <code>
   *     affiliation</code> is <code>null</code> when it is disallowed by the product variant. See
   *     RSPAC-246
   */
  ...
  private void validateArgs(String affiliation, ProductType productType) {
    ...
    if (ProductType.COMMUNITY.equals(productType) && StringUtils.isBlank(affiliation)) {
      throw new IllegalArgumentException(
          "Affiliation can't be null for product type " + productType);
    }
  }
  ...
  public static final Comparator<User> LAST_NAME_COMPARATOR =
      (User o1, User o2) -> {
  ...
  /**
   * Utility method to parse out usernames from an auto-complete string formatted as "user1<Bob
   * Jones>,user 2<Simon Smith>,user3",
   *
   * <p>i.e. a comma separated list of usernames with optional extra information between angle
   * brackets
   *
  ...
  /**
   * Date/time of last login attempt. If the last attempt was successful, this should be the same
   * value as lastLogin
   *
   * @return
   */
  ...
  public Set<UserPreference> getAllUserPreferences() {
    return Arrays.stream(Preference.values())
        .filter(p -> isApplicable(p))
        .map(p -> getValueForPreference(p))
  ...
  public boolean hasAnyPiOrLabGroupViewAllRole() {
    return getGroups().stream()
        .anyMatch(
            g ->
                hasRoleInGroup(g, RoleInGroup.PI)
  ...
  /**
   * Generates a publicly consumable user data lacking any sensitive security info
   *
   * @return
   */
  ...
  /**
   * A display name for the User. This implementation delegates to getFullName()
   *
  ...
  /**
   * Removes this role if the user has this role and it is not the ONLY role that the user has.<br>
   * I.e., this operation will not succeed and will return <code>false</code> if removing this role
   * would leave the user with no roles at all.
   *
   * @param role
   * @return <code>true</code> if this role was removed from this user's roles.
   */
  @Transient
   ...
   *
   * @param rolesToTest one or more Roles
  ...
  public void setEmail(String email) {
    if (email != null && email.length() >= DEFAULT_MAXFIELD_LEN) {
      ...
      throw new IllegalArgumentException(
          "Attempt to set  too long an email -should be < 255 characters");
    }
    this.email = email;
  }
  ...
  /**
   * Gets an unmodifiable Set of all groups that this user belongs to, both directly and indirectly
   *
  ...
  /**
   * Boolean test as to whether user <code>other</code> shares a group with this user.
   *
   * @param other
   * @return <code>true</code> if they share a group, <code>false</code> otherwise or if <code>other
   ...
   *
   * @param includeDisabled Boolean choice as to whether to include disabled permissions in the
   *     returned Collection.
   * @param inherit boolean as to whether to consider inherited permissions from Group/UserGroup or
   *     not. (<code>true</code>) or not ( <code>false</code>).
   * @return A new Set containing all permissions applicable to this user. Modifying this collection
   *     won't alter the user' underlying permissions.
   */
  @Transient
  ...
  public void setIncludePermissionForGroup(Group group, boolean include) {
    userGroups.stream()
        .filter(ug -> ug.getGroup().equals(group))
  ...
  /**
   * Convenience boolean test for whether this User owns the argument {@link BaseRecord}
   *
   * @param record
   * @return
   */
  ...
  /**
   * Overwrites existing Preference for user if it exists. This depends on implementation of
   * equals() for UserPreference, which only takes account of user and preference, not the value.
   *
   * @param upNew
   */
  ...
  /**
   * If user has a preference stored, returns the value; else will return the default value. This
   * means we only need to persist non-default values.
   *
   * @param preference
  ...
  public UserPreference getValueForPreference(Preference preference) {
    return userPreferences.stream()
        .filter(up -> up.getPreference().equals(preference))
        .findFirst()
  ...
  public Set<Group> getCollaborationGroups() {
    Set<Group> allGroups =
        userGroups.stream()
            .map((g) -> g.getGroup())
            .filter((grp) -> grp.isCollaborationGroup())
            .collect(Collectors.toCollection(() -> new HashSet<>()));
    return Collections.unmodifiableSet(allGroups);
  }
  ...
  public Set<User> getAllGroupMembers() {
    return getUserGroups().stream()
        .flatMap(ug -> ug.getGroup().getMembers().stream())
  ...
  /**
   * Convenience method to identify the first LabGroup of a PI. This implementation just uses the
   * first lab group that the user is a PI in.
   *
  ...
  /**
   * Boolean test for whether this user is a PI and is associated with a lab group that they're PI
   * of.
   *
   * @return
   */
  ...
  /**
   * Convenience boolean test for whether this user has a global PI role ( <code>true</code>) or not
   * (<code>false</code>)
   *
   * @return a boolean
   */
  ...
  /** Boolean test for whether login should be blocked, based on internal state of User. */
  @Transient
  public boolean isLoginDisabled() {
    return isAccountLockedOrDisabled()
        || isAccountLockedAwaitingAuthorisation()
        || isAnonymousGuestAccount();
  }

  /**
   * Boolean convenience method to determine if account is locked or disabled.
   *
   * <p>Deprecated - you probably want to check {@link #isLoginDisabled()}
   */
  @Deprecated
  @Transient
  ...
  /**
   * For setups required authorisation of new accounts by an admin, this method returns if account
   * is locked for this reason
   */
  @Transient
  ...
  /**
   * For setups requiring email verification of new accounts, this method returns if account is
  ...
  /**
   * Gets non-PI users in groups that this user is a PI or viewAllLabAdmin for. Can be used to
   * retrieve users that this user has automatic read-permission for. <br>
   * If this user is not a PI or labAdmin-viewAll, then returns an empty set.
   */
  @Transient
  public Set<User> getNonPiLabGroupMembersForPiOrViewAllAdmin() {
    Set<User> rc = new HashSet<>();
    for (Group grp : getGroups()) {
      if (hasRoleInGroup(grp, RoleInGroup.PI)
          || grp.getLabAdminsWithViewAllPermission().contains(this)) {
  ...
  /**
   * Gets a set of PIs and viewAll lab admins that can see this user's work by default from: <br>
   * - a list of groups if groups specified <br>
  ...
  /**
   * If deployment property 'user.signup.signupCode' is non-blank, then the user signup must include
   * a string that matches the code. <br>
   * . RSPAC-1796 This property is only used as additional security on user-signup and is not
   * persisted
   *
   * @return
   */
  ...
  public boolean hasAutoshareGroups() {
    return getUserGroups().stream().anyMatch(UserGroup::isAutoshareEnabled);
  }
}
