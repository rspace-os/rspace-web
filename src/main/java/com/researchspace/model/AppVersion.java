 ...
 *
 * <p>This is just a wrapper around the {@link SemanticVersion} utility class to add persistent
 * behaviour.<br>
 * Otherwise delegates through to {@link SemanticVersion} for most behaviour and validation Objects
 * should be immutable once constructed, the private setters are for Hibernate, as these objects can
 * be embedded in tables.
 */
@Embeddable
...
public class AppVersion implements Comparable<AppVersion>, Serializable {
  ...
  /**
   * Boolean test for whether this object is older (a lower version number) than the argument
   * version.
   *
   * @param version
   * @return
  ...
  /**
   * Boolean test for whether this object is newer (a higher version number) than the argument
   * version.
   *
   * @param version
   * @return
