...
public abstract class Record extends BaseRecord implements Serializable, UniquelyIdentifiable {
  ...
  /**
   * Bidirectional association between media files and records. This is the owning side of the
   * relationship.
   *
   ...
   *
   * @param mediaFile
   * @return <code>true</code> if mediaFile is successfully added to this field
   */
  ...
  /**
   * Always returns empty list since Records don't have children; this method is present just so
   * that getChildren can be called recursively while descending a folder tree without testing for
   * the type of BaseRecord.
   */
  @Transient
   ...
   * @return <code>true</code>if has been deleted from user's folder, false otherwise.
   */
