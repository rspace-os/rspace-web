...
/**
 * Represents a mapping between a Record and a Folder, also storing information about the owner of
 * the relationship. Equals/hashcode is defined in terms of the equality of Record and Folder, so
 * that duplicate entities with the same record and child are not allowed.
 */
@Entity
public class RecordToFolder implements Serializable {
  ...
  public String toString() {
    return "RecordToFolder [id="
        + id
        + ", userName="
        + userName
        + ", record="
        + (record == null ? "null" : record.getName())
        + ", folder="
        + (folder == null ? "null" : folder.getName())
        + ", recordInFolderDeleted="
        + recordInFolderDeleted
        + "]";
  }
  ...
  /**
   * Convenient redundant method to help identify a folder/child mapping with a user without having
   * to recurse up a folder tree to see which user this record/folder mapping belongs to.
   *
   * @return
   */
