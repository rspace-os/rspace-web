...
public class ImportOverride {
   ...
   * @param lastModified
   * @param originalCreatorUsername, nullable
   * @param allowCreationDateAfterModificationDate whether creation date that is later than
   *     modification date should be allowed
   */
  public ImportOverride(
      Instant created,
      Instant lastModified,
      String originalCreatorUsername,
      boolean allowCreationDateAfterModificationDate) {
    Validate.noNullElements(
        new Object[] {created, lastModified}, "Creation/modification dates must be non-null");
    ...
    if (created.minusSeconds(TOLERANCE_SECONDS).isAfter(lastModified)) {
      ...
      if (!allowCreationDateAfterModificationDate) {
        throw new IllegalArgumentException(
            "Last modified date: "
                + lastModified
                + " must not be before creation date: "
                + created);
      }
    }
