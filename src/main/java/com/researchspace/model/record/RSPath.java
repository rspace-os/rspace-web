 ...
 * Path order is from parent down to child. So getLastElement () will return the furthest descendant
 * from the parent( which is getFirstElement).
 *
 * <p>Only the last element can be a non-Folder, all other items must be folders.
 */
public class RSPath implements Iterable<BaseRecord> {
  ...
  /**
   * @param pathElementsIn A List of records in a parent/child hierarchy
   * @throws if path elements are not in parent/child relations
   */
      ...
      for (int i = pathElements.size() - 1; i > 0; i--) {
        if (!pathElements.get(i).getParentFolders().contains(pathElements.get(i - 1))) {
          throw new IllegalArgumentException(
              pathElements.get(i).getName()
                  + " not in folder tree: current elements are : "
  ...
  /**
   * Gets immediate parent of the argument br, so long as br is in the path and is not the 1st
   * element in the path. Otherwise, returns <code>Optional.empty</code>.
   *
   * @param br
   * @return
   */
  ...
  /**
   * Gets the first element in the path, or <code>Optional.empty</code> if path is empty.
   *
   * @return
   */
  ...
  /**
   * Gets the record at the specified index in the path, or <code>Optional.empty</code> if index is
   * outside the range.
   *
   * @param i
   * @return
  ...
  /**
   * Gets the first {@link BaseRecord} which has the specified <code>type</code> else returns <code>
   * null</code>
   *
   * @param type A RecordType
   * @return A {@link BaseRecord} or <code>Optional.empty</code> if no BaseRecord with this {@link
   *     RecordType} exists in this {@link RSPath}.
   */
  ...
  /**
   * Merges 2 paths together. Returns an empty path if there is no overlap between the 2 paths. <br>
   * Should be symmetrical, i.e., <code> a.merge(b).equals (b.merge(a)) == true </code> should be
   * invariant.
   *
