 ...
 *
 * <p>Combines basic information about record (from {@link RecordInformation}) with additional about
 * sharing status, workspace path etc.
 */
@Getter
@Setter
public class DetailedRecordInformation extends RecordInformation {
  ...
  private Map<String, String> sharedNotebooksAndOwners =
      new HashMap<>(); /* e.g. NB5215 - matthias */
  ...
  public void calculateSharedStatuses(RecordInfoSharingInfo sharingInfo) {
    ...
    for (RecordGroupSharing recordShare : sharingInfo.getDirectShares()) {
      ...
      if (targetNotebook != null) {
        ...
        sharedUsersAndAccess.put(
            sharee.getDisplayName(), recordShare.getPermType().getDisplayName());
      } else {
        sharedGroupsAndAccess.put(
            sharee.getDisplayName(), recordShare.getPermType().getDisplayName());
      }
    }
    ...
    for (RecordGroupSharing indirectShare : sharingInfo.getImplicitShares()) {
      ...
      implicitShares.merge(
          parentSharedNotebookGlobalId,
          shareeDisplayName,
