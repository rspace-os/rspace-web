...
/**
 * Strategy for altering the modified by property of a user depending on whether the user is running
 * as another user.
 */
public interface IActiveUserStrategy {
  ...
  IActiveUserStrategy NULL =
      new IActiveUserStrategy() {
        @Override
  ...
  /**
   * Consults security manager for whether modifier is running as another user, and returns the
   * original user if this is indeed the case. <br>
   * Otherwise, return the current username, unchanged.
   */
  IActiveUserStrategy CHECK_OPERATE_AS =
      new IActiveUserStrategy() {
        public String getOriginalUser(String currentSubjectUsername) {
