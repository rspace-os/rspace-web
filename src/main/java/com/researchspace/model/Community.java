...
/**
 * A community is an aggregation of LabGroups and forms an administrative unit organised by an
 * RSpaceAdmin role.<br>
...
public class Community implements Serializable, PermissionsAdaptable {
  ...
  /**
   * Default constructor. At least unique name should be set and admin admin added for Community to
   * be valid.
   */
  public Community() {
   ...
   * Doesn't copy database ids or collections, but <em> DOES </em> copy the unique name, which is
   * used for equals(). So, care should be taken that copies are not added to collections of the
   * originals, as they are considered equal. <br>
   * The creation data of the copy is the date of this object creation, not the original.
   *
   * @param admin
   * @param toCopy the community to copy
   */
  ...
  /**
   * Sets the display name - will abbreviate to MAX_DESC_LENGTH chars if the input is too long.
   *
   * @param displayName
   */
  ...
  public static String createUniqueName(String displayName) {
    ...
    if (displayName != null) {
      uniqueName =
          displayName.replaceAll("[^A-Za-z0-9]", "") + RandomStringUtils.randomAlphanumeric(4);
    } else {
  ...
  /**
   * Returns a copy of the encapsulated date; the returned date can be manipulated without affecting
  ...
  /** For hibernate, should not be a public setter - should be set once in API constructor */
  ...
  /**
   * Store mappings in a mapping table rather than in user table. One community can have several
   * admins; an admin can only be admin for 1 community (unless is the sysadmin - for this reason is
   ...
   *
   * @param admin A non-null RSpaceAdmin or SysAdmin user.
   * @return <code>true</code> if admin user was added, <code>false</code> otherwise.
   * @throws IllegalArgumentException if <code>admin</code> is null or does not have an admin role.
   */
   ...
   *
   * @param admin
   * @return <code>true</code> if admin was removed; <code>false</code> otherwise.
   */
   ...
   *
   * @param group where isLabGroup() = true
   * @return <code>true</code> if group was added successfully; <code>false</code> if group is not a
   *     LabGroup
   */
