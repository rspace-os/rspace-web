...
public class IntegrationInfo implements Serializable {
  ...
  /**
   * Whether the sysadmin has set the x.enabled property to make this integration available to users
   *
   * @return
   */
  ...
  /**
   * Boolean test for whether the user has enabled this Integration for themselves
   *
   * @return
   */
  ...
  /**
   * For integrations maintaining server-side OAauth credentials, stores whether credential is
   * stored.
   */
  ...
  /**
   * Check whether the App has any options set up. For configurable applications (e.g. Github,
   * Slack) that is equivalent with app having some configurations.
   *
   * @return
   */
  ...
  /**
   * For an integration where an OAUth access token is stored, this method returns
   * {@code}true{@code} if the app is available, enabled and an access token is stored.
   *
   * @return
   */
  @JsonIgnore
