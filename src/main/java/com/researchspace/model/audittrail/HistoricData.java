...
/** Simple POJO object holding generic information for audit trail / diagnostic logging */
...
public class HistoricData {
   ...
   * @param data An optional {@link AuditData}
   * @param username subject's username
   * @throws IllegalArgumentException if <code>domain</code> or <code>action</code> is <code>null
   *     </code>.
   */
  public HistoricData(
      AuditDomain domain, AuditAction action, String fullName, AuditData data, String username) {
    super();
  ...
  public HistoricData(AuditDomain domain, HistoricalEvent event, AuditData data) {
    this(
        ...
        event.getSubject().getFullName(),
        data,
        event.getSubject().getUniqueName());
