...
public @interface AuditTrailData {
   ...
   * If delegateToCollection() is set, this value is ignored - the objects in the collection are
   * inspected for their audit domains. E.g.,
   *
   * <pre>
  ...
  /**
   * If set, this instructs that the class annotated with this annotation holds a collection of
   * objects to process for auditing, and these will be processed as individual events.
   *
   * <p>The value of this must be a Javabean style property name, and the annotated class must
   * contain a public getter method returning a collection of any objects. For example,
   *
   * <pre>
