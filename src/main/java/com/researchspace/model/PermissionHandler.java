...
class PermissionHandler implements Serializable {
  ...
  private static final ConstraintPermissionResolver permissionResolver =
  ...
  void setPermissionStrings(Set<String> permissionStrings) {
    ...
    for (String permStr : permissionStrings) {
      // RSPAC-2706, known issue, parsing will just fail with error-level exception, so let's log on
      // info and move on
      ...
      try {
        tempPermissions.add(
            ConstraintPermissionResolver.populatePermssion(
                new ConstraintBasedPermission(), permStr));
      } catch (Exception e) {
