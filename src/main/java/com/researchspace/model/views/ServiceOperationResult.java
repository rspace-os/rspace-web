 ...
 *
 * <p>In some historical cases the entity is String message; since 1.63 there is an additional field
 * for messages that should be used preferentially for consistency. Here is a table of the
 * invariants.
 *
 * <p>In summary, an object of this class must have a message set if succeeded == false.
 *
 * <table>
 ...
 * </table>
 *
 * Given that both entity and message are nullable, we should consider redesigning this class or
 * creating a version2 to avoid breaking the 150+ uses of it
 *
 * @param <T>
 */
@Getter
public class ServiceOperationResult<T> {
  ...
  /**
   * Creates a new ServiceOperationResult with an empty message, unless the entity is a
   * java.lang.String, in which case the message will be set to that String.
   *
  ...
  /**
   * Static factory method for when a ServiceOperationResult's entity is a string error message in
   * event of failure.
   *
   * @param optionalErrorMessage
   * @return
   */
  public static ServiceOperationResult<String> fromOptionalError(
      Optional<String> optionalErrorMessage) {
    return optionalErrorMessage
        .map(errorMsg -> new ServiceOperationResult<>(errorMsg, false))
        .orElse(new ServiceOperationResult<>("", true));
  }

  /**
   * Converts a ServiceOperationResult<?> to ServiceOperationResult<String>. This is kept for
   * backwards compatibility where UI is expecting a ServiceOperationResult<String> returned to it.
   * <br>
   * Note:
   *
   * <ul>
   *   <li>The returned object's entity is <code>replacementEntity</code> String
   *   <li>The returned object's message is kept the same as the original if it was not empty; else
   *       is set to <code>replacementEntity</code>
   * </ul>
   *
   * @param toConvert
   ...
   * @return
   */
  public static ServiceOperationResult<String> convertToStringEntity(
      ServiceOperationResult<?> toConvert, String replacementEntity) {
    return new ServiceOperationResult<>(
        replacementEntity,
        toConvert.isSucceeded(),
