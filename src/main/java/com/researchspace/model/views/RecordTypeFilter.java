...
/** Utility class to parameterise database searches by including or excluding records by type. */
public class RecordTypeFilter extends AbstractEnumFilter<RecordType> {

  @Override
  public String toString() {
    return "RecordTypeFilter [getWantedTypes()="
        + getWantedTypes()
        + ", getExcludedTypes()="
        + getExcludedTypes()
        + "]";
  ...
  public static final RecordTypeFilter WORKSPACE_FILTER =
      new RecordTypeFilter(
          ...
          EnumSet.of(RecordType.MEDIA_FILE, RecordType.NORMAL_EXAMPLE, RecordType.ROOT_MEDIA));

  /** Types of records we want as Move targets in Move dialog */
  public static final RecordTypeFilter MOVE_DIALOGFILTER =
      new RecordTypeFilter(
          EnumSet.of(RecordType.FOLDER, RecordType.ROOT, RecordType.SHARED_FOLDER), true);

  /** Types of records we want as Move targets in Move dialog */
  public static final RecordTypeFilter MOVE_DIALOGFILTER_PLUS_NOTEBOOKS =
      new RecordTypeFilter(
          EnumSet.of(
              RecordType.FOLDER, RecordType.ROOT, RecordType.SHARED_FOLDER, RecordType.NOTEBOOK),
          true);
