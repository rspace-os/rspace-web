...
@Setter
@EqualsAndHashCode(
    of = {"clientId", "user", "hashedAccessToken", "tokenType", "expiryTime", "hashedRefreshToken"})
...
public class OAuthToken implements Serializable, AuthenticationToken {
  ...
  /**
   * Can be null if only JWT tokens are requested. Use expiry time to test the validity of the
   * token.
   */
  ...
  public OAuthToken(User user, String clientId, OAuthTokenType tokenType) {
    Validate.noNullElements(
        new Object[] {user, clientId, tokenType}, "userId/clientId/tokenType cannot be null");
  ...
  public void setHashedAccessToken(String hashedAccessToken) {
    Validate.isTrue(
        !isBlank(hashedAccessToken) || hashedAccessToken == null, "access token must be non-empty");
