...
/**
 * Adds a linked list capability to connect a sequence of communications. These communications are
 * user initiated.
 */
@Entity
@XmlAccessorType(XmlAccessType.NONE)
public class MessageOrRequest extends Communication
    implements Serializable, Comparable<MessageOrRequest> {
  ...
  /**
   * Creates a new object, setting its creation time and the status to {@link
  ...
  /**
   * Sets the previous message in the chain, which must be created before this message
   *
   * @param previous
   */
  public void setPreviousMessage(MessageOrRequest previous) {
    if (previous != null && previous.getCreationTime().after(this.getCreationTime())) {
      ...
      throw new IllegalArgumentException(
          "Cannot set previous message to be later than this message");
    }
  ...
  /**
   * Sets the next message in the chain, which must be created after this message
   *
   * @param next
   */
  ...
  public boolean isTerminated() {
    return CommunicationStatus.CANCELLED.equals(status)
        || CommunicationStatus.COMPLETED.equals(status)
        || CommunicationStatus.REJECTED.equals(status);
  }

  @Transient
  public boolean isSimpleMessage() {
    return MessageType.SIMPLE_MESSAGE.equals(messageType)
        || MessageType.GLOBAL_MESSAGE.equals(messageType);
  }
  ...
  /**
   * Boolean flag to indicate the most recent message, so as to avoid traversing the list each time
