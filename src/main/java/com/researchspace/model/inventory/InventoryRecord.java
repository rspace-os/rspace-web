...
public abstract class InventoryRecord {
  ...
  /**
   * tags field is redundant as all info is in tagMetaData but the field is present to allow
   * efficient database searches.
   */
  private String tags;

  /**
   * tagMetaData is the source of truth for tag data - the tags field should now be derived from
   * tagMetaData.
   */
  ...
  public List<String> getSharedWithUniqueNames() {
    ...
    return getSharingACL().getAclElements().stream()
  ...
  public static final Comparator<InventoryRecord> NAME_COMPARATOR =
      new Comparator<>() {
        @Override
  ...
  public static final Comparator<InventoryRecord> TYPE_COMPARATOR =
      new Comparator<>() {
        @Override
        public int compare(InventoryRecord invRec1, InventoryRecord invRec2) {
  ...
  public static final Comparator<InventoryRecord> GLOBAL_ID_PREFIX_COMPARATOR =
      new Comparator<>() {
        @Override
        public int compare(InventoryRecord invRec1, InventoryRecord invRec2) {
          return invRec1.getType().toString().compareTo(invRec2.getType().toString());
  ...
  /**
   * For hibernate. When deleting don't try setting the flag directly, call {@link
   * #setRecordDeleted(boolean)}
   */
   ...
   * polymorphically (Sample returns its own template, SubSample delegates to its parent sample) so
   * callers need no unproxy/downcast when the record is a Hibernate proxy.
   */
  @Transient
  ...
  protected void verifyFieldNameAllowed(String fieldName) {
    ...
    if (getReservedFieldNames().contains(fieldName.toLowerCase())) {
      throw new IllegalArgumentException(
          String.format(
              "'%s' is not a valid name for a field, "
  ...
  public List<ExtraField> getActiveExtraFields() {
    if (activeExtraFields == null) {
      activeExtraFields =
          getExtraFields().stream().filter(ef -> !ef.isDeleted()).collect(Collectors.toList());
    }
  ...
  public List<InventoryFile> getAttachedFiles() {
    if (attachedFiles == null) {
      attachedFiles =
          getFiles().stream().filter(ef -> !ef.isDeleted()).collect(Collectors.toList());
    }
  ...
  public List<Barcode> getActiveBarcodes() {
    if (activeBarcodes == null) {
      activeBarcodes =
          getBarcodes().stream().filter(ef -> !ef.isDeleted()).collect(Collectors.toList());
    }
  ...
  public List<DigitalObjectIdentifier> getActiveIdentifiers() {
    if (activeIdentifiers == null) {
      activeIdentifiers =
          getIdentifiers().stream().filter(i -> !i.isDeleted()).collect(Collectors.toList());
    }
