...
public class PaginationCriteria<T> implements Serializable, IPagination<T> {
   ...
   */
  @Getter @Setter private boolean resultsPerPageExplicitlySet = false;
  ...
  /**
   * Creates default {@link PaginationCriteria} with 10 records per page, set to page 0, sort order
   * DESC
   *
   * @param clazz
  ...
  public static <T> PaginationCriteria<T> createDefaultForClass(Class<T> clazz) {
    return createForClass(
        clazz, null, SortOrder.DESC.toString(), 0L, IPagination.DEFAULT_RESULTS_PERPAGE);
  }
   ...
   * @return
   */
  public static <T> PaginationCriteria<T> createForClass(
