...
public class App implements Serializable {
  ...
  public void setAppConfigElementDescriptors(
      Set<AppConfigElementDescriptor> appConfigElementDescriptors) {
  ...
  public AppConfigElementDescriptor getDescriptorByName(String propertyName) {
    return appConfigElementDescriptors.stream()
        .filter(aced -> aced.getDescriptor().getName().equals(propertyName))
        .findFirst()
        .orElse(null);
  }
  ...
  public boolean isRepositoryApp() {
    return APP_DATAVERSE.equals(name)
        || APP_DSPACE.equals(name)
        || APP_FIGSHARE.equals(name)
        || APP_DRYAD.equals(name)
        || APP_ZENODO.equals(name)
        || APP_DIGITAL_COMMONS_DATA.equals(name);
  }
  ...
  /** Converts App name to IntegrationInfo name e.g. 'app.slack' -> 'SLACK' */
  @Transient
