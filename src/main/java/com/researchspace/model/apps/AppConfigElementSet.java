...
public class AppConfigElementSet implements Serializable {
  ...
  /**
   * Stable natural key for equals/hashCode; avoids HashSet corruption when id is assigned after
   * insertion.
   */
  ...
  @OneToMany(
      mappedBy = "appConfigElementSet",
      cascade = CascadeType.ALL,
      orphanRemoval = true,
      fetch = FetchType.EAGER)
  private Set<AppConfigElement> configElements = new HashSet<>();

  @ManyToOne private UserAppConfig userAppConfig;
  ...
  public Set<PropertyDescriptor> getProperties() {
    return configElements.stream()
        .map((el) -> el.getAppConfigElementDescriptor().getDescriptor())
  ...
  public boolean propertiesMatch(AppConfigElementSet other) {
    ...
    return isEqualSet(theseProperties, otherProperties)
        && isEqualSet(otherProperties, theseProperties);
  }
  ...
  public void merge(AppConfigElementSet other) {
    for (AppConfigElement thisEl : getConfigElements()) {
      for (AppConfigElement otherEl : other.getConfigElements()) {
        if (thisEl
            .getAppConfigElementDescriptor()
            .equals(otherEl.getAppConfigElementDescriptor())) {
  ...
  public AppConfigElement findElementByPropertyName(String name) {
    return configElements.stream()
        .filter((el) -> el.getAppConfigElementDescriptor().getDescriptor().getName().equals(name))
        .findFirst()
        .orElse(null);
  }
