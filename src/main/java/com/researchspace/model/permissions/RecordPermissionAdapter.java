...
public class RecordPermissionAdapter extends AbstractEntityPermissionAdapter {
  ...
  protected @Override PropertyConstraint handleSpecialProperties(String propertyName) {
    if (propertyName.equals(FORM_PROP_NAME) && record.isStructuredDocument()) {
      return new PropertyConstraint(
          FORM_PROP_NAME, ((StructuredDocument) record).getForm().getName());
    }
  ...
  private PropertyConstraint allowGroupPIToPublishIfSharedWithThemOrWithTheirGroup(
      String propertyName, User owner) {
    ...
    if (shareeIsPIOfOwnersGroup) {
      for (String userAcl : userACls) {
        ...
        if (userOrGrpName.indexOf('=') == -1
            && userOrGrpName.indexOf('_') == -1
  ...
  public Set<GroupConstraint> getGroupConstraints() {
    ...
    for (Group group : u.getGroups()) {
      ...
      if (PermissionType.READ.equals(getAction())
          && u.hasRole(Role.PI_ROLE)
