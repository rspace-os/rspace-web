...
/**
 * Convenience class to encrypt small pieces of text using a symmetric key, which can be supplied
 * e.g via a deployment property or external source.<br>
 * Uses ECB mode - repeated encryption of the same string produces the same result.
 *
 * <p>This is not suitable for encryption of large documents, where default CBC mode is better.
 */
public class SymmetricTextEncryptor implements TextEncryptor, Serializable {
  ...
  private void assertArgs(byte[] keyBytes) {
    Validate.isTrue(
        keyBytes != null && isValidAESKeyLength(keyBytes),
        " must be valid AES key length but was " + keyBytes == null
            ? "null"
            : keyBytes.length + "");
  }
  ...
  /**
   * Static factory to generates a SymmetricTextEncryptor with AES-128 bit key derived from a
   * SHA-384 digest of the input source string.
   *
