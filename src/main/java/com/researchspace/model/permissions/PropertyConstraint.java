 ...
 * For resolving against entities, the property names should be Javabean names for accessor methods
 * in the object, and will work for simple properties (primitive values and strings).
 *
 * <p>An exception is the property name 'template' whose value should be a template name, and will
 * be tested if the permission to be tested is for a structured document.
 *
 * <p>Property values can be wildcards or variable expressions: W wildcard implies all values.<br>
 * A property constraint value can also be a variable , e.g.,'${self}' which refers to the current
 * logged in subject's principal name.
 */
public class PropertyConstraint implements Serializable {
  ...
  public String getString() {
    ...
    sb.append(ConstraintPermissionResolver.PROPERTY_PARAM_PREFIX)
        .append(name)
        .append("=")
        .append(value);
    return sb.toString();
   ...
   * @param name
   * @param value
   * @throws IllegalArgumentException if value has invalid characters
   */
  ...
  /**
   * Removes a value. If this is a single value, and is removed, this method returns <code>true
   * </code>, indicating that this constraint should be removed from the containing permission.
   * Otherwise,if value is a list returns false and modifies the list in place.
   *
   * @param valueToRemove
   * @return
   */
