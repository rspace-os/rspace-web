...
public abstract class AbstractEntityPermissionAdapter implements IEntityPermission {
  ...
  @Getter @Setter private PermissionDomain domain = PermissionDomain.RECORD;

  @Getter @Setter private PermissionType action;
  ...
  /**
   * This performs a permissions check in the 'reverse' way that regular permissions are checked.
   * The regular way consults an object's properties to see if they match permissions assigned to
   * act a user.
   *
   * <p>The ACL, though, is set on the entity, and queries the properties of the user to see if they
   * match the ACL list. This default implementation returns <code>false</code>, subclasses can
   * override if they support ACLs. See {@link RecordSharingACL} as an example.
   *
