...
public class RecordSharingACL implements Serializable {
  ...
  static final String ACL_ELEMENT_DELIMITER = "&";
  ...
  /**
   * Static factory method to create a new {@link RecordSharingACL} for a given user/group and
   * permission type.
   *
   * @param userOrGrp
   * @param permType
   * @return
   */
  public static RecordSharingACL createACLForUserOrGroup(
      UserOrGroup userOrGrp, PermissionType permType) {
  ...
  public boolean removeACLElement(
      AbstractUserOrGroupImpl userOrGrp, ConstraintBasedPermission cbp) {
    ACLElement el = new ACLElement(userOrGrp.getUniqueName(), cbp);
  ...
  /**
   * Returns <code>true</code> if there are ACL elements defined - this is equivalent to testing
   * that :<br>
  ...
  /**
   * Permissions check as to whether the specified user meets the assigned permissions in the ACL.
   *
   * @param user
   * @param requestedAction
   * @return <code>true</code> if user is authorised, <code>false</code> otherwise.
   */
  ...
  public Set<ACLElement> removeACLsforRolesInGroup(RoleInGroup... roles) {
    ...
    ACLElement groupAdminACL =
        ACLElement.createRoleRestrictedGroupACL(null, emptyPermission, roles);
    Set<ACLElement> toRemove = new HashSet<>();
    for (ACLElement element : aclElements) {
  ...
  /**
   * Gets String representation of this ACL for persistence. The syntax is asfollows:
   * user1=RECORD:READ:&user2=RECORD:WRITE:&group1=RECORD:COPY:
   *
   * <p>I.e., individual elements are separated by an ampersand.
   *
   * @return
   */
   ...
   * UNSAFE!
   *
   * <p>getAclElements should only be used with displaying share icons in the workspace home page.
   *
   * <p>Unsafe for performance reasons -> we do not want to perform a deep copy of aclElements for
   * each record shown in the workspace, that is potentially cloning hundreds of objects.
   */
  @Transient
  ...
  /**
   * Calculates ACLElements in common between this ACL and the <code>other</code>ACL and returns a
   * List of the share ACLs.
   *
   * @param other
   * @return
   */
  ...
  /**
   * Joins the content of this ACL with the <code>other</code> ACL, ignoring duplicates.
   *
   * @param other
   */
