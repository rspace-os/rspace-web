 ...
 * Shouldn't use DB/ Spring so can be used throughout application wherever permissions need to be
...
public class DefaultPermissionFactory implements PermissionFactory {
  ...
  public Set<ConstraintBasedPermission> createDefaultPermissionsForGroupPI(Group group) {
    ...
    ConstraintBasedPermission readAllRecords =
   ...
   */
  private void addGroupLeaderPermissions(
      Set<ConstraintBasedPermission> rc, GroupConstraint groupConstraint) {
  ...
  public Set<ConstraintBasedPermission> createDefaultPermissionsForProjectGroupOwner(Group group) {
    ...
    ConstraintBasedPermission inviteNewUser =
        createRequestPermissionForMsgType(MessageType.REQUEST_JOIN_PROJECT_GROUP);
    inviteNewUser.setGroupConstraint(groupConstraint);
    rc.add(inviteNewUser);
    return rc;
  }
  ...
  private ConstraintBasedPermission createFormPermission() {
    ConstraintBasedPermission createForms =
        new ConstraintBasedPermission(PermissionDomain.FORM, PermissionType.CREATE);
    return createForms;
  }
  ...
  private ConstraintBasedPermission createEditGroupPermissions() {
    ConstraintBasedPermission editGroup =
        new ConstraintBasedPermission(PermissionDomain.GROUP, PermissionType.READ);
  ...
  private ConstraintBasedPermission createAccessAllForms(Set<ConstraintBasedPermission> rc) {
    ConstraintBasedPermission accessAllForms =
  ...
  private void addRequestExternalSharePermission(Set<ConstraintBasedPermission> rc) {
    ConstraintBasedPermission requestExternalShare =
        createRequestPermissionForMsgType(MessageType.REQUEST_EXTERNAL_SHARE);
   ...
   * @param rc
   */
  private ConstraintBasedPermission addRequestJoinLabGroupPermission(
      Set<ConstraintBasedPermission> rc) {
    ConstraintBasedPermission requestJoinLabGroup =
        createRequestPermissionForMsgType(MessageType.REQUEST_JOIN_LAB_GROUP);
    return requestJoinLabGroup;
  }

  /**
   * Adds permission to invite another user to join an existing collaboration group.
   *
   * @param rc
   */
  private void addRequestJoinExistingCollabGroupPermission(Set<ConstraintBasedPermission> rc) {
    ConstraintBasedPermission requestCollabGpJoin =
        createRequestPermissionForMsgType(MessageType.REQUEST_JOIN_EXISTING_COLLAB_GROUP);
  ...
  private ConstraintBasedPermission createRequestPermissionForMsgType(MessageType type) {
    ConstraintBasedPermission perm =
        new ConstraintBasedPermission(PermissionDomain.COMMS, PermissionType.READ);
  ...
  public Set<ConstraintBasedPermission> createDefaultGlobalGroupPermissions(Group group) {
    ...
    ConstraintBasedPermission perm2 =
    ...
    ConstraintBasedPermission useForms =
        new ConstraintBasedPermission(PermissionDomain.FORM, PermissionType.SHARE);
    ...
    useForms.addPropertyConstraint(
        new PropertyConstraint(FormPermissionAdapter.GROUP_PROPERTY_NAME, "true"));
    rc.add(useForms);

    ConstraintBasedPermission shareingroup =
        new ConstraintBasedPermission(PermissionDomain.GROUP, PermissionType.SHARE);
  ...
  @Override
  public ConstraintBasedPermission createIdPermission(
      PermissionDomain domain, PermissionType type, Long id) {
  ...
  /**
   * Sets up the ACL for a shared group folder, or does nothing if <code>grpFolder</code> is not a
   * root shared group folder.
   *
   * @param grp group the shared folder belongs to.
   * @param grpFolder A Shared group folder
  ...
  public void setUpACLForGroupSharedRootFolder(Group grp, Folder grpFolder) {
    ...
    ConstraintBasedPermission cbp =
        new ConstraintBasedPermission(
            PermissionDomain.RECORD, PermissionType.READ, PermissionType.CREATE_FOLDER);
    ACLElement el = new ACLElement(grp.getUniqueName(), cbp);
    grpFolder.getSharingACL().addACLElement(el);

    // delete and rename only apply to children of this folder. This permission is overridden in
    // DeleteManager and
    // Workspace permissionsDTOBuilder (RSPAC-1636) for the lab group folder
    ConstraintBasedPermission folderManipulationPermission =
        new ConstraintBasedPermission(
    ...
    if (!grp.isProjectGroup()) {
      ...
      folderManipulationElement =
          ACLElement.createRoleRestrictedGroupACL(
              grp, folderManipulationPermission, RoleInGroup.PI, RoleInGroup.RS_LAB_ADMIN);
    } else {
      // all members of a project group can manipulate folders and docs within the shared group
      // folder
  ...
  @Override
  public Set<ConstraintBasedPermission> createCommunityPermissionsForAdmin(
    ...
    ConstraintBasedPermission cbp =
        new ConstraintBasedPermission(PermissionDomain.COMMUNITY, PermissionType.WRITE);
    cbp.setIdConstraint(new IdConstraint(community.getId()));
    admin.addPermission(cbp);

    ConstraintBasedPermission cbp2 =
        new ConstraintBasedPermission(PermissionDomain.RECORD, PermissionType.READ);
    cbp2.setCommunityConstraint(new CommunityConstraint(community.getId()));
    admin.addPermission(cbp2);

    ConstraintBasedPermission cbp3 =
        new ConstraintBasedPermission(PermissionDomain.GROUP, PermissionType.WRITE);
    cbp3.setCommunityConstraint(new CommunityConstraint(community.getId()));
    admin.addPermission(cbp3);

    ConstraintBasedPermission cbp4 =
        new ConstraintBasedPermission(PermissionDomain.GROUP, PermissionType.CREATE);
  ...
  public void setUpACLForIndividSharedFolder(User sharer, User sharee, Folder sharedFolder) {
    ...
    ConstraintBasedPermission folderManips =
        new ConstraintBasedPermission(
            PermissionDomain.RECORD, PermissionType.DELETE, PermissionType.READ);
  ...
  public void setUpACLForUserRoot(User u, Folder rootForUser) {
    ...
    ConstraintBasedPermission createRecordPerm =
    ...
    ConstraintBasedPermission createFolderPerm =
    ...
    ConstraintBasedPermission deleteRecordPerm =
        new ConstraintBasedPermission(PermissionDomain.RECORD, PermissionType.DELETE);
    acl.addACLElement(new ACLElement(u.getUsername(), deleteRecordPerm));
    ConstraintBasedPermission sendPerm =
    ...
    ConstraintBasedPermission receive =
    ...
    ConstraintBasedPermission rename =
    ...
    ConstraintBasedPermission copy =
  ...
  /**
   * Adds template sub-folder permissions to folder/template in Templates folder
   *
   * @param templateChild
   * @param subject
   */
  ...
  public void setUpAclForIndividualInboxFolder(Folder inboxFolder, User subject) {
    Validate.isTrue(
        inboxFolder.isImportedContentFolder(), "Folder must be  an ApiInbox or Imports folder");
    if (!inboxFolder.getOwner().equals(subject)) {
      throw new AuthorizationException(
          String.format(
              "Subject %s must be the owner of inbox folder but is %s",
  ...
  private ConstraintBasedPermission standardFolderPermissions() {
    return new ConstraintBasedPermission(
        PermissionDomain.RECORD,
        PermissionType.COPY,
        PermissionType.DELETE,
        PermissionType.FOLDER_RECEIVE,
