 ...
 * </ul>
 *
 * Names are compared case-sensitively after trimming leading/trailing whitespace. Null or blank
 * names are skipped — they have their own validation upstream (required-name checks).
 *
 ...
 * by the controller advice's {@code BindException} handler).
 */
public final class InventoryFieldNameUniquenessValidator {
   ...
   * the SECOND occurrence of any name match. Errors are added with explicit nested paths (e.g.
   * {@code fields[1].name}, {@code extraFields[0].name}) so API clients can pinpoint the
   * problematic entry. Used from the Spring {@link org.springframework.validation.Validator}s
