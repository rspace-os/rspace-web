 ...
 * public API contract (plan-operations-server-builds.md M6, shapes frozen in
 * operations-facade-design-m0.md). Each carries what is consumed ({@code origin}, or {@code
 * origins} for Pool, identified by global id, M0 D3), the values the definition declares as inputs,
 * and for a creating operation the template (numeric like {@code POST /samples}, D4) and the
 * documentation target (D8: no other sample metadata; set it with a follow-up {@code PUT}).
 *
