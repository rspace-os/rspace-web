...
public class ApiRequestThrottlerImpl extends AbstractTokenBucketThrottler
  ...
  public boolean proceed(String userId, Double requestedResourceUnits) {
    ...
    // Deviation from v1: v1 runs this read-modify-write unlocked, so concurrent requests for one
    // key each read the same allowance, all pass isAllowed, and the limit is silently overspent.
    // Locking the tracker gives per-key mutual exclusion; BoundedAllowanceTrackerSource guarantees
    // one tracker instance per key, which is what makes the monitor meaningful.
  ...
  public Optional<APIUsageStats> getStats(String id, ThrottleInterval throttleInterval) {
    ...
    // Same monitor as proceed(), so the reported allowance is a consistent read rather than a value
    // torn from a concurrent update.
    synchronized (userTracker) {
      APIUsageStats stats =
