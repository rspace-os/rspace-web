...
public interface BookingCalendarSubscriptionDao
  ...
  /** Loads subscriptions to revalidate after a user's status or memberships change. */
  List<BookableItemCalendarSubscription> findByUserId(Long userId);

  /** Loads subscriptions to revalidate after a configuration's assignments change. */
  List<BookableItemCalendarSubscription> findByConfigurationId(Long configurationId);
