...
public class BasicLogQuerySearcher implements IAuditFileSearch {
  ...
  public BasicLogQuerySearcher(
      @Autowired ILogResourceTracker logFileTracker,
      @Autowired LogLineContentProvider lineContentProvider) {
  ...
  @Override
  public ISearchResults<AuditTrailSearchResult> search(
      IPagination pgCrit, AuditTrailSearchElement searchConfig) {
    ...
    if (hits.isEmpty()) {
      ...
      for (LogLine hit : hits) {
        HistoricData hd =
            new HistoricData(
                hit.domain, hit.action, hit.fullname, AuditData.fromJson(hit.data), hit.username);
  ...
  private boolean isUserNameMatch(AuditTrailSearchElement searchConfig, LogLine logline) {
    ...
    if (searchConfig.getUsernames().contains(logline.username)) {
      ...
      rc =
          searchConfig.getUsernames().contains(operateAs[0])
              || searchConfig.getUsernames().contains(operateAs[1]);
    }
    return rc;
  }
