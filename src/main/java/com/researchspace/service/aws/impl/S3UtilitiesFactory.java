...
public class S3UtilitiesFactory {
  ...
  public S3Utilities createS3UtilitiesForAwsArchiveExport(String s3ExportRegion, String s3ExportBucketName) {
  ...
  public S3Utilities createS3UtilitiesForNfsConnector(NfsFileSystem fileSystem) {
    ...
    boolean s3PathStyleAccessEnabled = "true".equalsIgnoreCase(fileSystem.getClientOption(NfsFileSystemOption.S3_PATH_STYLE_ACCESS_ENABLED));

    S3UtilitiesImpl s3utils = new S3UtilitiesImpl();
    s3utils.setChunkedUploadMbThreshold(chunkedUploadMbThreshold);
    s3utils.setChunkedUploadMbSize(chunkedUploadMbSize);
    s3utils.initializeS3Client(fileSystem.getUrl(),
        s3Region, s3BucketName, s3PathStyleAccessEnabled, nfsS3accessKey, nfsS3secretKey);
    return s3utils;
  }
