...
public class SearchResultsImpl<T> implements ISearchResults<T>, Serializable {
   ...
   * No null arguments.
   *
   * @param results A possibly empty but non-null <code>List</code> of results.
   * @param pageNumber The desired page number to display.
   * @param totalHits The total number of possible hits (disregarding pagination)
   */
  ...
  /**
   * Convenience constructor that populates internal fields from a PaginationCriteria object
   *
   * @param results A <code>List</code> of results.
   * @param pgCrit The pagination criteria where we can access information about current page
   *     number, total hits per page, sort/ordering.
   * @param totalHits The total number of possible hits (disregarding pagination)
   */
