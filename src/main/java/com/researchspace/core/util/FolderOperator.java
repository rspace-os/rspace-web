...
/**
 * In conjunction with URI, create, remove, and retrieve a path of folder all working with canonical
 * path, i.e without the file separator at end.
 */
public class FolderOperator {
  /** A folder added below the folder specified by the system or environment property. */
  ...
  /**
   * Sets the filestore root based on system properties. After construction, getBaseDir and
   * getFileRoot will be set. The folder path will be searched for in the order:
   *
   * <ol>
   ...
   *
   * @param path such as: /Users/guangyang/myapps/workspace/akubra4u/bin
   * @throws IllegalStateException if teh file_store subfolder cannot be created below this path.
   */
  public void setFileStoreRootDir(String path) {
    ...
    if (!baseDir.exists() && !baseDir.mkdir()) {
      log.warn(
          "Error cannot create sub-folder: {}file_store from {}",
          FILE_STORE_DIR_NAME,
