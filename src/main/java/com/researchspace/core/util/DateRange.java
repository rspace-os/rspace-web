...
public class DateRange implements Comparable<DateRange>, Serializable {
  ...
  /**
   * Parses string in format 'yyyy-MM-dd' (from), or ',yyyy-MM-dd' (to) or 'yyyy-MM-dd,yyyy-MM-dd'
   * (between - inclusive)
   *
   * @param fromTodateString
   * @return A date range
   * @throws IllegalArgumentException if input string is incorrect format
   */
   ...
   * @throws IllegalArgumentException if <code>from </code>> <code>to</code>.
   */
   ...
   *
   * @param fromDate If null, set to 1970
   * @param toDate If null ,set to far future (date value of Long.MAXVALUE)
  ...
  /**
   * Boolean test as to whether there is any overlap between this range and <code>range</code> E.g.
   * for 2 date ranges A and B, this method returns true if:
   *
   * <ul>
  ...
  public boolean overlapsWith(DateRange range) {
    return contains(range.getFrom())
        || contains(range.getTo())
        || range.contains(this.from)
        || range.contains(this.to);
  }
  ...
  /**
   * Returns a new date range if the two ranges overlap, else returns a copy of this DateRange
   *
   * @param range
