...
public interface TimeLimitedMemoryCache<T> {
  ...
  /**
   * Gets cached item, or <code>null</code> if cache is unavailable or has expired.
   *
  ...
  /**
   * Sets an item into the cache. Cache will remain VALID until cacheTimeMillis millis has elapsed,
   * after which time cache will be STALE.
   *
   * @param license
   */
  ...
  /**
   * Cache does not hold an item, i.e., getItem() would return <code>null</code>.
   *
   * @return
   */
  boolean isEmpty();
  ...
  /**
   * Cache holds an item whose cache time has expired. The item is still retrievable though.
   *
   * @return
   */
  boolean isStale();
