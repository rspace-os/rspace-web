...
public class DateRangeRestrictor {
  ...
  /**
   * Adjust a date range in the given <code>dateRangeAdjustable</code> to be no loner than the
   * specified {@link Duration}
   *
   * @param dateRangeAdjustable The to/from end dates are optional and can be <code>null</code>
   * @param maxRange, non-null, must be a +ve duration
   * @throws IllegalArgumentException if either argument <code>null</code> or <code>maxRange</code>
   *     is negative or 0
   */
  public void restrictDateRange(DateRangeAdjustable dateRangeAdjustable, Duration maxRange) {
    ...
    Validate.isTrue(
        !maxRange.isNegative() && !maxRange.isZero(), "Must supply a positive duration");
    ...
    if (dateRangeAdjustable.getDateFrom() == null && dateRangeAdjustable.getDateTo() == null) {
    ...
    } else if (dateRangeAdjustable.getDateFrom() != null
        && dateRangeAdjustable.getDateTo() == null) {
      ...
      if (range.isNegative()) {
        throw new IllegalArgumentException(
            String.format(
                "Time range cannot be negative but was to: %s, from :%s",
