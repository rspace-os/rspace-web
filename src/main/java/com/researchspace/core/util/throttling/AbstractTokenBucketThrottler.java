...
/**
 * Implementation of a Bucket Throttler that subclasses can customise. See
...
public abstract class AbstractTokenBucketThrottler {
   ...
   * @throws IllegalArgumentException if any argument is null or <code>throttleRateDefinitions
   *     </code> is empty
   */
  public AbstractTokenBucketThrottler(
    ...
    Validate.isTrue(
        throttleLimitDefinitions.getDefinitionCount() > 0,
  ...
  /**
   * Template method for iterating over throttle limits and determinging if any throttle limits have
   * been exceeded.
   */
  protected final void doProceed(
      AllowanceTracker userTracker,
      DateTime current,
      long interval,
      Double requestedResourceUnits) {
    for (Map.Entry<ThrottleInterval, ThrottleLimitDefinition> pair :
        throttleLimitDefinitions.getThrottleLimits().entrySet()) {
      log.trace("interval (millis) between requests is: {} ms", interval);
      userTracker.setLastCheck(current);
      double allowance =
          calculateAllowance(
              current, interval, userTracker.getAllowance(pair.getKey()), pair.getValue());
      userTracker.getAllowances().put(pair.getKey(), allowance);
      log.debug(
          "allowance for  user {} for throttle definition {} is {} {}",
          userTracker.getUserId(),
          pair.getKey(),
          String.format("%.4f", userTracker.getAllowance(pair.getKey())),
          pair.getValue().getUnits());
  ...
  /**
   * Calculates if allowance is large enough to handle requestedResourceUnits. Subclasses may
   * override.
   *
   ...
   * @return
   */
  protected abstract String getThrottleLimitExceededMessage(
      Map.Entry<ThrottleInterval, ThrottleLimitDefinition> pair);

  private double calculateAllowance(
      DateTime current, long interval, double allowance, ThrottleLimitDefinition ratePer) {
