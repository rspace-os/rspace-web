 ...
 *
 * <p>If a new CSS class is added that identifies an Rspace -generated link, image or other HTML
 * element then we need to add this to {@link #RSPACE_IMAGE_CSSCLASSES}.
 */
public class FieldParserConstants {
  ...
  private static final String[] FIELDPARSER_KEYS =
      new String[] {
        ATTACH_ELEMENTS_KEY,
        VIDEOS_ELEMENTS_KEY,
        AUDIOS_ELEMENTS_KEY,
        IMAGES_ELEMENTS_KEY,
        IMAGEANNOTATIONS_ELEMENTS_KEY,
        SKETCHES_ELEMENTS_KEY,
        CHEM_ELEMENTS_KEY,
        CHEM_ATTACHMENT_KEY,
        MATH_ELEMENTS_KEY,
        LINKEDRECORDS_ELEMENTS_KEY,
        COMMENTS_ELEMENTS_KEY
      };

  /** Identifies a link as a link to an external file, not an internal Gallery file. */
  ...
  private static final String[] RSPACE_IMAGE_CSSCLASSES =
      new String[] {
        ATTACHMENT_CLASSNAME,
        AUDIO_CLASSNAME,
        CHEM_IMG_CLASSNAME,
        CHEM_ATTACHMENT_CLASSNAME,
        COMMENT_CLASS_NAME,
        IMAGE_DROPPED_CLASS_NAME,
        LINKEDRECORD_CLASS_NAME,
        SKETCH_IMG_CLASSNAME,
        VIDEO_CLASSNAME,
        ANNOTATION_IMG_CLASSNAME,
        MATH_CLASSNAME
      };

  /**
   * Gets an array of CSS classes that are used for content inserted into TinyMCE by RSpace
   * operations.
   *
   * @return
   */
  ...
  /**
   * Boolean test fo whether the argument is a known RSpace css class name that identifies linked
   * objects
   *
   * @param toTest
   * @return
