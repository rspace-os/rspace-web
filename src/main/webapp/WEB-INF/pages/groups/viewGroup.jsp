...
<div id="groupBlocks">
  <div id="groupBlockLeft">
    <h2 style="flex-grow:1;">
      <spring:message code="groups.view.heading">
        <spring:argument value="${group.groupType}"/>
        <spring:argument value='<span class="displayname">${group.displayName}</span>'/>
      </spring:message>
    </h2>
                ...
                <c:forEach var="grp" items="${userGroups}">
                  <c:if test="${grp.id ne group.id}">
                    <c:url value="/groups/view/${grp.id}"
                      var="groupURL"></c:url>
                    <li><a href="${groupURL}">
                        ${grp.displayName} (<spring:message code="${grp.groupType.labelKey}"/>)</a></li>
                  </c:if>
                </c:forEach>
              </ul>
            </dd>
