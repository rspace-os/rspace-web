...
const NameCell = withStyles<{ material: Material }, { withBadge: string; withoutBadge: string }>(() => ({
    withBadge: {
        "@media print": {
            display: "none",
        },
...
>(() => ({
    ...
    <span className={clsx(className, classes.root)} style={{ flex }} data-test-id={datatestid}>
        {children}
    </span>
));
...
function MaterialsTable({ list, isSingleColumn, onRemove, canEdit }: TableArgs): React.ReactNode {
    ...
    }, [list.mixedSelectedCategories, list.selectedMaterials.forEach, list.setAdditionalQuantity]);
    ...
    const ResponsiveMaterialsHeaderRow = observer(() => {
        ...
        return (
            <TableRow
                className={clsx(classes.tableRow, isSingleColumn ? classes.tableRowMobile : classes.tableRowDesktop)}
            >
                <TableCell
                    className={clsx(
                        classes.tableRowCell,
                        isSingleColumn ? classes.tableRowCellMobile : classes.tableRowCellDesktop,
                    )}
                >
                ...
                {editingMode ? (
                    ...
                    >
                        <div style={{ flex: 3 }} className={classes.columnWrapper}>
                            <span className={clsx(!isSingleColumn && classes.centeredText)}>Batch Edit</span>
                            <Select
                                sx={{ width: "75px" }}
                    ...
                    >
                        <span style={{ flex: 5 }} className={clsx(!isSingleColumn && classes.centeredText)}>
                            Location
                        </span>
                        <span style={{ flex: 2 }} className={classes.centeredText}>
                            Global ID
    ...
    const CustomTableCell = observer(({ className, children }: { className?: string; children: React.ReactNode }) => {
        return (
            <TableCell
                className={clsx(
                    className,
