...
import { doNotAwait, preventEventBubbling } from "../../util/Util";
import MaterialsTable from "./MaterialsTable";
import PopoutPrintIcon from "./PopoutPrintIcon";

const EmptyListText = ({ currentList }: { currentList: ListOfMaterials | null | undefined }) =>
...
const CardWrapperInternals = forwardRef<React.ElementRef<typeof Grid>, CardWrapperInternalsArgs>(
...
const CardWrapper = withStyles<Omit<React.ComponentProps<typeof CardWrapperInternals>, "classes">, { root: string }>(
    () => ({
        root: {
            position: "absolute",
            right: 0,
...
const CustomDialog = withStyles<React.ComponentProps<typeof Dialog>, { paper?: string }>((_theme, { fullScreen }) => ({
    paper: {
        overflow: "hidden",
...
const BigButton = withStyles<{ onClick: () => void; icon: React.ReactNode }, { root: string }>((theme) => ({
    root: {
        padding: 0,
        cursor: "pointer",
    ...
    <IconButton classes={classes} component="div" size="small" color="primary" onClick={onClick}>
        {icon}
    </IconButton>
));
...
const ActionsBar = observer(
    ({
        ...
        return (
            <Grid container spacing={2} className={clsx(classes.actionsBar, classes.hideWhenPrinting)}>
                <Grid item>
                    <Button
                        color="primary"
                        variant="contained"
...
function MaterialsDialog({ open, setOpen, standalonePage = false }: DialogArgs): React.ReactNode {
    ...
    }, [currentList.pickerSearch.fetcher.performInitialSearch, currentList?.pickerSearch]);
                            ...
                            >
                                ...
                                <DialogContent className={classes.contentWrapper}>
                                    <Grid container>
                                        ...
                                        <Slide in={openSlide} direction="left" onClick={preventEventBubbling(() => {})}>
                                            <CardWrapper>
