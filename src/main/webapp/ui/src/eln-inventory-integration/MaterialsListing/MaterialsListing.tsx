...
const WrappingMenuItem = withStyles<React.ComponentProps<typeof MenuItem>, { root: string }>(() => ({
    root: {
        display: "flex",
...
const NewLoMButtonWrapper = withStyles<{ children: React.ReactNode }, { root: string }>(() => ({
    root: {
        "@media print": {
            display: "none",
        },
...
const useStyles = makeStyles<{ fabRightPadding: number }>()((theme, { fabRightPadding }) => ({
    launcherWrapper: {
        position: "absolute",
        top: FAB_SIZE,
...
const CustomBadge = withStyles<{ children: React.ReactNode; count: number }, { root: string; badge: string }>(() => ({
    root: {
        position: "sticky",
        top: FAB_SIZE,
        zIndex: 1, // so it appears above the TinyMCE Editor
                                                ...
                                                <MenuList>
                                                    {fieldListings?.map((list, i) => (
                                                        ...
                                                        >
                                                            ...
                                                            <em className={classes.itemText}>{list.description}</em>
                                                        </WrappingMenuItem>
                                                    ))}
                                                </MenuList>
                                            </ClickAwayListener>
...
const MaterialsListing = observer(({ elnFieldId, canEdit, fabRightPadding }: MaterialsListingArgs) => {
    ...
    }, [materialsStore.setup]);

    useEffect(() => {
        if (!loading) void materialsStore.getFieldMaterialsListings(parseInt(elnFieldId, 10));
    }, [loading, elnFieldId, materialsStore.getFieldMaterialsListings]);
...
function initListOfMaterials({
    ...
    [...document.getElementsByClassName("invMaterialsListing")].forEach((wrapperDiv) => {
