...
const AdditionalOptionButton = ({ field }: { field: FieldModel }): React.ReactNode => {
...
function DefaultValueField({ field, editing }: DefaultValueFieldArgs): React.ReactNode {
        ...
        field.setAttributesDirty(
            match<void, object>([
                ...
                [
                    () => field.type === "date",
                    {
                        content: value ? truncateIsoTimestamp(String(value), "date").orElse("NaN-NaN-NaN") : null,
                    },
                ],
                [() => true, { content: value }],
            ])(),
    ...
    return (
        ...
        >
            {custom}
            {hasOptions(field.fieldType) && editing && <AdditionalOptionButton field={field} />}
        </InputWrapper>
    );
}
