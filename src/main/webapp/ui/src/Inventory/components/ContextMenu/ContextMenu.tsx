...
function ContextMenu({
    ...
    const mixedSelectedStatus = selectedResults.some((r) => r.deleted) && selectedResults.some((r) => !r.deleted); // affecting all except select
    ...
    const widthOfRef = (ref: { current: HTMLElement | null }): number => ref.current?.offsetWidth ?? 0;
    ...
    const calculateWhatIsInOverflowMenu = () => {
        ...
        setTimeout(() => {
            const buttonWidths = actions("button").map((_a, i) => widthOfRef(buttons[i].ref));
            const availableSpace = widthOfRef(spaceForButtons);
            const cumulativeWidth =
                buttonWidths.reduce((totalWidth, buttonWidth, i) => {
                    totalWidth += Math.ceil(buttonWidth + (buttonWidth > 0 ? MENU_PADDING : 0));
    ...
    const containerObserver = useRef(new ResizeObserver(calculateWhatIsInOverflowMenu));
                    ...
                    >
                        <Grid container>
                            {actions("button").map((action: ContextAction, i) =>
                                !action.hidden ? (
                                    <GridItem key={i}>
                                        <Box ref={buttons[i].ref as React.RefObject<HTMLDivElement>}>
                                            {action.component}
                                        </Box>
                                    </GridItem>
                                ) : null,
                    ...
                    <GridItem className={classes.moreButtonWrapper}>
                        ...
                        >
                            {actions("menuitem").map((action: ContextAction, i) =>
                                !action.hidden && buttons[i]?.inOverflow ? action.component : null,
                            )}
                        </StyledMenu>
                    </GridItem>
                </Grid>
