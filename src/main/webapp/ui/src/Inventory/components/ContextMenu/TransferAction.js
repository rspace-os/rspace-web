...
const TransferAction: ComponentType<TransferActionArgs> = forwardRef(
  ({ as, selectedResults, disabled, closeMenu }: TransferActionArgs, ref) => {
    const { search } = useContext(SearchContext);
    const [btnPos, setBtnPos] = useState<?{ top: number, left: number }>(null);
    const [recipient, setRecipient] = useState<?Person>(null);
    ...
    const ownersOfSelectedResults = new RsSet(selectedResults)
      .filter(({ owner }) => Boolean(owner))
      .map(
        // $FlowExpectedError[incompatible-use] All these records have owners
        (r) => r.owner.username
      );
