...
const HelperText = () => (
    <DialogContentText component="span">
        <Typography variant="body1" paragraph>
            Select someone to transfer ownership to. By performing this action you will give the new owner full control
            over the item. <strong>This action can only be undone by the recipient or their PI.</strong>
        </Typography>
        <Typography variant="body1" paragraph>
...
const TransferAction = forwardRef<React.ElementRef<typeof ContextMenuAction>, TransferActionArgs>(
    ({ as, selectedResults, disabled, closeMenu }, ref) => {
        ...
        const ownersOfSelectedResults = new RsSet<Username>(
            selectedResults.filter(({ owner }) => Boolean(owner)).map((r) => r.owner?.username),
        );
        const excludedFromSelection =
            ownersOfSelectedResults.size === 1 ? ownersOfSelectedResults : new RsSet<Username>();
