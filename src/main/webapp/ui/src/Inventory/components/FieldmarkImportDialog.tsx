    ...
    return (
        <ThemeProvider theme={createAccentedTheme(ACCENT_COLOR)}>
            <Dialog open={open} onClose={handleClose} maxWidth="lg" fullWidth fullScreen={isViewportSmall}>
                ...
                <DialogContent>
                    <Grid container direction="column" spacing={2} sx={{ height: "100%", flexWrap: "nowrap" }}>
                        <Grid item>
                            <Typography variant="body2" sx={{ maxWidth: "54em" /* entirely arbitrary */ }}>
                                Choose a Fieldmark notebook to import into Inventory. A Sample will be created for each
                                record inside the notebook. A new list container will be placed on your bench,
                                containing a singular subsample for each sample.
                            </Typography>
                                ...
                                <>
                                    {showIgsnMessage && (
                                        <Typography variant="body2" sx={{ ml: 0.5 }}>
                                            ...
                                            <Link href={docLinks.IGSNIdentifiers}>DataCite IGSN ID integration</Link> to
                                            be enabled.
                                        </Typography>
                                    )}
                                    {fetchingIdentifierFields ? (
                                        ...
                                        identifierFields.length > 0 && (
                                            <FormControl sx={{ width: "auto", minWidth: 200 }}>
                                                ...
                                                <FormHelperText>
                                                    Select a field to use as the IGSN ID for imported samples, or select
                                                    'Do not use IGSN IDs' to import without one
                                                </FormHelperText>
                                            </FormControl>
                                        )
                                    )}
                        ...
                        <Grid item sx={{ ml: "auto" }}>
                            <Stack direction="row" spacing={1}>
                                ...
                                <ValidatingSubmitButton
                                    onClick={() => {
                                        if (selectedNotebook)
                                            void importNotebook(selectedNotebook).then(() => handleClose());
                                    }}
                                    validationResult={
                                        !selectedNotebook ? IsInvalid("No Notebook selected.") : IsValid()
                                    }
                                    loading={importing}
                                >
                                    Import
