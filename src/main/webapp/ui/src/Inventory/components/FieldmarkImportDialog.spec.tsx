    ...
    When: async ({ page }, use) => {
        await use({
            ...
            "the user selects a notebook and clicks import": async () => {
                await page.getByRole("radio", { name: "Select notebook: Test Notebook 1" }).click();
    ...
    Then: async ({ page, networkRequests }, use) => {
        await use({
            ...
            "a success alert should be visible": async () => {
                ...
                await alert.getByRole("button", { name: "1 sub-messages. Toggle to show" }).click();
            ...
            "a detailed error alert should be visible": async () => {
                ...
                await alert.getByRole("button", { name: "2 sub-messages. Toggle to show" }).click();
...
test.describe("FieldmarkImportDialog", () => {
    test.describe("accessibility", () => {
        feature("should not have any accessibility issues when open", async ({ Given, When, Then }) => {
    ...
    test.describe("notebook import", () => {
        feature(
            ...
            async ({ Given, When, Then }) => {
                ...
                Then["an import request should be made to the server with the correct notebook ID"]();
            },
        );
        ...
        feature(
            ...
            async ({ Given, When, Then }) => {
                ...
                Then["an import request should be made with the selected identifier column"]();
            },
        );

        feature("should show importing alert during import process", async ({ Given, When, Then, router }) => {
            await router.route("/api/inventory/v1/import/fieldmark/notebook", async (route) => {
        ...
        feature("should show loading state on import button during import", async ({ Given, When, Then, router }) => {
            await router.route("/api/inventory/v1/import/fieldmark/notebook", async (route) => {
                // Add a longer delay to ensure importing alert is visible long enough for tests
                await sleep(2500);
                await route.fulfill({
