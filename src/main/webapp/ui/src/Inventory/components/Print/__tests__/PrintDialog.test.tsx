...
describe("Print Tests", () => {
  ...
  const renderDialog = async () => {
    render(<Dialog />);
    await waitFor(() => {
      expect(screen.queryByText("print.dialog.loading")).not.toBeInTheDocument();
    });
  };
  describe("PrintDialog with items to print (barcodes)", () => {
    test("renders, has radio options for printerType, printMode, printSize (plus help text)", async () => {
      ...
      const standardOption = screen.getByLabelText("print.options.standardPrinter");
      expect(standardOption).toBeInTheDocument();

      expect(standardOption).toBeChecked();
      const labelOption = screen.getByLabelText("print.options.labelPrinter");
      expect(labelOption).toBeInTheDocument();

      expect(labelOption).not.toBeChecked();
      const previewHeader = "print.dialog.previewLabelLayout";
  ...
  describe("PrintDialog with items to print (barcodes)", () => {
    test("renders, content responds to clicked options", async () => {
      ...
      const basicOption = screen.getAllByLabelText("print.options.basic")[0];
      await act(async () => {
