...
describe("PublishButton is not offered where publishing can never succeed", () => {
  ...
  test("still offers Publish, disabled, while a review is open", () => {
    renderButton(identifier({ doiType: "PIDINST_B2INST", state: "submitted" }));
    expect(screen.getByRole("button", { name: PUBLISH })).toBeDisabled();
  });

  /* a review created but never submitted is driven forward by pressing Publish again */
  test("still offers Publish for a created review", () => {
    renderButton(identifier({ doiType: "PIDINST_B2INST", state: "created" }));
