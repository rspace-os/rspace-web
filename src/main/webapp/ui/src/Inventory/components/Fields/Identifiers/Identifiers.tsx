...
import { capitaliseJustFirstChar, doNotAwait, match } from "../../../../util/Util";
...
const IdentifierWrapper = observer(
    ({
        ...
        const handleUpdate = (f: IdentifierField, value: string | number | CreatorType) => {
        ...
        return (
            <>
                                ...
                                <InputWrapper label={f.key}>
                                    {editable && isRadio(f) ? (
                                        ...
                                        <TextField
                                            ...
                                            error={
                                                editable &&
                                                (f.value === "" || !(f.isValid ?? ((_: any) => true))(f.value))
                                            }
                                            helperText={
                                                editable &&
                                                (f.value === "" || !(f.isValid ?? ((_: any) => true))(f.value))
                ...
                <section>
                    ...
                    <Alert severity="info">
                        You can include Inventory fields in the item's landing page, to openly share domain-specific
                        metadata outside the IGSN schema.{" "}
                        <strong>
                            Before publishing the IGSN ID, please ensure the fields do not contain sensitive
                            information.
                        </strong>
                    </Alert>
                    <FormControlLabel
        ...
        if (identifierState === "findable")
            return (
                <>
                    ...
                    . The metadata is publicly available through the landing page, DataCite Commons and the DataCite
                    APIs.
                </>
            );
        ...
        <Grid mt={1} sx={{ padding: "0px 12px" }}>
            ...
            >
                {activeResult.state === "preview" && activeResult.identifiers.length > 0 && (
                    <Alert severity="info" className={classes.bottomSpaced} sx={{ width: "100%" }}>
                    ...
                    <Grid item key={id.doi} sx={{ width: "100%" }}>
                        ...
                        <Grid container direction="row" spacing={1} sx={{ width: "100%", marginBottom: "8px" }}>
                            ...
                            <Grid
                                item
                                xs={2}
                                className={id.state === "findable" ? classes.highlight : undefined}
                        ...
                        >
                            ...
                            <Grid item>
                                <PublishButton identifier={id} disabled={activeResult.state === "edit"} />
                            </Grid>
                            <Grid item>
                                <CustomTooltip
                                    title={
                                ...
                                >
                                    <Button
                                        ...
                                        size="small"
                                        onClick={
                                            id.state === "draft" ? () => handleDelete(id) : () => handleRetract(id)
                                        }
                                        disabled={activeResult.state === "edit" || id.state === "registered"}
                                    >
                    ...
                    <DialogContent>
                        <Stack spacing={2}>
                            ...
                            <Alert severity="warning">
                                <strong>This action cannot be undone!</strong> Once an IGSN ID has been associated with
                                an item, it cannot be later re-associated with a different Inventory item; a new IGSN ID
        ...
        <>
            ...
            {activeResult.state !== "create" && identifiers.length === 0 && (
                <Stack direction="row" spacing={1}>
                    <Button color="primary" variant="outlined" onClick={doNotAwait(() => activeResult.addIdentifier())}>
                        Create new IGSN ID
                    </Button>
                    <Button
                        color="primary"
