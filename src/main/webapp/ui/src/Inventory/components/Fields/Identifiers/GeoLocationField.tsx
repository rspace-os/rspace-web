...
const CustomFieldset = withStyles<{ children: ReactNode }, { root: string }>((theme) => ({
    root: {
...
}))(({ classes, children }) => <fieldset className={classes.root}>{children}</fieldset>);
        ...
        ({ setOpenPolygonDialog }: { setOpenPolygonDialog: (open: boolean) => void }): ReactNode => {
            return (
                <Card variant="outlined">
                    <CardContent>
                        <Alert severity="info">
                            At least one of four elements must be completed (Point, Place, Box, Polygon). Point, Box and
                            Polygon cannot be completed partially.
                        </Alert>
        ...
        ({ setOpenPolygonDialog }: { setOpenPolygonDialog: (open: boolean) => void }): ReactNode => {
            ...
            return (
                <Card variant="outlined">
                    <CardMedia>
                        <Suspense
                            fallback={<Skeleton variant="rectangular" width="100%" height={200} sx={{ mb: 1 }} />}
                        >
                            <LazyMapViewer
