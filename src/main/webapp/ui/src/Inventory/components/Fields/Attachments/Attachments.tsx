...
const CustomCardHeader = withStyles<React.ComponentProps<typeof CardHeader>, { root: string; action: string }>(
    (theme) => ({
        root: {
                ...
                <React.Suspense fallback={<></>}>
                    <GalleryPicker
                        ...
                        onSubmit={(files) => {
                            activeResult.setAttributesDirty({
                                attachments: [
                                    ...activeResult.attachments,
                                    ...files.map((f) =>
                                        newGalleryAttachment(f, () => activeResult.setAttributesDirty({})),
                                    ),
                                ],
                            });
                            setGalleryDialogOpen(false);
                        ...
                        validateSelection={(file) => {
                            if (file.isSnippet)
                                return Result.Error([new Error("Snippets cannot be attached to Inventory records.")]);
                            if (file.isFolder)
                                return Result.Error([new Error("Folders cannot be attached to Inventory records.")]);
                            if (!file.globalId)
                                return Result.Error([
        ...
        return (
            <Card variant="outlined">
                <CustomCardHeader
                    ...
                    action=<ToggleButton attachmentCount={attachments.length} open={open} setOpen={setOpen} />
                />
                {editable && (
