...
const MaxSizeCard = withStyles<{ elevation: number; testId?: string; children: React.ReactNode }, { root: string }>(
    (theme) => ({
        root: {
            height: "100%",
            width: "100%",
...
const CustomCardHeader = withStyles<{ title: React.ReactNode }, { root: string }>(() => ({
    root: {
        flexWrap: "nowrap",
    },
}))(({ title, classes }) => <CardHeader title={title} classes={classes} sx={{ py: 1 }} />);
...
function InventoryPicker({
    ...
    if (search.searchView === "TREE" && search.tree.selectedNode) selectedRecords.push(search.tree.selectedNode);
    const selectedAndNotAllowed = new RsSet(selectedRecords).filter(search.alwaysFilterOut);
            ...
            {showActions && (
                <CardActions>
                    <Button
                        ...
                        disabled={selectedRecords.length === 0 || !selectedAndNotAllowed.isEmpty}
                    >
                        Choose
                    </Button>
                    <Button
