...
import type { BlobUrl, emptyObject } from "../../../util/types";
...
>((theme, { navigateOnClick }) => ({
    ...
    return <CardStructure className={clsx(classes.card, greyOut && globalClasses.greyOut)} {...rest} />;
});

const Details = withStyles<{ record: InventoryRecord }, { location: string }>((theme) => ({
    location: {
    ...
    <DescriptionList
        content={[
            ...
            ...(record.readAccessLevel === "full" && record instanceof SubSampleModel
                ...
                : []),
            ...(record.readAccessLevel !== "public" && record instanceof SubSampleModel
                ? [
                      {
                          label: "Sample",
...
const Modified = withStyles<{ record: InventoryRecord }, { container: string }>((theme) => ({
    container: {
    ...
    <div className={classes.container}>
        <span>Modified </span>
        <TimeAgoCustom time={record.lastModified} formatter={(value, unit, suffix) => `${value}${unit[0]} ${suffix}`} />
        <span> by </span>
        {record.modifiedByFullName}
    </div>
));

const ImagePlaceholder = withStyles<emptyObject, { media: string; icon: string }>((theme) => ({
    media: {
        display: "flex",
...
const useStyles = makeStyles<{ fetching: boolean }>()((theme, { fetching }) => ({
    menuButton: {
...
function RecordCard({ record }: CardArgs): React.ReactNode {
    const { search, disabled, isChild, scopedResult, differentSearchForSettingActiveResult } =
    ...
    return (
        <>
            ...
            <CustomCardStructure
                ...
                headerAvatar={
                    <RecordTypeIcon record={record} color={record.deleted ? theme.palette.deletedGrey : undefined} />
                }
                title={record.name}
                ...
                headerAction={
                    <>
                        <IconButton
                            ...
                            onClick={preventEventBubbling((e: React.MouseEvent<HTMLButtonElement>) => {
                        ...
                        >
                            {menuItems
                                ...
                                .map(({ component }: { hidden: boolean; component: React.ReactNode }) => component)}
                        </StyledMenu>
                    </>
                }
                content={<Details record={record} />}
                contentFooter={record.readAccessLevel === "full" ? <Modified record={record} /> : null}
                onClick={() => {
