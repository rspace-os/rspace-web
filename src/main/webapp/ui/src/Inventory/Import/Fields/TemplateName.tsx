...
function TemplateName({ disabled }: TemplateNameArgs): ReactNode {
    ...
    return (
        <TextField
            ...
            helperText={
                error ? "A name for the new template is required and should be no longer than 255 characters." : ""
            }
            onChange={({ target }) => {
                if (target instanceof HTMLInputElement && Boolean(importStore.importData))
