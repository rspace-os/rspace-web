...
jest.mock("../Sample/Content/SubsampleListing", () => jest.fn(() => <div></div>));
jest.mock("../Container/Content/Content", () => jest.fn(() => <div></div>));
jest.mock("../Template/Fields/SamplesList", () => jest.fn(() => <div></div>));
jest.mock("../components/Fields/Attachments/Attachments", () => jest.fn(() => <div></div>));
jest.mock("../components/Fields/ExtraFields/ExtraFields", () => jest.fn(() => <div></div>));
jest.mock("../components/ContextMenu/ContextMenu", () => jest.fn(() => <div></div>));
jest.mock("../Sample/Fields/Template/Template", () => jest.fn(() => <div></div>));
...
function getSectionNames(reactComponent: React.ReactNode, rootStore: MakeRootStoreArgs): Array<string> {
    ...
    const sectionNames = screen
        .getAllByRole("region")
        .map((r) => within(r).getByRole("heading", { level: 3 }).textContent as string);
    cleanup();
    return sectionNames;
}
...
describe("Form Section Order", () => {
    test("Across all of the forms, all of the sections should be in a consistent order.", () => {
        window.ResizeObserver = ResizeObserver as unknown as typeof global.ResizeObserver;
        ...
        assertConsistentOrderOfLists(
            new Map([
                ...
                [
                    ...
                    getSectionNames(
                        <MixedBatchForm records={new RsSet([makeMockSubSample(), makeMockContainer()])} />,
                        {
                            activeResult: null,
