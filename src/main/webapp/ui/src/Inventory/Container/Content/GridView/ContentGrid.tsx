 ...
 */
const WrappedGridCell = observer((props: Omit<GridCellArgs, "hoverEffect">) => {
...
const LoadedContent = observer(({ container }: LoadedContentProps) => {
     ...
     * model.
     */
    const [tabIndexCoord, setTabIndexCoord] = useState<Coord>({ x: 1, y: 1 });
                ...
                <Table
                    ...
                    onKeyDown={(e: KeyboardEvent) => {
                        ...
                        container.locations?.forEach((l) => {
                            l.toggleSelected(
                                l.coordX >= left && l.coordX <= right && l.coordY >= top && l.coordY <= bottom,
                            );
                        });
                                    ...
                                    container.columns.map((col, index) => (
                                        ...
                                        >
                                            <LocationContent
                                                ...
                                                container={container}
                                                tabIndex={
                                                    tabIndexCoord.x === col.value && tabIndexCoord.y === row.value
                                                        ? 0
                                                        : -1
                                                }
    ...
    return (
        <>
            {(!container.initializedLocations || container.loading) && (
                <TableContainer>
                    <Table stickyHeader role="grid" className={classes.table} padding="normal" size="small">
                        <TableHead>
                            <TableRow>
...
export default observer(ContentGrid);
