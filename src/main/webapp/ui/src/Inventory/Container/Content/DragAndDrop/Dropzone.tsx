...
const AllowedIcon = styled(({ allowed, className }: { allowed: boolean; className?: string }) => (
...
export const Dropzone = observer(({ children, location }: DropzoneArgs): ReactNode => {
    const { thisLocationIsTheOrigin, dragAndDropInProgress, anItemIsBeingMoveOutOfLocation, isChoosing } = useHelpers();
    ...
    return (
        ...
        >
            {renderContent()}
            {dragAndDropInProgress && isChoosing(location) && <AllowedIcon allowed={allowed} />}
        </WrapperDiv>
    );
});
