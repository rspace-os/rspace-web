...
const selectionStyle = (location: Location, classes: Record<string, string>, search: Search): Class => {
    if (location.isShallowUnselected(search)) {
        return classes.shallowUnselected;
    }
    if (location.isShallowSelected(search)) {
        return location.isSelectable(search) ? classes.shallowSelected : classes.shallowSelectedUnselectable;
    }
...
const useWrapperClasses = (
    ...
    const ret = new Set([classes.wrapper, selectionStyle(location, classes, search)]);
...
const EmptyLocationContent = ({
    ...
    if (location.parentContainer.cType === "IMAGE") {
        ...
        return <div className={classes.emptyImageCellContentWrapper}>{imageIndex()}</div>;
    }
    return <></>;
};
        ...
        <div className={wrapperClassName}>
            <RelativeBox className={classes.locationContentBox}>
                {location.content ? (
                    ...
                    <EmptyLocationContent location={location} tabIndex={tabIndex} hasFocus={hasFocus} />
                )}
            </RelativeBox>
        </div>
    );
