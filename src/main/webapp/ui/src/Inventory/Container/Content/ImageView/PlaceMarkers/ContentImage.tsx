...
const useStyles = makeStyles<{ editable: boolean }>()((_theme, { editable }) => ({
    rounded: {
        padding: 0,
        width: "auto",
        height: "auto",
...
function ContentImage({
    ...
    const resizeObserver = useRef(
        new ResizeObserver((entries) => {
            setImageDimensions(pick("width", "height", "left", "top")(entries[0].target.getBoundingClientRect()));
        }),
    );
    ...
    const newMarker = (event: React.MouseEvent<HTMLImageElement>) => {
        ...
        const newPoint = {
            x: clamp(
                tappedLocationRelativeToViewport.x - imgTopLeftCornerRelativeToViewport.x,
                iconDimension / 2,
                imageWidth - iconDimension / 2,
            ),
            y: clamp(
                tappedLocationRelativeToViewport.y - imgTopLeftCornerRelativeToViewport.y,
                iconDimension,
                imageHeight,
            ),
        };
