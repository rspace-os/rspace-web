...
describe("PermalinkNotFound", () => {
  test("a versioned permalink names the missing version and links to the latest", () => {
    ...
    expect(screen.getByText("inventory:permalinkNotFound.versionedTitle")).toBeVisible();
    const link = screen.getByRole("link", { name: "inventory:permalinkNotFound.latestLink" });
  ...
  test("an unversioned permalink shows the generic not-found message", () => {
    ...
    expect(screen.getByText("inventory:permalinkNotFound.unversionedTitle")).toBeVisible();
