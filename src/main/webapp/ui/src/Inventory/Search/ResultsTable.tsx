...
const CustomTablePagination = withStyles<React.ComponentProps<typeof TablePagination>, { root: string }>(() => ({
    root: {
        overflow: "unset",
    },
}))((props) => (
...
function ResultsTable({ contextMenuId }: ResultsTableArgs): React.ReactNode {
    ...
    const toggleAll = () => {
        const results = search.filteredResults.filter((r) => !search.alwaysFilterOut(r));
    ...
    const onSelectOptions: Array<SplitButtonOption> = [
        ...
        },
        {
            text: "Mine",
            selection: () => {
                search.filteredResults.forEach((r) => r.toggleSelected(r.currentUserIsOwner ?? false));
            },
        },
        {
            text: "Not Mine",
