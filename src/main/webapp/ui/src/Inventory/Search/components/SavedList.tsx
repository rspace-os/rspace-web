...
const DeleteSavedItemIcon = withStyles<React.ComponentProps<typeof DeleteIcon>, { root: string }>((theme) => ({
    root: {
        color: theme.palette.warningRed,
    },
}))(DeleteIcon);

const EditSavedItemIcon = withStyles<React.ComponentProps<typeof EditIcon>, { root: string }>((theme) => ({
    root: {
        color: theme.palette.primary.main,
    },
}))(EditIcon);

const Action = withStyles<React.ComponentProps<typeof ListItemSecondaryAction>, { root: string }>((theme) => ({
    root: {
        "& .MuiIconButton-root": {
        ...
        <>
            <StyledMenu anchorEl={anchorEl} open={Boolean(anchorEl)} onClose={() => onSelect(null)}>
                {items.length > 0 ? (
                    ...
                    <Alert className={classes.alert} severity="info" onClose={() => onSelect(null)}>
                        {helpText[itemType]}
                    </Alert>
                )}
            </StyledMenu>
