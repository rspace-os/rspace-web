...
const CustomChip = withStyles<React.ComponentProps<typeof Chip>, { label: string; root: string }>(() => ({
    label: {
...
const CustomGridItem = withStyles<{ children: React.ReactNode }, { root: string }>(() => ({
    root: {
        maxWidth: "100%",
    },
}))(({ children, classes }) => (
    ...
    return (
        <Grid container direction="row" spacing={1}>
            ...
            {search.currentBasket(searchStore.savedBaskets) && (
                <Grid item>
                    <CustomChip
                        label={`Basket: ${search.currentBasket(searchStore.savedBaskets)?.name ?? "UNKNOWN"}`}
                        onDelete={() => {
