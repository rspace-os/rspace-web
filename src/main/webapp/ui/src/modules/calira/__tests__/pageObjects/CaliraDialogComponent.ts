...
export class CaliraDialogComponent {
  ...
  constructor(page: Page) {
    this.root = page.getByRole("dialog", { name: "Insert from Calira" });
    this.frame = this.root.frameLocator("iframe");
    this.resultsTable = this.frame.getByRole("table", { name: "booking search results" });
  ...
  async selectFirstBooking(): Promise<void> {
