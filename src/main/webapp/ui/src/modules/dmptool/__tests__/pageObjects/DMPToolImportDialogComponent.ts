...
export class DMPToolImportDialogComponent {
  ...
  async selectFirstPlan(): Promise<string> {
    ...
    if (!label?.startsWith("Select plan: ")) throw new Error("The first DMPTool plan has no accessible name.");
    await radio.click();
  ...
  async clickImport(): Promise<void> {
    const [response] = await Promise.all([
      this.page.waitForResponse(
        (r) => r.request().method() === "POST" && new URL(r.url()).pathname.includes("/apps/dmptool/jsonById/"),
      ),
      this.importButton.click(),
    ]);
    if (!response.ok()) {
      throw new Error(`POST /apps/dmptool/jsonById failed: ${response.status()} ${response.statusText()}`);
    }
  }
}
