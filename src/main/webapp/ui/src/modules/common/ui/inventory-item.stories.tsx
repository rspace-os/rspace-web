...
export const LocationOnOwnLine: Story = {
  args: {
    ...confocal,
  ...
  render: (args) => <InventoryItem {...args} style={{ width: "192px" }} />,
  play: async ({ canvasElement }) => {
    const canvas = within(canvasElement);
    const id = canvas.getByRole("link", { name: confocal.idLinkLabel });
    const location = canvas.getByRole("link", { name: "Lab 2.14" });
    expect(location.getBoundingClientRect().top).toBeGreaterThanOrEqual(id.getBoundingClientRect().bottom);
  },
};
