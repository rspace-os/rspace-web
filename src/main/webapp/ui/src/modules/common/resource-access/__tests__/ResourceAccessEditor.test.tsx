...
describe("ResourceAccessEditor", () => {
  it("announces loading without exposing assignment controls before access resolves", async () => {
    const response = Promise.withResolvers<Response>();
    server.use(http.get("/api/v2/test-collections/9/access", () => response.promise));
    const { container } = editor();
    try {
      expect(await screen.findByRole("status")).toHaveTextContent("common:resourceAccess.loading");
      expect(screen.getByRole("status")).toHaveClass("sr-only");
      expect(screen.queryByRole("button")).not.toBeInTheDocument();
      expect(screen.queryByText("Ada Owner")).not.toBeInTheDocument();
      await expectAccessible(container);
    } finally {
      response.resolve(new HttpResponse(null, { status: 403 }));
    }
    expect(await screen.findByRole("alert")).toHaveTextContent("common:resourceAccess.loadError");
  ...
  it("pins a dirty draft to its base across refetches, including after conflict recovery", async () => {
    const client = new QueryClient({ defaultOptions: { queries: { retry: false } } });
    const remote = { grantee: { ...second.grantee, id: 13, key: "user:13", name: "Remote reader" }, role: "READER" };
    let current = document(3, [owner, second]);
    const requests: Request[] = [];
    server.use(
      http.get("/api/v2/test-collections/9/access", () => HttpResponse.json(current)),
    ...
    await screen.findByRole("table");
    await user.click(row("Bob Steward").getByRole("button", { name: "common:resourceAccess.removeNamed" }));
    current = document(4, [owner, second, remote]);
    await act(() => client.refetchQueries({ queryKey: ["api-v2", "test-collections", 9, "access"] }));
    expect(row("Bob Steward").getByText("common:resourceAccess.staged.removed")).toBeInTheDocument();
    const save = screen.getByRole("button", { name: "common:resourceAccess.saveChanges" });
    await user.click(save);
    await screen.findByText("common:resourceAccess.conflictMerged");
    expect(requests[0].headers.get("If-Match")).toBe('"3"');
    expect(table().getByText("Remote reader")).toBeInTheDocument();

    current = document(5, [owner, second, { ...remote, role: "CONTRIBUTOR" }]);
    await act(() => client.refetchQueries({ queryKey: ["api-v2", "test-collections", 9, "access"] }));
    await user.click(save);
    ...
    await expect(requests[1].json()).resolves.toEqual({
      assignments: [
        { granteeKey: "user:11", role: "STEWARD" },
        { granteeKey: "user:13", role: "READER" },
      ],
    });
    await waitFor(() =>
      expect(row("Remote reader").getByRole("button", { name: "common:resourceAccess.roleFor" })).toHaveTextContent(
        "Contributor",
      ),
    );
