...
export function ResourceAccessEditor({ resource, resourceId, token, adapter, onLeave, readOnly = false }: Props) {
  ...
  const [base, setBase] = useState<ResourceAccessDocument>();
  ...
  useEffect(() => {
    if (access.data && access.data !== base && !recoveringConflict && (!base || sameDraft(draft, base.assignments))) {
      setBase(access.data);
  ...
  const refreshAfterConflict = async (original = base?.assignments ?? [], local = draft) => {
    setRecoveringConflict(true);
    setConflictRefreshError(false);
    const latest = await access.refetch();
    if (latest.isSuccess && latest.data) {
      const merged = mergeAccessDraft(original, local, latest.data.assignments);
      setBase(latest.data);
  ...
  if (access.isPending)
    return (
      <div aria-busy="true" className="min-w-0 space-y-4 overflow-x-clip">
        <p role="status" className="sr-only">
          {t("resourceAccess.loading")}
        </p>
        <div aria-hidden="true" className="space-y-4">
          <Skeleton className="h-14 w-full max-w-sm" />
          <Skeleton className="h-9 w-full" />
          {[0, 1, 2, 3].map((row) => (
            <Skeleton key={row} className="h-24 w-full sm:h-14" />
          ))}
        </div>
      </div>
    );
  ...
  const saved = { ...access.data, assignments: base?.assignments ?? access.data.assignments };
