...
describe("relationship filter picker", () => {
  ...
  it("searches by global ID through the row ID", async () => {
    ...
    await user.click(screen.getByRole("button", { name: "common:tableList.filters.noneApplied" }));
    await user.click(screen.getByRole("button", { name: "common:tableList.actions.addFilter" }));
    await user.type(screen.getByRole("combobox", { name: "common:tableList.filters.value" }), "IN123");
    ...
    render(
      <QueryClientProvider client={queryClient}>
        <RelationshipPicker
          ...
          value=""
          onChange={onChange}
          ariaLabel="common:tableList.filters.value"
        />
      </QueryClientProvider>,
    );

    const picker = screen.getByRole("combobox", { name: "common:tableList.filters.value" });
