...
async function openFilterOn(field: string) {
  ...
  await user.selectOptions(screen.getByRole("combobox", { name: "common:tableList.filters.field" }), field);
  return user;
}
...
describe("filtering on a relationship target's field", () => {
  it("suggests names from the target collection while keeping the value free text", async () => {
    ...
    const user = await openFilterOn("target.name");
    expect(screen.getByRole("option", { name: "Bookable item: Name" })).toBeInTheDocument();
    // An input bound to a datalist exposes role combobox, not textbox.
    const value = screen.getByRole("combobox", { name: "common:tableList.filters.value" });

    // A datalist rather than a select: the server matches text, so any value is valid.
    expect(screen.getAllByRole("option", { hidden: true }).map((option) => option.getAttribute("value"))).not.toContain(
      "Confocal microscope",
    );
    ...
    // The value is the name, because that is what the server matches and what the box receives.
    await waitFor(() =>
      expect(screen.getAllByRole("option", { hidden: true }).map((option) => option.getAttribute("value"))).toEqual(
        expect.arrayContaining(["Confocal microscope", "Alpha scope"]),
      ),
    );
