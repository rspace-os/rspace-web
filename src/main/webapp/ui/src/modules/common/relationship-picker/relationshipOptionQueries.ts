...
function searchExpression(source: RelationshipSource, term: string): string | null {
  ...
  return id === null
    ? serializeRsql<Record<string, unknown>>(
        { kind: "comparison", field: source.searchField, operator: "contains", value: term },
        { [source.searchField]: { operators: ["=contains="], wildcards: false } },
        filterLimits,
      )
    : serializeRsql<Record<string, unknown>>(
        { kind: "comparison", field: "id", operator: "equals", value: id },
        { id: { operators: ["=="], wildcards: false } },
        filterLimits,
      );
}
