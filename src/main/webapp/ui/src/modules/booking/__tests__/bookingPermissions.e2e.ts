    ...
    test("keeps assignment, role loss, repair, and coordinated transfer coherent", async ({
      ...
      await test.step("role loss retains only the requester's own read-only non-navigable booking", async () => {
        ...
        try {
          ...
          const bookingsTable = session.page.getByRole("table");
          await expect(bookingsTable.getByRole("cell", { name: bookerPurpose, exact: true })).toBeVisible();
          await expect(bookingsTable.getByText("Read-only: you no longer have access to this item.")).toBeVisible();
          const details = session.page.getByRole("link", { name: "View details" });
          await expect(details).toHaveCount(1);
          await expect(session.page.getByRole("link", { name: new RegExp(instrument.globalId) })).toHaveCount(0);
          await details.click();
          await expect(session.page).toHaveURL(new RegExp(`/booking/calendar/bookings/${bookerBooking.id}$`));
          await expect(session.page.getByText(bookerPurpose, { exact: true })).toBeVisible();
          await expect(session.page.getByRole("link", { name: "Return to My Bookings" })).toBeVisible();
          await expect(session.page.getByRole("link", { name: "Edit" })).toHaveCount(0);
          await expect(session.page.getByRole("button", { name: "Cancel booking" })).toHaveCount(0);
          await expect(session.page.getByRole("button", { name: /^\.ics file/ })).toHaveCount(0);
          await expect(session.page.getByRole("link", { name: new RegExp(instrument.globalId) })).toHaveCount(0);
        } finally {
          await session.close();
        }
