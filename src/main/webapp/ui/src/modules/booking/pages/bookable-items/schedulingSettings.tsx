...
export async function saveBookingSettings(
  ...
  const response = await fetch("/api/v2/booking-settings", {
    ...
    body: JSON.stringify({ ...parseOrThrow(SchedulingSettingsSchema, input), configurationVersion }),
  });
...
export function SchedulingSettingsFields({ form, disabled = false }: { form: FormStore; disabled?: boolean }) {
