...
describe("addRaidIdentifierAjax", () => {
  ...
  it("should handle empty raidTitle", async () => {
    fetchMock.mockResponseOnce("", { status: 201 });
    ...
    const result = await addRaidIdentifierAjax(paramsWithEmptyTitle);
    ...
    expect(requestBody.raid.raidTitle).toBe("");
  });
  ...
  it("should handle special characters in parameters", async () => {
    ...
    const paramsWithSpecialChars = {
      ...
      raidTitle: 'Test "Project" with special chars & symbols',
    };
    ...
    expect(requestBody.raid.raidTitle).toBe(
      'Test "Project" with special chars & symbols'
    );
  });
});
