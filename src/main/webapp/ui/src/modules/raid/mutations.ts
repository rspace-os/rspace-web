...
import { AssociateRaidIdentifierRequestBody } from "@/modules/raid/schema";
...
export const addRaidIdentifierAjax = async ({ groupId, raidServerAlias, raidIdentifier, raidTitle }: { groupId: string, raidServerAlias: string; raidIdentifier: string; raidTitle: string }) => {
  ...
  if (!response.ok) {
    throw new Error(
      `Failed to add RaID identifier: ${response.statusText}`,
    );
  }
...
export const useAddRaidIdentifierMutation = ({ groupId }: { groupId: string }) => {
  ...
  return useMutation({
    mutationFn: ({
      ...
      raidTitle: string;
    }) => addRaidIdentifierAjax({ groupId, raidServerAlias, raidIdentifier, raidTitle }),
    onSuccess: async () => {
...
export const removeRaidIdentifierAjax = async ({ groupId }: { groupId: string }) => {
  ...
  if (!response.ok) {
    throw new Error(
      `Failed to remove RaID identifier: ${response.statusText}`,
    );
  }
