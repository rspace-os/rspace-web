...
import {
  getLinkedByRecords,
  getLinkedDocuments,
} from "@/modules/workspace/linkedRecords";
...
describe("getLinkedByRecords", () => {
  it("requests the linked-by endpoint with the numeric target record id", async () => {
    fetchMock.mockResponseOnce(
      JSON.stringify({ data: [], error: null, success: true }),
    );
...
describe("getLinkedDocuments", () => {
  ...
  it("returns readable linked documents as global-id rows", async () => {
    fetchMock.mockResponseOnce(
      JSON.stringify({
        data: [
          { id: 7, oid: { idString: "SD7" }, name: "Linked doc", ownerFullName: "Ada Lovelace" },
        ],
        error: null,
        success: true,
      }),
    ...
    expect(result.readable).toEqual([
      { globalId: "SD7", name: "Linked doc", ownerFullName: "Ada Lovelace" },
    ]);
    expect(result.privateByOwner).toEqual([]);
  });
});
