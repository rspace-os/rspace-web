...
export default function LoginStatus({ fileSystems, fileStoreCheck }: LoginStatusArgs): React.ReactNode {
    ...
    return (
        <Card sx={{ p: 1 }}>
            ...
            {loggedOutCount > 0 && (
                <p>
                    You are not logged into all File Systems referenced by filestore links. Please login to remaining
                    File Systems or some linked files may be omitted during the export.
                </p>
            )}
            {loggedOutCount > 0 ? (
                <Button
