...
describe("DMPDialog", () => {
  ...
  test("Should render mock data correctly.", async () => {
    ...
    expect(
      await findTableCell(screen.getByRole("grid"), {
  ...
  describe("Pagination should work.", () => {
    test(
      ...
      async () => {
        ...
        const plansRequests = mockAxios.history.get.filter(({ url }) => /\/apps\/argos\/plans/.test(url ?? ""));
        expect(plansRequests.length).toBe(3);
        expect(
          plansRequests.map(({ url }) =>
            new URLSearchParams(url?.match(/\/apps\/argos\/plans?(.*)$/)?.[1]).get("page"),
          ),
        ).toEqual(["0", "1", "0"]);
      },
      20 * 1000,
    ...
    test(
      ...
      async () => {
        ...
        await user.click(within(screen.getByRole("listbox")).getByRole("option", { name: "5" }));
        const plansRequests = mockAxios.history.get.filter(({ url }) => /\/apps\/argos\/plans/.test(url ?? ""));
        expect(plansRequests.length).toBe(2);
        expect(
          plansRequests.map(({ url }) =>
            new URLSearchParams(url?.match(/\/apps\/argos\/plans?(.*)$/)?.[1]).get("pageSize"),
          ),
        ).toEqual(["10", "5"]);
      },
      10 * 1000,
  ...
  describe("Search filters should work.", () => {
    test(
      ...
      async () => {
        ...
        expect(
          plansRequests.map(({ url }) =>
            new URLSearchParams(url?.match(/\/apps\/argos\/plans?(.*)$/)?.[1]).get("like"),
          ),
        ).toEqual([null, "Foo"]);
      },
      10 * 1000,
