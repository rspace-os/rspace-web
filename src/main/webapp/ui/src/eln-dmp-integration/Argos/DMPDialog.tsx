...
const CustomChip = withStyles<ChipArgs, { label: string; root: string }>(() => ({
    ...
    return (
        <Grid item>
            <Chip size="small" label={`${name}: ${value}`} onDelete={onDelete} classes={classes} />
        </Grid>
    );
});
...
const SearchControls = ({
    ...
    }, [getDMPs, searchParameters]);

    return (
        <Grid container direction="column" spacing={1}>
            <Grid item container direction="row" spacing={1} role="group" aria-label="Search filters">
                <CustomChip
                    name="Label"
...
const CustomDialog = withStyles<{ fullScreen: boolean } & React.ComponentProps<typeof Dialog>, { paper?: string }>(
    (theme, { fullScreen }) => ({
        paper: {
            overflow: "hidden",
                ...
                >
                    <Grid item>
                        <Typography variant="body2">
                            Importing a DMP from <strong>argos.openaire.eu</strong> will make it available to view and
                            reference within RSpace.
                        </Typography>
                        <Typography variant="body2">
