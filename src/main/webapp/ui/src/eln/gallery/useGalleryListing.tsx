...
function generateIconSrc(
    ...
    return idToString(id)
        .flatMap((idStr) => {
            if (type === "Image")
                return Result.Ok(`/gallery/getThumbnail/${idStr}/${Math.floor(modificationDate.getTime() / 1000)}`);
...
export class LocalGalleryFile implements GalleryFile {
    ...
    get canMoveToIrods(): Result<null> {
        if (this.isSystemFolder) return Result.Error([new Error("Cannot move system folders to iRODS.")]);
        return Result.Ok(null);
    }
    ...
    get canUploadNewVersion(): Result<null> {
        ...
        if (!this.extension)
            return Result.Error([
                new Error("An extension is required to be able to update the file with a new version."),
            ]);
        return Result.Ok(null);
    }
...
export class RemoteFile implements GalleryFile {
    ...
    get canDuplicate(): Result<null> {
        return Result.Error([
            new Error(`Cannot duplicate ${this.isFolder ? "folders" : "files"} stored in filestores.`),
        ]);
    }
...
export function useGalleryListing({
    ...
    React.useEffect(() => {
        if (listingOf.tag === "folder") {
            void (async () => {
                try {
                    const response = await (await api).get<unknown>(
        ...
        try {
            ...
            const { data } = await axios.get<unknown>(`/gallery/getUploadedFiles`, {
                params: new URLSearchParams({
                    ...
                    sortOrder,
                    orderBy,
                    foldersOnly: foldersOnly !== null && Boolean(foldersOnly) ? "true" : "false",
                }),
            });
            ...
            setTotalPages(
                Parsers.objectPath(["data", "items", "totalPages"], data).flatMap(Parsers.isNumber).orElse(1),
            );
                            ...
                            axios
                                .get<unknown>(`/gallery/getUploadedFiles`, {
                                    params: new URLSearchParams({
                                        mediatype: s,
                                        currentFolderId:
                                            pa.length > 0 ? idToString(pa[pa.length - 1].id).elseThrow() : "0",
                                        name: searchTerm,
                                        pageNumber: `${p}`,
                                        sortOrder,
                                        orderBy,
