...
export function TreeViewWithPreviewProviders(): React.ReactNode {
  const listing = {
    tag: "list" as const,
    list: [mockFolder, mockImageFile, mockPdfFile],
    totalHits: 3,
  ...
  const [folderOpenCallCount, setFolderOpenCallCount] = React.useState(0);
  const [openedFolderName, setOpenedFolderName] = React.useState<string>("");
                      ...
                      <LandmarksProvider>
                        <CallableImagePreview>
                          <CallablePdfPreview>
                            <OpenFolderProvider
                              setPath={(path) => {
                                setFolderOpenCallCount((prev) => prev + 1);
                                const lastFolder = path[path.length - 1];
                                if (lastFolder) {
                                  setOpenedFolderName(lastFolder.name);
                                }
                              }}
                            >
                              <div style={{ width: 300, height: 400 }}>
                                <TreeView
                                  ...
                                  orderBy="name"
                                />
                                <div data-testid="folder-open-count">
                                  {folderOpenCallCount}
                                </div>
                                <div data-testid="opened-folder-name">
                                  {openedFolderName}
                                </div>
                              </div>
                            </OpenFolderProvider>
...
export function TreeViewWithEdgeCases(): React.ReactNode {
  const listing = {
    tag: "list" as const,
    list: [mockVeryLongNameFile, mockSpecialCharsFile, mockDocumentFile],
    totalHits: 3,
    loadMore: Optional.empty<() => Promise<void>>(),
    refreshing: false,
  };
...
export function TreeViewWithFiltering(): React.ReactNode {
  const listing = {
    tag: "list" as const,
    list: [mockFolder, mockImageFile, mockPdfFile, mockDocumentFile],
    totalHits: 4,
    loadMore: Optional.empty<() => Promise<void>>(),
    refreshing: false,
  };
                                ...
                                <TreeView
                                  ...
                                  filter={(file) => {
                                    if (file.name.includes("report"))
                                      return "disabled";
                                    if (file.name.includes("Test Folder"))
                                      return "hide";
                                    return "enabled";
                                  }}
                                />
