...
beforeEach(() => {
  ...
  mockAxios
    .onPost("/api/v1/gallery/filestores/1/move")
    .reply(200, OPERATION_SUCCESS_RESPONSE);
  mockAxios
    .onPost("/api/v1/gallery/filestores/1/copy")
    .reply(200, OPERATION_SUCCESS_RESPONSE);
  // Default transfer route: source filestore id=2 (as used in transfer stories)
  mockAxios
    .onPost("/api/v1/gallery/filestores/2/transfer")
    .reply(200, OPERATION_SUCCESS_RESPONSE);
...
async function selectMyS3Bucket(user: ReturnType<typeof userEvent.setup>) {
  ...
  await user.click(
    await screen.findByRole("menuitem", { name: /my s3 bucket/i }),
  );
}
...
describe("MoveToS3", () => {
  describe("Accessibility", () => {
    test("Should have no axe violations", async () => {
      ...
      expect(
        await screen.findByRole("heading", { name: /move to s3/i }),
      ).toBeVisible();
  ...
  describe("Empty state", () => {
    test("When no filestores are configured shows a 'no S3 filestore' message", async () => {
      ...
      expect(
        await screen.findByText(/no s3 filestore has been configured/i),
      ).toBeVisible();
    });

    test("When only non-S3 filestores are configured shows a 'no S3 filestore' message", async () => {
      mockAxios
        .onGet("/api/v1/gallery/filestores")
        .reply(200, [IRODS_FILESTORE]);
      render(<MoveToS3DialogWithOneFile />);
      expect(
        await screen.findByText(/no s3 filestore has been configured/i),
      ).toBeVisible();
  ...
  describe("Submit button state", () => {
    test("Submit button is disabled until a filestore is selected", async () => {
      ...
      expect(
        await screen.findByText(/a destination filestore is required/i),
      ).toBeVisible();
      ...
      await waitFor(() => {
        expect(
          screen.queryByText(/a destination filestore is required/i),
    ...
    test("Submit button label switches from 'Move' to 'Copy' when retain-copy is checked", async () => {
      ...
      await user.click(
        screen.getByRole("checkbox", { name: /retain a copy in rspace/i }),
      );
      expect(
  ...
  describe("API calls", () => {
    ...
    test("The correct record IDs are sent to the move endpoint", async () => {
      ...
      expect(await screen.findByText(/successfully moved/i)).toBeVisible();
      const moveRequest = mockAxios.history.post.find(
        ({ url }) => url === "/filestores/1/move",
      );
  ...
  describe("Transfer mode", () => {
    ...
    test("Transfer sends correct body with deleteSource=true by default", async () => {
      ...
      const transferRequest = mockAxios.history.post.find(
        ({ url }) => url === "/filestores/2/transfer",
      );
    ...
    test("All source paths are sent to the transfer endpoint for multiple files", async () => {
      ...
      expect(
        (await screen.findAllByText(/successfully transferred/i))[0],
      ).toBeVisible();

      const transferRequests = mockAxios.history.post.filter(
        ({ url }) => url === "/filestores/2/transfer",
      );
      expect(transferRequests).toHaveLength(2);
      const capturedBodies = transferRequests.map(
