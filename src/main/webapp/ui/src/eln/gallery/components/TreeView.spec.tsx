...
const feature = test.extend<{
  ...
  When: {
    "the user double-clicks on a {fileType}": ({ fileType }: { fileType: string }) => Promise<void>;
    ...
    "the user holds Ctrl and clicks on {fileName}": ({ fileName }: { fileName: string }) => Promise<void>;
  ...
  Then: {
    ...
    "the {fileType} should be selected": ({ fileType }: { fileType: string }) => Promise<void>;
    ...
    "the {fileType} should have focus": ({ fileType }: { fileType: string }) => Promise<void>;
  ...
  When: async ({ page }, use) => {
    await use({
      ...
      "the user holds Ctrl and clicks on {fileName}": async ({ fileName }) => {
        await page.getByRole("treeitem", { name: new RegExp(fileName) }).click({ modifiers: ["ControlOrMeta"] });
      },
  ...
  Then: async ({ page }, use) => {
    await use({
      ...
      "the {fileType} should be selected": async ({ fileType }) => {
        const fileName = getFileNameByType(fileType);
        await expect(page.getByRole("treeitem", { name: new RegExp(fileName) })).toHaveAttribute(
          "aria-checked",
          "true",
        );
      },
...
test.describe("TreeView", () => {
  ...
  test.describe("Double-click behavior", () => {
    feature("Should handle double-click on folder without crashing", async ({ Given, When }) => {
      await Given["the tree view with files is mounted"]();
    ...
    feature("Should handle double-click on image without crashing", async ({ Given, When }) => {
      await Given["the tree view with files is mounted"]();
      await When["the user double-clicks on a {fileType}"]({
        fileType: "image",
      });
    ...
    feature("Should handle double-click on PDF without crashing", async ({ Given, When }) => {
      await Given["the tree view with files is mounted"]();
      await When["the user double-clicks on a {fileType}"]({
        fileType: "pdf",
      });
  ...
  test.describe("Selection behavior", () => {
    feature("Should select file on single click", async ({ Given, When, Then }) => {
      await Given["the tree view with files is mounted"]();
  ...
  test.describe("Multi-Selection", () => {
    feature("Should support Ctrl+click for multi-selection", async ({ Given, When, Then }) => {
      await Given["the tree view with files is mounted"]();
      await When["the user single-clicks on a {fileType}"]({
        fileType: "image",
      });
  ...
  test.describe("Filtering", () => {
    feature("Should show only folders when foldersOnly is true", async ({ Given, Then }) => {
