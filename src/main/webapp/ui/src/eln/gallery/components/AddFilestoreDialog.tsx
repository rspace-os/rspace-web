    ...
    return (
        <Step key="filesystemSelection" {...rest}>
            ...
            <StepContent>
                ...
                >
                    {(filesystems ?? []).map((fs) => (
                        <FormControlLabel key={fs.id} value={fs.id} control={<Radio />} label={fs.name} />
                    ))}
                </RadioGroup>
                <Box sx={{ mb: 2 }}>
                    <Button
...
function TreeListing({
    ...
    React.useEffect(() => {
        async function browse(): Promise<void> {
            try {
                const { data } = await (await api.current).get<{
                    content: FilesystemListing;
        ...
        void browse();
    }, [fsId, path, addAlert, fsName, login, onFailToAuthenticate]);
    ...
    return (
        <Step key="folderSelection" {...rest}>
            <StepLabel
                optional={
                    <Typography variant="body2">
                        You can configure multiple Filestores from the same File system with different top-level
                        folders, to facilitate accessing deeply-nested content.
                    </Typography>
                }
            >
