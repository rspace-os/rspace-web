...
const Preview = ({ file, zoom, visible }: { file: GalleryFile; zoom: number; visible: boolean }) => {
    ...
    }, [canPreviewAsImage, canPreviewAsPdf, canPreviewWithAspose, file]);
...
export default function Carousel({ listing }: CarouselArgs): React.ReactNode {
    ...
    React.useEffect(() => {
        ...
        listing,
        selection.append, // when enter Carousel view, focus on already selected file
        selection.asSet,
        selection.isEmpty,
    ]);
    ...
    }, [decrementVisibleIndex, incrementVisibleIndex]);
