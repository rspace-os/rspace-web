...
import { alpha, lighten, type SxProps, styled, useTheme } from "@mui/material/styles";
...
const DragOverlayContents = styled(
    observer(({ className }: { className?: string }) => {
        ...
        return <Badge badgeContent={numberBeingMoved} color="callToAction" className={className}></Badge>;
    }),
)(({ theme }) => ({
    width: "100%",
    height: "100%",
...
>(({ folder, section, setSelectedSection, tabIndex, sx }, ref) => {
    ...
    return (
        <StyledBreadcrumb
            ...
            style={{
                ...(dndInProgress && !isOver ? { animation: "drop 2s linear infinite" } : {}),
                ...(isOver ? { border: SELECTED_OR_FOCUS_BORDER } : {}),
            }}
            tabIndex={tabIndex}
            label={folder?.name ?? getByKey(section, gallerySectionLabel).orElse("UNKNOWN")}
            {...(folder
                ? {}
...
const Path = observer(
    ({
        ...
        return (
            <Stack direction="row">
                <StyledBreadcrumbs onFocus={onFocus} onBlur={onBlur} onKeyDown={onKeyDown} aria-label="Breadcrumbs">
                    <BreadcrumbLink
                        section={section}
                ...
                <IconButtonWithTooltip
                    ...
                    sx={{
                        backgroundColor: (theme) =>
                            alpha(lighten((theme.palette.primary as { main: string }).main, 0.1), 0.6),
                        "&:hover": {
                            backgroundColor: (theme) =>
                                alpha(lighten((theme.palette.primary as { main: string }).main, 0.1), 0.85),
                        },
                        padding: "2px",
        ...
        React.forwardRef(
            (
                ...
                const dropStyle = {
                    ...
                    !isBeingMoved(
                        file,
                        dndContext.active?.data.current?.fileBeingMoved as GalleryFile | null | typeof undefined,
                    )
                        ? {
                              border: "2px solid white",
                              borderWidth: "2px",
                        ...
                        >
                            ...
                            >
                                            ...
                                            <Typography
                                                sx={{
                                                    ...(selected
                                                        ...
                                                        : {}),
                                                    fontSize: "0.8125rem",
                                                    fontWeight: window.matchMedia("(prefers-contrast: more)").matches
                                                        ? 700
                                                        : 400,
                                ...
                                {typeof file.version === "number" && file.version > 1 && (
                                    <div className="versionIndicator" aria-label={`version ${file.version}`}>
                                        v{file.version}
                                    </div>
                                )}
                            </CardActionArea>
                ...
                <div
                    ...
                    onKeyDown={(e) => {
                        ...
                        listing.list.forEach((file, i) => {
                            ...
                            if (fileX >= left && fileX <= right && fileY >= top && fileY <= bottom)
        ...
        return (
            ...
            >
                <Grid item sx={{ flexGrow: 1, minWidth: 0 }}>
                    {path !== null && (
                        <Path section={selectedSection} path={path} setSelectedSection={setSelectedSection} />
                    )}
                </Grid>
...
function GalleryMainPanel({
    ...
    return (
        ...
        >
            ...
            >
                <Grid container direction="column" sx={{ height: "100%", flexWrap: "nowrap" }} spacing={1}>
                    <Grid item>
                        <PathAndSearch
                    ...
                    >
                        ...
                        >
                            <Grid item sx={{ overflowY: "auto", mt: 1, userSelect: "none" }} flexGrow={1}>
                                {viewMode === "tree" &&
