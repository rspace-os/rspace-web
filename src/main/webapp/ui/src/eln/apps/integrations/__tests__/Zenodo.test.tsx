...
describe("Zenodo", () => {
    ...
    test("should enable integration when dialog is open and integration is disabled", async () => {
        ...
        when(update).calledWith(expectedState).mockReturnValue(expectedState);
    ...
    test("should open, set API key, enable, and close dialog in sequence", async () => {
        ...
        when(update).calledWith(apiKeyState).mockReturnValue(apiKeyState);

        when(update).calledWith(enabledState).mockReturnValue(enabledState);
