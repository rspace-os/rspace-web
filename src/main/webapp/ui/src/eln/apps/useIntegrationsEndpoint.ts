 ...
 */
function parseState({ enabled, available }: FetchedState): IntegrationState<unknown>["mode"] {
 ...
 */
function parseCredentialString<K extends string>(options: Record<K, unknown>, key: K): Optional<string> {
 ...
 * in Optional.present, otherwise Optional.empty is returned.
 */
function parseCredentialBoolean<K extends string>(options: Record<K, unknown>, key: K): Optional<boolean> {
  return getByKey(key, options).flatMap((cred) =>
...
function decodeClustermarket(data: FetchedState): IntegrationStates["CLUSTERMARKET"] {
  return {
    mode: parseState(data),
    credentials: {
...
function decodeDigitalCommonsData(data: FetchedState): IntegrationStates["DIGITALCOMMONSDATA"] {
  return {
    mode: parseState(data),
    credentials: {
...
function decodeDmpAssistant(data: FetchedState): IntegrationStates["DMPASSISTANT"] {
  return {
    mode: parseState(data),
    credentials: {
      ACCESS_TOKEN: parseCredentialString(data.options, "ACCESS_TOKEN"),
      ...
      EGNYTE_DOMAIN:
        Object.values(data.options).length === 1
          ? ArrayUtils.head(Object.values(data.options))
              ...
              .flatMap(Parsers.isRecord)
              .toOptional()
              .flatMap((option) => parseCredentialString(option, "EGNYTE_DOMAIN"))
          : Optional.empty(),
    },
  };
}
...
function decodeGalaxy(data: FetchedState): IntegrationStates["GALAXY"] {
  return {
    mode: parseState(data),
    credentials: {
      configuredServers: Parsers.objectPath(["options", "GALAXY_CONFIGURED_SERVERS"], data)
        .flatMap(Parsers.isObject)
        .flatMap(Parsers.isNotNull)
          ...
          Result.all(
            ...Object.values(configuredServers).map((config: unknown) => {
              try {
                ...
                const alias = Parsers.getValueWithKey("alias")(server).flatMap(Parsers.isString).elseThrow();
                const url = Parsers.getValueWithKey("url")(server).flatMap(Parsers.isString).elseThrow();
      ...
      authenticatedServers: Parsers.objectPath(["options"], data)
        .flatMap(Parsers.isObject)
        .flatMap(Parsers.isNotNull)
        .map((servers) => Object.entries(servers).filter(([k]) => k !== "GALAXY_CONFIGURED_SERVERS"))
        .flatMap((servers) =>
          Result.all(
...
function decodeGoogleDrive(data: FetchedState): IntegrationStates["GOOGLEDRIVE"] {
  return {
    mode: parseState(data),
    credentials: {
...
function decodeProtocolsIo(data: FetchedState): IntegrationStates["PROTOCOLS_IO"] {
  return {
    mode: parseState(data),
    credentials: {
      ACCESS_TOKEN: parseCredentialString(data.options, "ACCESS_TOKEN"),
...
function decodePyrat(data: FetchedState): IntegrationStates["PYRAT"] {
  return {
    mode: parseState(data),
    credentials: {
      configuredServers: Parsers.objectPath(["options", "PYRAT_CONFIGURED_SERVERS"], data)
        .flatMap(Parsers.isObject)
        .flatMap(Parsers.isNotNull)
        .flatMap((configuredServers) =>
          Result.all(
...
function decodeRaid(data: FetchedState): IntegrationStates["RAID"] {
  return {
    mode: parseState(data),
    credentials: {
      configuredServers: Parsers.objectPath(["options", "RAID_CONFIGURED_SERVERS"], data)
        .flatMap(Parsers.isObject)
        .flatMap(Parsers.isNotNull)
        .flatMap((configuredServers) =>
          Result.all(
      ...
      authenticatedServers: Parsers.objectPath(["options"], data)
        .flatMap(Parsers.isObject)
        .flatMap(Parsers.isNotNull)
        .map((servers) => Object.entries(servers).filter(([k]) => k !== "RAID_CONFIGURED_SERVERS"))
        .flatMap((servers) =>
          Result.all(
            ...servers.map(([key, config]: [string, unknown]) => {
              try {
                const server = Parsers.isObject(config).flatMap(Parsers.isNotNull).elseThrow();
                const alias = Parsers.getValueWithKey("RAID_ALIAS")(server).flatMap(Parsers.isString).elseThrow();
                const url = Parsers.getValueWithKey("RAID_URL")(server).flatMap(Parsers.isString).elseThrow();
                const authenticated =
                  Parsers.getValueWithKey("RAID_OAUTH_CONNECTED")(server).flatMap(Parsers.isString).elseThrow() ===
                  "true";
                const optionsId = Parsers.isString(key).elseThrow();
  ...
  return {
    ...
    credentials: Object.entries(data.options).map(([optionsId, config]) => {
      return Parsers.isObject(config)
        ...
        .flatMap((configRecord) => {
          try {
            const SLACK_TEAM_NAME = Parsers.isString(configRecord.SLACK_TEAM_NAME).elseThrow();
            const SLACK_CHANNEL_ID = Parsers.isString(configRecord.SLACK_CHANNEL_ID).elseThrow();
...
function decodeIntegrationStates(
  data: {
...
const encodeIntegrationState = <I extends Integration>(integration: I, data: IntegrationStates[I]): FetchedState => {
  ...
  if (integration === "DATAVERSE") {
    // @ts-expect-error Looks like this is a bug in TypeScript
    const creds: IntegrationStates["DATAVERSE"]["credentials"] = data.credentials;
    return {
      name: "DATAVERSE",
      available: data.mode !== "UNAVAILABLE",
      enabled: data.mode === "ENABLED",
  ...
  if (integration === "DIGITALCOMMONSDATA") {
    // @ts-expect-error Looks like this is a bug in TypeScript
    const creds: IntegrationStates["DIGITALCOMMONSDATA"]["credentials"] = data.credentials;
    return {
  ...
  if (integration === "DMPASSISTANT") {
    // @ts-expect-error Looks like this is a bug in TypeScript?
    const creds: IntegrationStates["DMPASSISTANT"]["credentials"] = data.credentials;
    return {
      name: "DMPASSISTANT",
      available: data.mode !== "UNAVAILABLE",
      enabled: data.mode === "ENABLED",
  ...
  if (integration === "DMPONLINE") {
    // @ts-expect-error Looks like this is a bug in TypeScript?
    const creds: IntegrationStates["DMPONLINE"]["credentials"] = data.credentials;
    return {
      name: "DMPONLINE",
      available: data.mode !== "UNAVAILABLE",
      enabled: data.mode === "ENABLED",
  ...
  if (integration === "DSW") {
    // @ts-expect-error Looks like this is a bug in TypeScript
    const creds: IntegrationStates["DSW"]["credentials"] = data.credentials;
    return {
      name: "DSW",
      available: data.mode !== "UNAVAILABLE",
      enabled: data.mode === "ENABLED",
  ...
  if (integration === "FIELDMARK") {
    // @ts-expect-error Looks like this is a bug in TypeScript?
    const creds: IntegrationStates["FIELDMARK"]["credentials"] = data.credentials;
    return {
      name: "FIELDMARK",
      available: data.mode !== "UNAVAILABLE",
      enabled: data.mode === "ENABLED",
  ...
  if (integration === "FIGSHARE") {
    // @ts-expect-error Looks like this is a bug in TypeScript?
    const creds: IntegrationStates["FIGSHARE"]["credentials"] = data.credentials;
    return {
      name: "FIGSHARE",
      available: data.mode !== "UNAVAILABLE",
      enabled: data.mode === "ENABLED",
  ...
  if (integration === "GOOGLEDRIVE") {
    // @ts-expect-error Looks like this is a bug in TypeScript?
    const creds: IntegrationStates["GOOGLEDRIVE"]["credentials"] = data.credentials;
    return {
      name: "GOOGLEDRIVE",
      available: data.mode !== "UNAVAILABLE",
      enabled: data.mode === "ENABLED",
  ...
  if (integration === "NEXTCLOUD") {
    // @ts-expect-error Looks like this is a bug in TypeScript?
    const creds: IntegrationStates["NEXTCLOUD"]["credentials"] = data.credentials;
    return {
      name: "NEXTCLOUD",
      available: data.mode !== "UNAVAILABLE",
      enabled: data.mode === "ENABLED",
  ...
  if (integration === "OWNCLOUD") {
    // @ts-expect-error Looks like this is a bug in TypeScript?
    const creds: IntegrationStates["OWNCLOUD"]["credentials"] = data.credentials;
    return {
      name: "OWNCLOUD",
      available: data.mode !== "UNAVAILABLE",
      enabled: data.mode === "ENABLED",
...
export function useIntegrationsEndpoint(): {
   ...
   */
  update: <I extends Integration>(integration: I, newState: IntegrationStates[I]) => Promise<IntegrationStates[I]>;
   ...
   */
  deleteAppOptions: <I extends Integration>(appName: I, optionsId: OptionsId) => Promise<void>;
} {
  ...
  const update = React.useCallback(
    async <I extends Integration>(integration: I, newState: IntegrationStates[I]): Promise<IntegrationStates[I]> => {
      try {
        ...
        if (!("success" in responseData)) throw new Error(responseData.exceptionMessage);
        if (!responseData.success) {
          if (!responseData.success && responseData.errorMsg !== null && typeof responseData.errorMsg !== "undefined") {
          ...
          switch (integration) {
            ...
            case "DMPASSISTANT":
              return decodeDmpAssistant(responseData.data) as IntegrationStates[I];
            case "DMPONLINE":
            ...
            case "GOOGLEDRIVE":
              return decodeGoogleDrive(responseData.data) as IntegrationStates[I];
            case "MSTEAMS":
            ...
            case "PROTOCOLS_IO":
              return decodeProtocolsIo(responseData.data) as IntegrationStates[I];
            case "PYRAT":
  ...
  const deleteAppOptions = async <I extends Integration>(appName: I, optionsId: OptionsId): Promise<void> => {
