    ...
    Then: async ({ page }, use) => {
        await use({
            ...
            "the usage should be shown in human-readable format": async () => {
                ...
                await expect(grid.getByRole("row").nth(2).getByRole("gridcell").nth(usageIndex)).toHaveText(
                    "362.01 kB",
                );
...
test.describe("Table Listing", () => {
    feature("Usage should be in a human-readable format", async ({ Given, When, Then }) => {
