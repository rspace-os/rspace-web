...
}>()((theme, { index, keyboardFocusIndex }: { index: number; keyboardFocusIndex: number | null }) => ({
    menuItem: {
        padding: "8px",
        cursor: "default",
...
export default function TagsCombobox({ onSelection, value, anchorEl, onClose }: TagsComboboxArgs): React.ReactNode {
    ...
    useEffect(() => {
        if (anchorEl) {
             ...
             */
            setTimeout(() => {
                (getInputProps().ref as React.RefObject<HTMLInputElement>).current?.focus();
            }, 0);
        }
    }, [anchorEl, getInputProps]);
