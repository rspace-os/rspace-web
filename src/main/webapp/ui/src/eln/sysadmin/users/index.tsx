...
const CustomGrow = React.forwardRef<typeof Grow, React.ComponentProps<typeof Grow>>(
                    ...
                    <Grid container direction="column" spacing={2}>
                        <Grid item>
                            <Typography variant="body2">
                                You can tag users to categorise them, and filter users by tag. These tags are only
                                visible to System Admins and Community Admins. If you&apos;ve selected several users,
                                only shared tags will be shown.{" "}
...
const PiAction = ({
    ...
    const allowedPiAction: Result<{ user: User; action: "revoke" | "grant" }> = selectedUser
                ...
                {allowedPiAction
                    .map(({ action, user }) => (
                        ...
                        >
                            ...
                            >
                                <DialogTitle>
                                    {action === "grant" ? "Grant user PI role" : "Revoke PI role from user"}
                                </DialogTitle>
                                <DialogContent>
                                    {FetchingData.match(verificationPasswordNeeded, {
                                        ...
                                        success: (veriPwdNeeded) =>
                                            veriPwdNeeded ? (
                                                <DialogContentText variant="body2" sx={{ mb: 2 }}>
                                                    Please set your verification password in My RSpace before performing
                                                    this action.
                                                </DialogContentText>
                                            ) : (
                                                <>
                    ...
                    >
                        ...
                        >
                            ...
                            <DialogContent>
                                <DialogContentText variant="body2" sx={{ mb: 2 }}>
                                    <Typography variant="body2">
                                        SysAdmins can set a username alias for a user, enabling both the username and
                                        its alias to be used during login. The alias is only recognised during the login
                                        process, and does not replace the user’s username as their main identifier
                                        inside RSpace.
                                    </Typography>
                                </DialogContentText>
                                <TextField
                        ...
                        >
                                ...
                                <DialogContentText variant="body2" sx={{ mb: 2 }}>
                                    ...
                                    {user.hasFormsUsedByOtherUsers && (
                                        <Typography variant="body2" sx={{ mb: 1 }}>
                                            ...
                                            <strong> this System Administrator</strong> account. Forms that are not used
                                            by others will be deleted.
                                        </Typography>
                                    )}
                                    <Typography variant="body2" sx={{ mb: 1 }}>
                                        An XML archive will be made of the user&apos;s work which will be available for
                                        a short time on the server.
                                    </Typography>
                                    <Typography variant="body2" sx={{ mb: 1 }}>
                            ...
                            <DialogActions>
                                ...
                                <SubmitSpinnerButton
                                    ...
                                    loading={false}
                                    disabled={false}
                                    label={user.hasFormsUsedByOtherUsers ? "Transfer Forms And Delete" : "Delete"}
                                />
                            </DialogActions>
                        </form>
                    </Dialog>
...
const SelectionActions = ({
    ...
    const enableDisableAction: Result<{
        ...
        .map((user) => (user.enabled ? { user, action: "disable" } : { user, action: "enable" }));
    ...
    return (
        <>
            {FetchingData.match(fetchedListing, {
                ...
                error: () => <></>,
                success: (listing) => (
                    <Grid container direction="row" sx={{ width: 598, alignItems: "center", mb: 0.5 }} spacing={1}>
                        <Grid item>
                            <Button
                                variant="outlined"
                                color="primary"
    ...
    return (
        <GridToolbarContainer sx={{ width: "100%" }}>
                    ...
                    <CardContent sx={{ pt: 1 }}>
                        <Grid container direction="column" spacing={2}>
                            <Grid item container direction="column" spacing={1}>
                                ...
                                <Grid item container direction="row" spacing={1} sx={{ flexWrap: "nowrap" }}>
            ...
            <GridToolbarExportContainer /*variant="outlined"*/>
                <ExportMenuItem onClick={() => exportAllRows()}>Export all rows to CSV</ExportMenuItem>
                <ExportMenuItem
                    onClick={() => {
                        exportVisibleRows();
                ...
                >
                    Export {rowSelectionModel.length > 0 ? "selected" : "this page of"} rows to CSV
                </ExportMenuItem>
            </GridToolbarExportContainer>
        </GridToolbarContainer>
    );
...
export const UsersPage = (): React.ReactNode => {
    ...
    const [columnVisibility, setColumnVisibility] = useUiPreference(PREFERENCES.SYSADMIN_USERS_TABLE_COLUMNS, {
        defaultValue: {
            email: false,
            recordCount: false,
            created: false,
    ...
    const columns = [
        DataGridColumn.newColumnWithValueMapper("fullName", (fullName) => fullName, {
                                ...
                                <Grid container direction="column" spacing={1.25}>
                                    <Grid item>
                                        <Typography variant="body2" sx={{ maxWidth: 575 }}>
                                            You can search, filter, and tag user accounts, as well as export summary
                                            information about the users on this server. See our{" "}
                                            <Link target="_blank" rel="noreferrer" href={docLinks.taggingUsers}>
                                                Tagging docs
                                            </Link>{" "}
                                            for more.
                                                    ...
                                                    FetchingData.match(userListing, {
                                                        ...
                                                        success: (listing) => {
                                                            ...
                                                            const [{ field: newOrderBy, sort: newSortOrder }] =
                                                                newSortModel;
                                                            setSortModel([{ field: newOrderBy, sort: newSortOrder }]);
                                                            const apiOrderBy = {
                                                                username: "username",
                                                                fileUsage: "fileUsage()",
                                                            ...
                                                            if (!apiOrderBy)
                                                                throw new Error(`Invalid order by: ${newOrderBy}`);
