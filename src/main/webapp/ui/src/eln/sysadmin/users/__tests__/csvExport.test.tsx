...
describe("CSV Export", () => {
    describe("Selection", () => {
        test("When no rows are selected, every row of the current page should be included in the export.", async () => {
            ...
            if (typeof blob === "undefined") throw new Error("Impossible, because createObjectURL has been called");
    ...
    describe("Column", () => {
        test("All of the columns should be included in the CSV file.", async () => {
            ...
            fireEvent.click(await screen.findByRole("button", { name: /Select columns/ }));
            ...
            expect(createObjectURL).toHaveBeenCalled();
            if (typeof blob === "undefined") throw new Error("Impossible, because createObjectURL has been called");
            expect((await blob.text()).split("\n")[0].split(",").length).toBe(numberOfColumns);
        });
