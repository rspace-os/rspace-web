...
function ExternalWorkflowInvocations({ isForNotebookPage = false, fieldId }: ExternalWorkflowInvocationsArgs) {
    ...
    }, [shouldCheckForGalaxyData, checkForRSpaceGalaxyData]);

    useEffect(() => {
        if (galaxyDataFoundInRSpace) {
            fetchDataFromGalaxy();
        }
    }, [galaxyDataFoundInRSpace, fetchDataFromGalaxy]);
    ...
    const getGalaxyData = async (): Promise<Array<GalaxyDataSummary>> => {
        return (await axios.get(`/apps/galaxy/getSummaryGalaxyDataForRSpaceField/${fieldId}`)).data;
    };
    ...
    const getGalaxyDataCount = async (): Promise<InvocationsAndDataCount> => {
        return (await axios.get(`/apps/galaxy/getGalaxyInvocationCountForRSpaceField/${fieldId}`)).data;
    };

    window.addEventListener("galaxy-used", (e) => {
        // @ts-expect-error
