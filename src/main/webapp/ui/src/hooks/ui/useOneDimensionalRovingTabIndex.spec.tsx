            ...
            "the roving list has focus": async () => {
                ...
                while (!elementHasFocus) {
                    ...
                    elementHasFocus =
                        (await page.evaluate(
                            () =>
                                (document.activeElement?.parentNode as HTMLElement | null)?.tagName.toLowerCase() ===
                                "li",
                        )) ?? false;
                }
            },
...
test.describe("useOneDimensionalRovingTabIndex", () => {
    ...
    feature("Pressing tab thrice focusses the after button", async ({ Given, When, Then }) => {
        await Given["the simple example component is rendered"]();
    ...
    feature("Tabbing back to the roving list returns focus to last focussed element", async ({ Given, When, Then }) => {
        await Given["the simple example component is rendered"]();
        await When["the roving list has focus"]();
        await When["the user presses the down arrow key"]();
