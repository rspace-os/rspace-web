...
export default function useStoichiometry(): {
     ...
     */
    calculateStoichiometry: ({ chemId }: { chemId: number }) => Promise<StoichiometryResponse>;
     ...
     */
    deleteStoichiometry: ({ stoichiometryId }: { stoichiometryId: number }) => Promise<void>;
    ...
    async function calculateStoichiometry({ chemId }: { chemId: number }): Promise<StoichiometryResponse> {
        try {
            const formData = new FormData();
            formData.append("chemId", chemId.toString());
            const { data } = await axios.post<StoichiometryResponse>("/api/v1/stoichiometry", formData, {
                headers: {
    ...
    async function getStoichiometry({
        ...
        try {
            const { data } = await axios.get<StoichiometryResponse>("/api/v1/stoichiometry", {
    ...
    async function updateStoichiometry({
        ...
        try {
            const { data } = await axios.put<StoichiometryResponse>("/api/v1/stoichiometry", stoichiometryData, {
    ...
    async function deleteStoichiometry({ stoichiometryId }: { stoichiometryId: number }): Promise<void> {
        try {
    ...
    async function getMoleculeInfo({ smiles }: { smiles: string }): Promise<MoleculeInfo> {
        try {
