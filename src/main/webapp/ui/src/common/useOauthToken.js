 ...
 * gets a token that cann thereafter be used to make requests to the API
...
export default function useOauthToken(): {| getToken: () => Promise<string> |} {
  ...
  return {
    getToken: async () => {
      ...
      /*
       * If this hook as not been previously called then we preferably get the
       ...
       * storage, we set up a time to trigger the continuous refreshing of the
