...
describe("action: moveSelected", () => {
    describe("Moving the contents of a location into its current location should", () => {
        test("result in new records being allocated in memory.", async () => {
            ...
            jest.spyOn(destination.contentSearch.fetcher, "performInitialSearch").mockImplementation(() =>
                Promise.resolve(),
            );
            ...
            jest.spyOn(searchStore.search.fetcher, "performInitialSearch").mockImplementation(() => Promise.resolve());
