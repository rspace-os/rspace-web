...
describe("action: setTargetContainer", () => {
    describe("When called, setTargetContainer should", () => {
        ...
        test("cause clearLocationsWithContentBeingMovedOut to be called.", async () => {
            ...
            const clearLocationsSpy = jest.spyOn(moveStore, "clearLocationsWithContentBeingMovedOut");
            jest.spyOn(Search.prototype, "setSearchView").mockImplementation(() => Promise.resolve());
            await moveStore.setIsMoving(true);
            jest.spyOn(container, "fetchAdditionalInfo").mockImplementation(() => Promise.resolve());
            jest.spyOn(container.contentSearch.fetcher, "performInitialSearch").mockImplementation(() =>
                Promise.resolve(),
            );
