...
export class ColumnFieldMap {
    ...
    get validFieldName(): boolean {
        return (
            ...
            (!invalidFieldNames.includes(this.fieldName.trim()) && this.isNameUnique(this))
        );
    }
...
export default class Import {
    ...
    get anyParentSamplesFieldIsSelected(): boolean {
        return this.subSamplesMappings.some(
            (m) =>
                (m.field === Fields.parent_sample_import_id || m.field === Fields.parent_sample_global_id) &&
                m.selected,
        );
    }
    ...
    validateMappings(mappings: Array<ColumnFieldMap>): boolean {
        ...
        return (
            allMappingsAreValid && allFieldNamesAreUnique && nameFieldIsSelected && !parentContainersImportIdUndefined
        );
    }
            ...
            if (!this.someFileSubmitted) {
                ...
                if (this.isSamplesImport) {
                    ...
                    runInAction(() => {
                        ...
                        this.samplesMappings = (data.templateInfo as TemplateAttrs).fields.map(
                            ({ name, type }): ColumnFieldMap => {
                                const originalColumnName = Object.keys(
                                    data.fieldNameForColumnName as Record<string, string>,
                                ).find((key) => this.fieldNameForColumnName[key] === name) as string;
                                ...
                                if (
                                    newFieldMap.isCompatibleWithField(fieldsByRecordType[nameToMatch(name as string)])
                                ) {
                                    newFieldMap.setField(fieldsByRecordType[nameToMatch(name as string)]);
                                }
                                if (
                                    data.fieldMappings !== undefined &&
                                    Object.hasOwn(data.fieldMappings, originalColumnName)
                                ) {
        ...
        const templateFieldWithMappings: Array<{
            ...
            ArrayUtils.filterNull(
                this.templateInfo.fields.map(
                    ({ name }) => this.samplesMappings.find((f) => f.fieldName === name) ?? null,
                ),
            ),
    ...
    makeMappingsObject(mappings: Array<ColumnFieldMap>): {
        ...
        const result = {
            ...
            ...Object.fromEntries(
                mappings.filter(({ selected }) => !selected).map(({ columnName }) => [columnName, null]),
            ),
        };
        return result;
    }
