...
import { containerAttrs, makeMockContainer } from "../ContainerModel/mocking";
...
describe("action: deleteRecords", () => {
    describe("When it is called,", () => {
        test("the search should be refreshed.", async () => {
                    ...
                    mockFactory({
                        newRecord: jest
                            ...
                            .mockImplementation((attrs: unknown) =>
                                makeMockContainer(attrs as Readonly<Partial<typeof containerAttrs>>),
                            ),
                    }),
            });
            ...
            const searchSpy = jest.spyOn(search.fetcher as any, "search").mockImplementation((...args: any[]) => {
        ...
        test("and there is an error, there should be a helpful error message.", async () => {
            ...
            jest.spyOn(search.fetcher as any, "search").mockImplementation((...args: any[]) => {
                // Call the callback with empty results if it exists
                if (args[1] && typeof args[1] === "function") {
                    args[1]([]);
                }
