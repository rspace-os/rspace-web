...
const everyOutputForArbitraryNumOfRecords = arbitraryInput
    .map((n) => n + 1)
    .map((numOfRecords) => new RsSet(take(incrementForever(), numOfRecords)).map((i) => formatIndex(i, numOfRecords)));
...
describe("formatIndex", () => {
    ...
    test("Parsed output should give index input plus one.", () => {
        fc.assert(
            fc.property(arbitraryInputs, ([index, numOfRecords]) => {
                expect(parseInt(formatIndex(index, numOfRecords), 10)).toEqual(index + 1);
            }),
        );
    });
