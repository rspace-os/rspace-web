...
export default class AlwaysNewFactory implements Factory {
  newRecord(params: any): Result {
    ...
    // prettier-ignore
    const record =
      patterns.sample.test(g)          ? new SampleModel   (this, params) :
      patterns.subsample.test(g)       ? new SubSampleModel(this, params) :
      patterns.container.test(g)       ? new ContainerModel(this, params) :
      patterns.sampleTemplate.test(g)  ? new TemplateModel (this, params) :
      patterns.bench.test(g)           ? new ContainerModel(this, params) :
      /* otherwise */                    null;
