...
export default class TemplateModel extends SampleModel implements Template {
    ...
    async updateSamplesToLatest(): Promise<void> {
        ...
        if (
            !(await getRootStore().uiStore.confirm(
                ...
                <>
                    All of your samples created from this template will be updated to pick up the structural changes
                    that have been made to the template since the sample was created or last updated, such as the
                    addition, deletion and reordering of fields, and the change to available options in choice and radio
                    fields.&nbsp;
    ...
    get permalinkURL(): URLType | null {
        ...
        if (this.historicalVersion) return `/inventory/${this.recordType.toLowerCase()}/${id}?version=${this.version}`;
    ...
    moveField(field: Field, newIndex: number): void {
        ...
        if (oldIndex === -1)
            throw new Error(`Could not find field with id '${field.id ?? "NEW FIELD"}' in this sample.`);
