        ...
        HasUneditableFields<InventoryBaseRecordUneditableFields>
{
    ...
    async expiryCheck() {
        ...
        if (editLockExpired) {
            ...
            await getRootStore().uiStore.confirm(
                ...
                <>
                    Another user may be editing this {this.recordTypeLabel.toLowerCase()}. Please copy any information
                    you need and then press <strong>Cancel</strong>.
                </>,
                "OK",
                "",
            );
                        ...
                        details: Result.any(
                            ...validationErrors.map((e) =>
                                Parsers.isObject(e)
                                    ...
                                    .flatMap((obj) =>
                                        Result.lift2<string, string, { title: string; help: string; variant: "error" }>(
                                            (title, help) => ({
                                                title,
                                                help,
                                                variant: "error",
    ...
    isFieldEditable(field: string): boolean {
        return !this.loading && !this.deleted && this.currentlyEditableFields.has(field);
    }
        ...
        try {
            if (
                await getRootStore().uiStore.confirm(
                    ...
                    <>
                        An IGSN ID in <strong>Draft</strong> state will be created. No metadata will be made public at
                        this stage.
                    </>,
                    "OK",
                    "CANCEL",
        ...
        try {
            if (
                await getRootStore().uiStore.confirm(
                    ...
                    <>
                        The IGSN ID will be deleted, and this item will no longer have an IGSN ID associated with it. Do
                        you want to proceed?
                    </>,
                    "OK",
                    "CANCEL",
