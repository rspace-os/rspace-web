...
const feature = test.extend<{
  ...
  Once: emptyObject;
  When: {
    "the user expands a folder node": (params: { folderName: string }) => Promise<void>;
    "the user clicks on a folder": (params: { folderName: string }) => Promise<void>;
    "the user clicks the Load More button": () => Promise<void>;
    "the user hovers over a folder": (params: { folderName: string }) => Promise<void>;
    "the user clicks the add folder button for a folder": (params: { folderName: string }) => Promise<void>;
    "the user clicks the Add Folder button": () => Promise<void>;
    "the user enters a folder name": (params: { folderName: string }) => Promise<void>;
  ...
  Then: {
    ...
    "the folder should be expanded showing its subfolders": (params: { folderName: string }) => Promise<void>;
    "the folder should be selected": (params: { folderName: string }) => Promise<void>;
    ...
    "the add folder button should be visible": (params: { folderName: string }) => Promise<void>;
    "the add folder button should not be visible": (params: { folderName: string }) => Promise<void>;
    "the create folder dialog should be open": () => Promise<void>;
    "the new folder should appear in the tree": (params: { folderName: string }) => Promise<void>;
    "the new folder should be selected": (params: { folderName: string }) => Promise<void>;
    "a validation error should be displayed": () => Promise<void>;
  };
}>({
  Given: async ({ mount, page }, use) => {
    await use({
      "the FolderTree component is rendered with mocked API responses": async () => {
        ...
        await page.route("**/api/v1/folders", async (route) => {
          if (route.request().method() === "POST") {
            const requestBody = (await route.request().postDataJSON()) as CreateFolderRequest;
  ...
  When: async ({ page }, use) => {
    await use({
      ...
      "the user clicks the add folder button for a folder": async ({ folderName }) => {
        const treeItem = page.getByRole("treeitem", {
          name: new RegExp(folderName),
        });
        await treeItem.hover();
  ...
  Then: async ({ page }, use) => {
    await use({
      "the folder tree should display the root folders": async () => {
        await expect(page.getByRole("treeitem", { name: /Research Projects/ })).toBeVisible();
        await expect(page.getByRole("treeitem", { name: /Lab Notebooks/ })).toBeVisible();
      },
      "the folder should be expanded showing its subfolders": async ({ folderName }) => {
        const treeItem = page.getByRole("treeitem", {
          name: new RegExp(folderName),
        });
      ...
      "the selected folder details should be displayed": async ({ folderName, folderId }) => {
        await expect(page.getByTestId("selected-folder")).toBeVisible();
        await expect(page.getByTestId("folder-name")).toHaveText(folderName);
        await expect(page.getByTestId("folder-id")).toHaveText(folderId.toString());
      },
      ...
      "additional folders should be loaded": async () => {
        await expect(page.getByRole("treeitem", { name: /Subfolder 21/ })).toBeVisible();
      },
...
test.describe("FolderTree", () => {
  feature("Initially displays root folder listing", async ({ Given, Then }) => {
    await Given["the FolderTree component is rendered with mocked API responses"]();
  ...
  feature("Allows expanding folder nodes to show subfolders", async ({ Given, When, Then }) => {
    await Given["the FolderTree component is rendered with mocked API responses"]();
  ...
  feature("Shows Load More button for folders with more than 20 items", async ({ Given, When, Then }) => {
    await Given["the FolderTree component is rendered with mocked API responses"]();
    await When["the user expands a folder node"]({
      folderName: "Research Projects",
    });
  ...
  feature("Loads additional folders when Load More is clicked", async ({ Given, When, Then }) => {
    await Given["the FolderTree component is rendered with mocked API responses"]();
    await When["the user expands a folder node"]({
      folderName: "Research Projects",
    });
  ...
  feature("Prevents creating folders with empty names", async ({ Given, When, Then }) => {
    await Given["the FolderTree component is rendered with mocked API responses"]();
    await When["the user clicks the add folder button for a folder"]({
      folderName: "Research Projects",
    });
