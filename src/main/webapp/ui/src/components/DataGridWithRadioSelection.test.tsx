...
const radioFor = (name: string): HTMLInputElement =>
...
describe("DataGridWithRadioSelection", () => {
  describe("Basic Rendering and Initial State", () => {
    test("No row is selected by default", () => {
      ...
      expect(
        screen.queryAllByRole("radio").filter((radio) => (radio as HTMLInputElement).checked)
      ).toHaveLength(0);
      expect(screen.getByTestId("selection-indicator")).toHaveTextContent(
        "Nothing selected"
      );
    });
    ...
    test("Radio buttons have the correct ARIA labels", () => {
      ...
      expect(within(firstRow).getByRole("radio")).toHaveAttribute(
        "aria-label",
        "Select John Doe"
      );
    });
  ...
  describe("Controlled vs Uncontrolled Mode", () => {
    ...
    test("Controlled mode reflects external selection state", async () => {
      ...
      // John (id 1).
      render(
        <ControlledDataGridWithRadioSelectionExample initialSelectedRowId={1} />
      );
