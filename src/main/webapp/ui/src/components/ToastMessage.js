...
export default function Snackbars(_props) {
    ...
    return (
        ...
        >
            {toasts.map((toast_params) => (
                <ToastMessage key={toast_params.id} closeToast={(id) => removeToast(id)} {...toast_params} />
            ))}
        </div>
    );
}
...
const MySnackbarContentWrapper = forwardRef((props, ref) => {
    ...
    return (
        <SnackbarContent
            ...
            action={[
                <IconButton data-test-id="toast-close" key="close" aria-label="close" color="inherit" onClick={onClose}>
