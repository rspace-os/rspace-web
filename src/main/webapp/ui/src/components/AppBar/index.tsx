...
const NotificationCounter = ({ currentUser }: { currentUser: Person }) => {
    ...
    return (
        <Box ml={1} role="status" aria-live="polite" aria-relevant="text" aria-label="notifications and messages">
            <AdjustedBadge badgeContent={notificationCount + messageCount + specialMessageCount} color="error">
...
const StyledAvatar: React.FC<{ size?: "small" } & React.ComponentProps<typeof Avatar>> = styled(Avatar)(
    ({ size }: { size?: "small" }) => ({
        /*
...
const DynamicAvatar = ({
    ...
    return FetchingData.match(uiNavigationData, {
        ...
        success: ({ operatedAs, userDetails: { profileImgSrc, fullName } }) => {
            if (operatedAs) {
                if (size === "small") return <OperateAsIcon sx={{ color: "red !important" }} />;
                return (
            ...
            return (
                <StyledAvatar size={size} {...(profileImgSrc !== null ? { src: profileImgSrc } : {})}>
                    {fullName[0]}
                </StyledAvatar>
            );
        },
...
const OrcidIcon = styled(({ className }: { className?: string }) => (
    <svg xmlns="http://www.w3.org/2000/svg" version="1.1" viewBox="0 0 50 50" className={className}>
        <g>
            <g id="Layer_1">
                <g>
                    <path
                    ...
                    />
                    <g>
                        <path className="whiteElements" d="M17.1,36.1h-2.9V15.7h2.9v20.3Z" />
                        <path
...
function GalleryAppBar({
    ...
    }, [uiNavigationData, setBrandingHref]);
        ...
        <AppBar position="relative" aria-label={variant === "page" ? "page header" : "dialog header"}>
            <Toolbar variant="dense">
                ...
                {!isViewportSmall && variant === "page" && (
                    <Stack direction="row" spacing={2} sx={{ mx: 1 }} component="nav" aria-label="main links">
                        <Link
                            target="_self"
                ...
                {isViewportSmall && variant === "page" && (
                    <>
                        <List component="nav" aria-label="Main Navigation" disablePadding sx={{ ml: 1 }}>
                            <ListItemButton
                                dense
                                id="app-menu-button"
                                aria-haspopup="menu"
                ...
                {variant === "page" && (
                    <>
                        {FetchingData.getSuccessValue(fetchedCurrentUser)
                            .map((currentUser) => (
                                <NotificationCounter key="notification counter" currentUser={currentUser} />
                            ))
                            .orElse(null)}
                        <Box ml={1}>
                            <IconButtonWithTooltip
                                                    ...
                                                    <ListItemText
                                                        ...
                                                        primaryTypographyProps={{
                                                            sx: {
                                                                ...
                                                                alignItems: "center",
                                                                textDecoration:
                                                                    userDetails.orcidId === null ? "none" : "underline",
                                                            },
                                                        }}
                                                        primary={
                                                            userDetails.orcidId === null ? (
