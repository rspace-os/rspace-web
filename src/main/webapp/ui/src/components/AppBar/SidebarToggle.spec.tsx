...
test.describe("Functional requirements", () => {
    test("Clicking the button should toggle the sidebar.", async ({ mount, page }) => {
...
test.describe("Accessibility", () => {
    ...
    test("When sidebar is open, aria-expanded should be true.", async ({ mount, page }) => {
    ...
    test("When sidebar is closed, aria-expanded should be false.", async ({ mount, page }) => {
        await mount(<SimplePageWithSidebarToggle sidebarOpen={false} />);
        const ariaExpanded = await page.getAttribute("button", "aria-expanded");
        expect(ariaExpanded).toBe("false");
    });

    test("Applies the sidebarId as the aria-controls attribute.", async ({ mount, page }) => {
