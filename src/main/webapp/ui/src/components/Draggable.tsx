...
const useStyles = makeStyles<{ draggable: boolean; position: Point }>()((_theme, { draggable, position }) => ({
    container: {
        position: "absolute",
        left: position.x,
        top: position.y,
...
function Draggable({
    ...
    const [initiating, setInitiating] = React.useState<NodeJS.Timeout | null>(null);
    ...
    const onPointerDown = (_event: React.PointerEvent<HTMLDivElement>) => {
        ...
        if (!(event.target instanceof HTMLElement)) throw new Error("event.target is not a HTML Element");
    ...
    const onPointerMove = (event: React.PointerEvent<HTMLDivElement>) => {
        if (dragging) {
            ...
            const x = newPointOnViewport.x - parentTopLeftCorner.x - cursorOffsetInsideDiv.x;
            const y = newPointOnViewport.y - parentTopLeftCorner.y - cursorOffsetInsideDiv.y;

            setPosition({
                x: clamp(x, 0, dimensionsOfTheParent.width - dimensionsOfThisDiv.width),
                y: clamp(y, 0, dimensionsOfTheParent.height - dimensionsOfThisDiv.height),
            });
        }
    };
