...
const feature = test.extend<{
  ...
  When: {
    "the user clicks on tree item {string}": (itemName: string) => Promise<void>;
    "the user ctrl+clicks on tree item {string}": (itemName: string) => Promise<void>;
    ...
    "the user focuses on tree item {string}": (itemName: string) => Promise<void>;
    "the user presses ArrowDown": () => Promise<void>;
    "the user presses Space": () => Promise<void>;
  };
  Then: {
    "tree item {string} should be selected": (itemName: string) => Promise<void>;
    "tree item {string} should not be selected": (itemName: string) => Promise<void>;
    "tree item {string} should be expanded": (itemName: string) => Promise<void>;
    "tree item {string} should be collapsed": (itemName: string) => Promise<void>;
  ...
  Given: async ({ mount }, use) => {
    await use({
      "a simple tree is rendered": async () => {
        ...
        await mount(<SimpleTreeExample onSelectionChange={selectionSpy.handler} />);
        return { selectionSpy };
      },
      "a multi-select tree is rendered": async () => {
        const selectionSpy = createSelectionSpy();
        await mount(<MultiSelectTreeExample onSelectionChange={selectionSpy.handler} />);
        return { selectionSpy };
      },
      "an expandable tree is rendered": async () => {
        const expansionSpy = createExpansionSpy();
        await mount(<ExpandableTreeExample onExpansionChange={expansionSpy.handler} />);
        return { expansionSpy };
      },
      "a controlled tree is rendered": async () => {
        ...
        const expansionSpy = createExpansionSpy();
        await mount(
          <ControlledTreeExample onSelectionChange={selectionSpy.handler} onExpansionChange={expansionSpy.handler} />,
        );
  ...
  When: async ({ page }, use) => {
    await use({
      ...
      "the user ctrl+clicks on tree item {string}": async (itemName: string) => {
        await page.getByRole("treeitem", { name: itemName }).click({
  ...
  Then: async ({ page }, use) => {
    await use({
      "tree item {string} should be selected": async (itemName: string) => {
        await expect(page.getByRole("treeitem", { name: itemName })).toHaveAttribute("aria-checked", "true");
      },
      "tree item {string} should not be selected": async (itemName: string) => {
        await expect(page.getByRole("treeitem", { name: itemName })).toHaveAttribute("aria-checked", "false");
      },
      "tree item {string} should be expanded": async (itemName: string) => {
        await expect(page.getByRole("treeitem", { name: itemName })).toHaveAttribute("aria-expanded", "true");
      },
      "tree item {string} should be collapsed": async (itemName: string) => {
        await expect(page.getByRole("treeitem", { name: itemName })).toHaveAttribute("aria-expanded", "false");
      },
      "tree item {string} should be visible": async (itemName: string) => {
        await expect(page.getByRole("treeitem", { name: itemName })).toBeVisible();
      },
      ...
      "all expandable items should be expanded": async () => {
        ...
        for (let i = 0; i < count; i++) {
          await expect(expandableItems.nth(i)).toHaveAttribute("aria-expanded", "true");
        }
      },
      "all expandable items should be collapsed": async () => {
        ...
        const count = await expandableItems.count();
        for (let i = 0; i < count; i++) {
          await expect(expandableItems.nth(i)).toHaveAttribute("aria-expanded", "false");
        }
      },
...
test.describe("Tree Component", () => {
  test.describe("Single Selection", () => {
    feature("Should select a single tree item", async ({ Given, When, Then }) => {
    ...
    feature("Should replace selection when clicking different item", async ({ Given, When, Then }) => {
  ...
  test.describe("Multi Selection", () => {
    feature("Should select multiple items with Ctrl+click", async ({ Given, When, Then }) => {
  ...
  test.describe("Controlled Tree", () => {
    feature("Should handle both selection and expansion in controlled mode", async ({ Given, When, Then }) => {
  ...
  test.describe("Accessibility", () => {
    ...
    feature("Should have proper ARIA attributes for selected state", async ({ Given, When, Then }) => {
      await Given["a simple tree is rendered"]();
      await When["the user clicks on tree item {string}"]("Item 1");
      await Then["tree item {string} should be selected"]("Item 1");
    });
    feature("Should maintain focus management", async ({ Given, When, Then }) => {
      await Given["a simple tree is rendered"]();
      await When["the user clicks on tree item {string}"]("Item 1");
  ...
  test.describe("Keyboard Navigation", () => {
    feature("Should navigate with arrow keys", async ({ Given, When, Then }) => {
      await Given["a simple tree is rendered"]();
