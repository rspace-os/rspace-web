...
>((theme, { loading, error, selectedFilename }) => ({
    ...
    const avatar = match<void, React.ReactElement | null>([
        ...
        [() => Boolean(selectedFilename), <CheckCircleIcon key="checkicon" className={classes.avatar} />],
        [() => true, null],
    ])();
...
const SelectedFileInfo = withStyles<SelectedFileInfoArgs, { details: string; detailsLabel: string; filename: string }>(
    (theme) => ({
        details: {
            display: "flex",
            alignItems: "center",
