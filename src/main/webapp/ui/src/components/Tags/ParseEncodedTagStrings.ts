 ...
 * requested.
 */
export const SMALL_DATASET_SIGNAL = "=========SMALL_DATASET_IN_SINGLE_BLOCK=========";
...
export const decodeTagString = (str: string): string =>
  str.replaceAll("__rspactags_forsl__", "/").replaceAll("__rspactags_comma__", ",");
...
export function encodeTags(tags: Array<Tag>): Optional<string> {
  return ArrayUtils.all(
    tags.map((tag) =>
      lift3(
        (uri, voc, ver) =>
          `${encodeTagString(tag.value)}__RSP_EXTONT_URL_DELIM__${encodeTagString(
            uri,
          )}__RSP_EXTONT_NAME_DELIM__${encodeTagString(voc)}__RSP_EXTONT_VERSION_DELIM__${encodeTagString(ver)}`,
        tag.uri,
        tag.vocabulary,
        tag.version,
      ).orElseTry(() => {
        if (tag.vocabulary.isEmpty() && tag.uri.isEmpty() && tag.version.isEmpty()) return Optional.present(tag.value);
...
export function parseEncodedTags(encodedTags: Array<string>): Array<Tag> {
  return encodedTags
    .filter((tag) => tag !== "" && tag !== SMALL_DATASET_SIGNAL && tag !== FINAL_DATA_SIGNAL)
    .map((tag: string): Tag => {
      const groups =
