...
axios.get(url).then((response) => {
    ...
    members
        .filter((member) => Boolean(document.getElementById(`autoshareStatus-${member.userId}`)))
        .forEach((member) => {
            const root = createRoot(document.getElementById(`autoshareStatus-${member.userId}`));
            root.render(
