...
function GroupOntologiesManager({ groupId, isCloud, canManageOntologies }) {
            ...
            <>
                {isCloud && canManageOntologies && (
                    <Tooltip title={title} aria-label={title}>
                        <div>
                            <Button className={classes.publishManager} variant="outlined" size="small" disabled>
    ...
    function OntologiesButton(props) {
        return (
            <>
                {!isCloud && canManageOntologies && (
                    <Button className={classes.publishManager} onClick={props.callback} variant="outlined" size="small">
                        {props.mode} Ontologies
                    </Button>
                )}
            </>
        ...
        return (
            <>
                ...
                <Button onClick={props.onConfirm} color="primary" disabled={waiting}>
                    Confirm
                    {waiting && <CircularProgress size={20} className={classes.loading} />}
                </Button>
            </>
        );
    }
            ...
            {loaded && !ontologiesEnforcedStatus && (
                <>
                    ...
                    <Dialog open={enableDialogOpen} onClose={() => setEnableDialogOpen(false)} maxWidth="sm" fullWidth>
                        ...
                        <DialogContent>
                            <DialogContentText>
                                Enforcing ontologies for tags will result in all new tags coming from autopopulated
                                values read from ontology files. These ontology files must be shared with a Group and
                                any user creating new tags must be a member of that Group. Collaboration Groups can be
                                used for this purpose. Tags can not be created from free text and will not autopopulate
                                from existing tags.
                            </DialogContentText>
                        </DialogContent>
                        <DialogActions>
            ...
            {loaded && ontologiesEnforcedStatus && (
                <>
                    ...
                    >
                        ...
                        <DialogContent>
                            <DialogContentText>
                                Un-enforcing use of ontologies for tags will result in group members being able to enter
                                free text when creating new tags. They may also use values autopopulated from ontology
                                files. These ontology files may be their own, or may be files shared with them. They may
                                also use values autopopulated from existing tags.
                            </DialogContentText>
                        </DialogContent>
                        <DialogActions>
                            <DialogButtons
