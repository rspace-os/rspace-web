...
export default function Filter({
    ...
    return (
        <>
                ...
                {Object.entries(filter).map(([key, config]) => (
                    <Grid item xs={2} key={key}>
                        <TextField
                            ...
                            variant="outlined"
                            value={config.value}
                            inputProps={config.type === "number" ? { inputMode: "numeric" } : {}}
                            error={!valid[key]}
                    ...
                    <Grid item key={key} xs={2}>
                        <FormControl fullWidth>
                            <Autocomplete
                                ...
                                renderInput={(props) => (
                                    <TextField variant="outlined" {...props} label={config.label} />
                                )}
            ...
            <Button variant="contained" endIcon={<ClearIcon />} className={classes.button} onClick={handleClear}>
                Clear
            </Button>
            <Button
                variant="contained"
