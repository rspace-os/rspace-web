...
function PyratListing({ serverAlias, setSelectedAnimals }) {
    ...
    const [filterMultiReq, setFilterMultiReq] = useState({
        licence_id: {
            ...
            enumObj: {},
            renderFunc: ({ license_id, license_number }) => [license_id, { label: license_number, value: license_id }],
        },
        responsible_id: {
            label: "Responsible",
            value: "",
            ...
            enumObj: {},
            renderFunc: ({ userid, fullname }) => [userid, { label: fullname, value: userid }],
        },
        project_id: {
            label: "Project",
            value: "",
    ...
    }, [handlePyratError, pyrat.get, serverAlias]);
    ...
    }, [filterSpecial, handlePyratError, pyrat.get, serverAlias]);
            ...
            .then((response) => {
                if (response.data) {
                    ...
                    animals.forEach((animal) => {
                        ...
                        if (animal.projects) {
                            animal.projects = animal.projects.map((project) => project.project_label).join(", ");
                        }

                        if (animal.mutations) {
                            animal.mutations = animal.mutations
                                .map((mutation) => `${mutation.mutationname} ${mutation.mutationgrade}`)
                                .join(", ");
                        }
                    });
    ...
    }, [order, orderBy, rowsPerPage, page, filter, filterMultiReq, filterSpecial, serverAlias]);
    ...
    React.useEffect(() => {
        setSelectedAnimals(animals.filter((animal) => selectedAnimalIds.includes(animal.eartag_or_id)));
    ...
    function handleOptionsFilterChange(filterKey, input) {
        if (new Set(Object.keys(filterMultiReq)).has(filterKey)) {
            (async () => {
                try {
                    const response = await pyrat.get(`${filterMultiReq[filterKey].query}${input}`);

                    if (response.data) {
                        const enumObj = Object.fromEntries(response.data.map(filterMultiReq[filterKey].renderFunc));
                        setFilterMultiReq({
                            ...filterMultiReq,
            ...
            <DialogContent>
                {FetchingData.match(servers, {
                    ...
                    success: (servers) => {
                        if (servers.length === 1)
                            return (
                                <PyratListing serverAlias={servers[0].alias} setSelectedAnimals={setSelectedAnimals} />
                            );
                        if (serverAlias)
                            return <PyratListing serverAlias={serverAlias} setSelectedAnimals={setSelectedAnimals} />;
                        return (
                            <>
