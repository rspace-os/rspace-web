...
export const getArticle = async (articleId: ArticleId): Promise<{ data: Article }> => {
    const article = await makeGetArticleRequest(articleId);
    return { data: article.data.joveArticle };
};

const makeSearchRequest = (joveSearchRequest: unknown): AxiosPromise<{ joveSearchResult: JoveSearchResult }> => {
