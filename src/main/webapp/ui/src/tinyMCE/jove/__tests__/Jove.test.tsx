...
describe("Renders page with jove data ", () => {
    ...
    it("displays jove search results", async () => {
        render(<Jove />);
        await screen.findByText("Title");
        await screen.findByText("Induction and Validation of Cellular Senescence in Primary Human Cells");
        expect(screen.getByTestId("title0")).toHaveTextContent(
            "Induction and Validation of Cellular Senescence in Primary Human Cells",
        );
        expect(within(screen.getByTestId("thumbnail0")).getByRole("img")).toHaveAttribute(
            "src",
            "https://cloudfront.jove.com/files/thumbs/57782_t.png",
        );
        expect(screen.getByTestId("section0")).toHaveTextContent("Developmental Biology");
    });
    ...
    it("displays error message if 404 returned", async () => {
        ...
        expect(screen.getByText("Unable to retrieve any relevant results.")).toBeInTheDocument();
    });
    ...
    it("displays error message if 500 returned", async () => {
        ...
        render(<Jove />);
        await screen.findByText("Error");
        expect(screen.getByText("Unknown issue, please attempt to relogin to RSpace.")).toBeInTheDocument();
    });
});
