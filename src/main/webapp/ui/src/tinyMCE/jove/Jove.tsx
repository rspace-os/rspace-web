...
import { type Article, type ArticleId, search } from "./JoveClient";
...
export const getOrder = (): OrderType => {
    ...
    if (order !== "asc" && order !== "desc")
        throw new InvalidLocalStorageState(`Expected "asc" or "desc", found ${storedJson ?? "nothing"}`);
    return order;
};
export const getOrderBy = (): keyof Article | "" => {
    ...
    const validKeys: Set<keyof Article | ""> = new Set(["id", "hasvideo", "section", "thumbnail", "title", "url", ""]);
    ...
    throw new InvalidLocalStorageState(`Expected a key of Article, found ${orderBy}`);
};
...
export default function Jove(): React.ReactNode {
    ...
    const [searchParam, setSearchParam] = useState<(typeof SearchParam)[keyof typeof SearchParam]>(
    ...
    const [order, setOrder] = useLocalStorage<OrderType>("joveSearchOrder", Order.asc);
    const [orderBy, setOrderBy] = useLocalStorage<"" | "thumbnail" | "title" | "section">("joveSearchOrderBy", "");
    ...
    const searchJove = async (
        ...
        setErrorMessage("");
        try {
            const searchResult = await search(searchParam, searchQuery, page, rowsPerPage);
            if (searchResult) {
                // Only populate the search results array with results that have videos.
                setSearchResults(searchResult.data.articlelist.filter((article) => article.hasvideo));
    ...
    const _handleSearchParamChange = (event: { target: { value: (typeof SearchParam)[keyof typeof SearchParam] } }) => {
    ...
    useEffect(() => {
        void searchJove(searchParam, searchQuery, page, rowsPerPage);
        setOrderBy("");
    }, [page, rowsPerPage, searchParam, searchJove, searchQuery, setOrderBy]);

    SELECTED_RESULTS = useMemo(() => {
        const selected_searchResults = searchResults.filter((results) => selectedJoveIds.includes(results.id));
    ...
    }, [selectedJoveIds, searchResults.filter]);
                ...
                <Grid container spacing={1}>
                    <Grid item xs={12}>
                        {errorReason !== ErrorReason.None ? (
                            <ErrorView errorReason={errorReason} errorMessage={errorMessage} />
                        ) : (
                            <ResultsTable
                                page={page}
