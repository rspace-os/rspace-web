    ...
    return (
        <Toolbar className={classes.root}>
            <Typography className={classes.title} color="inherit" variant="subtitle1">
                The link currently points at {props.selected ? `version ${props.selected}.` : "latest version."}
            </Typography>
        </Toolbar>
    );
};
...
export default function InternalLink(props) {
    ...
    const generateParams = () => {
        ...
        return [
            revision.id, // id
            `${revision.oid.idString}${selected === latestRevision.version ? "" : `v${selected}`}`, // global id
            revision.name, // name
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await axios.get(`/workspace/revisionHistory/ajax/${props.id}/versions`);
                ...
                const idx = fetchedRevisions.findIndex((r) => r.version === props.version);
                if (idx !== -1) {
    ...
    return (
        <Dialog open={open} onClose={handleClose} aria-labelledby="form-dialog-title" fullWidth={true} maxWidth="lg">
            ...
            <DialogContent>
                ...
                <Table className={classes.table} size="small" aria-label="Internal link version options">
                    <TableBody>
                        <TableRow
                            hover
            ...
            <DialogActions>
                ...
                <Button
                    ...
                    variant="contained"
                    disableElevation
                    disabled={selected === props.version || (!props.version && selected === latestRevision.version)}
                >
...
document.addEventListener("tinymce-insert-revision", (e) => {
    ...
    root.render(<WrappedInternalLink id={e.detail.id} version={e.detail.version} initialEl={e.detail.el} />);
});
