...
describe("Has defaultOrderBy ", () => {
    ...
    it("returns Order By value in localStorage", () => {
        localStorageMock.getItem = jest.fn().mockImplementationOnce(() => '"description"');
...
const setUpProjectAndScreensInDescendingOrder = async () => {
    setUpLocalStorageWithOrder(Order.desc);
    await setUpComponent();
    await screen.findByText("idr0018-neff-histopathology/experimentA", undefined, {
        timeout: 5500,
    });
};
...
const navigateFromScreenToPlate = async (screenName, screenID, plateID, handleSetup = true) => {
    ...
    const fetchPlates = screen.getByTestId(`screen_fetch_childrenLink_${screenID}`);
...
const navigateFromScreenToPlateWIthoutSetup = async (screenName, screenID, plateID) => {
    await navigateFromScreenToPlate(screenName, screenID, plateID, false);
};
const navigateFromProjectToDataset = async (projectName, projectID, datasetID) => {
    setUpLocalStorageWithOrder(Order.asc);
    await setUpComponent();
    await screen.findByText(projectName, undefined, {
        timeout: 5500,
    });
    const fetchDatasets = screen.getByTestId(`project_fetch_childrenLink_${projectID}`);
...
const clickChildLinkAndShowPlateAcquisition = async (childLInkID, plateAcquisitionID) => {
    const fetchPlatesAcquisitions = screen.getByTestId(`plate_fetch_childrenLink_${childLInkID}`);
    fireEvent.click(fetchPlatesAcquisitions);
    await waitFor(
        () =>
            screen.findByTestId(`plateAcquisition_name_display_${plateAcquisitionID}`, undefined, {
                timeout: 5500,
            }),
        {
            timeout: 5500,
...
const clickChildLinkAndShowPlateAcquisitionWithImages = async (childLInkID, plateAcquisitionID, imageID) => {
    await waitFor(
        () =>
            screen.findByTestId(`plate_fetch_childrenLink_${childLInkID}`, undefined, {
                timeout: 15500,
            }),
        {
            timeout: 5500,
    ...
    const fetchGridOfImages = screen.getByTestId(`plate_fetch_childrenLink_${childLInkID}`);
    ...
    expect(screen.getByTestId(`plateAcquisition_name_display_${plateAcquisitionID}`)).toBeInTheDocument();
};

const clickOnImageWithIDInGridAndAwaitInsertionAsChildOfFullImageData = async (imageID) => {
...
const clickDatasetImageGridLinkAndCheckForImageWithID = async (datasetID, targetImageID) => {
...
const clickImageGridLinkAndCheckForImageWithID = async (imageGridLinkID, targetImageID) => {
    const fetchGridOfImages = screen.getByTestId(`plateAcquisition_show_grid_${imageGridLinkID}`);
    fireEvent.click(fetchGridOfImages);
    await waitFor(
        () =>
            screen.findByTestId(`image_img_${targetImageID}`, undefined, {
...
describe("Renders page with results data ", () => {
    ...
    it("displays Projects Only", async () => {
        ...
        expect(screen.queryByText("idr0094-ellinger-sarscov2/screenB")).not.toBeInTheDocument();
    }, 9999);
    ...
    it("displays Screens Only", async () => {
        ...
        expect(screen.queryByText("idr0018-neff-histopathology/experimentA")).not.toBeInTheDocument();
    }, 9999);
    ...
    it("when screen shows plates it also shows plates children", async () => {
        ...
        await clickChildLinkAndShowPlateAcquisition(2551, 2661);
        await hideChildren("screen", 3, "plate", 2551, 192);
        await navigateFromScreenToPlateWIthoutSetup("idr0001-graml-sysgro/screenA", 3, 2551);
        const plateAqcquisitionID = "plateAcquisition_name_display_2661";
    ...
    it("plates with only one (or none) plate acquisition can directly show grids of images beside the plate acquisition", async () => {
        await navigateFromScreenToPlate("idr0002-heriche-condensation/screenA", 102, 422);
    ...
    it("On click of an image in the grid of images it is inserted into the document as a new child and has annotations with image data", async () => {
        await navigateFromScreenToPlate("idr0002-heriche-condensation/screenA", 102, 422);
        await clickChildLinkAndShowPlateAcquisitionWithImages(422, 422, 179693);
        await clickOnImageWithIDInGridAndAwaitInsertionAsChildOfFullImageData(179693);
        assertElemWithTestIDHasTextContent(
            "path3",
