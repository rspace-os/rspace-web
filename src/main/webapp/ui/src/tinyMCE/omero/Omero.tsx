...
function Omero({ omero_web_url }: OmeroArgs): React.ReactNode {
    ...
    const [_latestGridOfThumbnails, setLatestGridOfThumbnails] = useState<OmeroItem["imageGridDetails"] | null>(null);
    ...
    }, [latestPlateAcquisition, addGridOfThumbnailsToItem]);
    ...
    const itemHasHiddenChildren = (item: OmeroItem) => {
        return items.some(
            (anItem) =>
                item.id === anItem.parentId && (anItem.hide === true || anItem.hideAsIndirectDescendant === true),
        );
    };
        ...
        if (item.hiddenImageGridDetails?.[gridIndex]) {
            ...
            images.map((dataValue, pos) => {
                ...
                const onClick = async (event: any): Promise<void> => {
                    ...
                    event.target.style.cssText = "opacity: 1; border: 4px solid blue;padding:0px";
    ...
    const addDataToDisplayList = (
        ...
        formatDataForDisplay(items, parentPaths, omeroConnectionKey, parent, shouldSort).map((item) => {
        ...
        return annotations.map((annotation, index) => {
            ...
            return annotation.indexOf("Publication Title") !== -1 ? (
                <>
                    {" "}
                    <dl>
                        <dt id={`${item.type}_first_description_${item.id}`} className={classes.firstDescription}>
                            {" "}
                            {publicationTitle}
                        </dt>
                        <dt id={`${item.type}_rest_description_${item.id}`} className={classes.restOfDescription}>
                            {" "}
                            {restOfText}
                        </dt>
                    </dl>
                ...
                <li data-testid={`${item.type}_annotation_${item.id}_${index}`}>{annotation}</li>
            );
        });
    };
    ...
    const makeEmptyThumbNail = () => {
        return <img title={"no image"} style={{ padding: "4px" }} src={"/images/White_square.png"} />;
    };
    const makeThumbNail = (
        data: OmeroItem,
        omeroConnectionKey: string,
                ...
                <Grid container spacing={1}>
                    <Grid item xs={12}>
                        <ResultsTable
                            ...
                            // @ts-expect-error - The component uses forwardRef but TypeScript isn't recognizing it
                            ref={scrollRef}
                        />
                    </Grid>
