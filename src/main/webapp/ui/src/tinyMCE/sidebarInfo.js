...
export default function SidebarInfo(props) {
    ...
    useEffect(() => {
        ...
        document.addEventListener("tinymce-chem-inserted", (e) => {
            const innerDoc = props.iframe.contentDocument || props.iframe.contentWindow.document;
            addItem($(innerDoc).find(`img#${e.detail}`)[0]);
        });

        // detect chem updated
        document.addEventListener("tinymce-chem-updated", (_e) => {
            closeAll();
        });
    ...
    }, [addItem, closeAll, props.iframe.contentDocument, props.iframe.contentWindow.document, removeItem]);
    ...
    }, [items, closeSidebar, open, openSidebar]);
    ...
    const onMutationsObserved = (mutations) => {
        mutations.forEach((mutation) => {
            ...
            if (mutation.removedNodes.length) {
                ...
                if (element.prevObject[0].classList?.contains("chem")) {
