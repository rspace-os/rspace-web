...
function Galaxy({ fieldId, recordId, attachedFileInfo }: GalaxyArgs) {
        ...
        async function getGalaxyIntegrationInfo() {
            try {
                ...
                if (!authenticatedServers || authenticatedServers.length === 0) {
                    throw new Error(
                        "You must configure at least one Galaxy server to use this feature. Please go to the Galaxy integration settings to do this. " +
    ...
    useEffect(() => {
        ...
        doTheUpload();
    }, [doUpload, sendDataToGalaxy]);
    useEffect(() => {
        window.parent.postMessage(
            {
                mceAction:
                    selectedAttachmentIds.length === 0 || servers.length === 0 ? "no-data-selected" : "data-selected",
            },
            "*",
        );
        void setAttachedFiles(attachedFileInfo);
    }, [selectedAttachmentIds, attachedFileInfo, servers.length]);
                        ...
                        <TitledBox title="View Workflow in Galaxy" border>
                            <Stack spacing={2} alignItems="flex-start">
                                ...
                                <p>
                                    ...
                                    (Opens in new tab.) (You must be logged into Galaxy or you will see 'Unnamed
                                    History'){" "}
                                </p>
                                <p>
                                    <b>
                                        {" "}
                                        The data you have uploaded to Galaxy has links back to RSpace present in its
                                        'annotation' metadata.
                                    </b>
                                </p>
                                <p>
                                    <b>
                                        {" "}
                                        Data uploaded to this history is now viewable by clicking on the 'workflow' icon
                                        which will appear in your document. Any invocations in Galaxy which use this
                                        data will be tracked in RSpace, the data being updated whenever you click on
                                        this 'workflow' icon. The badge count on the workflow icon indicates how many
                                        Galaxy Invocations are using the uploaded data.
                                    </b>
                                </p>
                            </Stack>
                        </TitledBox>
                        ...
                        <>
                            <TitledBox title="Choose Data" border>
                                <Stack spacing={2} alignItems="flex-start">
                                    ...
                                    <p>
                                        <b>
                                            "RSPACE_" + document name + "_" + global ID of document + "_" + name of
                                            field data was attached to + "_" + global ID of that field.
                                        </b>
                                    </p>
                                    ...
                                    <p>
                                        <b>
                                            RSpace will store the details of the files you have uploaded and also any
                                            use of these files on Galaxy in Invocations will be tracked.
                                        </b>
                                    </p>
                                    {errorMessage && (
                                        <ErrorView
