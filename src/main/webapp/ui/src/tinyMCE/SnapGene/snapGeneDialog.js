        ...
        <Dialog open={open} onClose={handleClose} aria-labelledby="form-dialog-title" fullWidth={true} maxWidth="xl">
            ...
            <DialogContent>
                <Grid container spacing={2}>
                    ...
                    {tab === 0 && (
                        <DnaPreview id={props.id} clicked={clicked["0"]} setDisabled={(d) => setDisabled(d)} />
                    )}
                    {tab === 1 && (
                        <EnzymeTable id={props.id} clicked={clicked["1"]} setDisabled={(d) => setDisabled(d)} />
                    )}
                    {tab === 2 && (
                        <FastaView id={props.id} clicked={clicked["2"]} setDisabled={(d) => setDisabled(d)} />
                    )}
                    {tab === 3 && <OrfTable id={props.id} clicked={clicked["3"]} setDisabled={(d) => setDisabled(d)} />}
                </Grid>
            </DialogContent>
            <DialogActions>
