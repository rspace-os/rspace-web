...
export const getMostRecentCompletedBooking = (
    ...
    const matching = relevantBookings
        ...
        .filter(
            (bookingDetail: BookingDetails) => getStatus(bookingDetail.id, bookingList).toLowerCase() === "completed",
        );
...
const getALabID = (bookingDetails: Array<BookingDetails>): number => {
    ...
    return bookingDetails.find((booking: BookingDetails) => typeof booking.requester_lab.id !== "undefined")
        .requester_lab.id;
};
