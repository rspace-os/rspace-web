...
const getBookingDetails = async (bookingIDs: string): AxiosPromise<Array<BookingDetails>> => {
    return axios.put("/apps/clustermarket/bookings/details", {
        bookingIDs,
    });
};

const getEquipmentDetails = async (equipmentIDs: string): AxiosPromise<Array<EquipmentDetails>> => {
...
export const getRelevantBookings = (
    ...
    if (bookingType === BookingType.BOOKED) {
        relevantBookings = bookingsList.filter((booking) => booking.status.toLowerCase() === bookingType.toLowerCase());
    } else {
        relevantBookings = bookingsList.filter((booking) => booking.status.toLowerCase() !== "declined");
    }
...
export const getAllBookingDetails = async (bookingsList: BookingsList): Promise<Array<BookingDetails>> => {
    const bookingIdsString = bookingsList.map((booking) => booking.id).join();
    return (await getBookingDetails(bookingIdsString)).data;
};

export const getUniqueEquipmentIdsFromBookings = (bookingsList: BookingsList): Array<number> => {
...
export const getAllEquipmentDetails = async (bookingsList: BookingsList): Promise<Array<EquipmentDetails>> => {
