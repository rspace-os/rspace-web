    ...
    return (
        <Alert severity="error">
            ...
            {errorReason === ErrorReason.NetworkError && (
                <>
                    ...
                    is down, or CORS for this server has not been configured properly. If you are responsible for
                    setting up the Clustermarket integration, open developer tools and have a look at the console and/or
                    the network tab to find out what the issue is.
                </>
            )}
            {errorReason === ErrorReason.NotFound && (
                <>
                    Please contact an Admin: Clustermarket returned HTTP status 404. Is Clustermarket endpoint set
                    correctly?
                </>
            )}
        ...
        When a refresh token expires the Clustermarket API responds with 400 response and 'invalid_grant' in the response message */}
            {(errorReason === ErrorReason.Unauthorized || errorMessage.includes("invalid_grant")) && (
                <>Invalid Clustermarket user or client token. Please re-connect to Clustermarket.</>
            )}
