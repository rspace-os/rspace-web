...
import { StoichiometryDialogWithCalculateButtonStory, StoichiometryDialogWithTableStory } from "./dialog.story";
...
const _createOnChangesUpdateSpy = () => {
    ...
    Given: async ({ mount }, use) => {
        await use({
            "the dialog is open without a stoichiometry table": async (spies) => {
                ...
                await mount(
                    <StoichiometryDialogWithCalculateButtonStory onTableCreated={onTableCreatedSpy?.handler} />,
                );
            },
            "the dialog is open with a stoichiometry table": async (spies) => {
                ...
                await mount(
                    <StoichiometryDialogWithTableStory onSave={onSaveSpy?.handler} onDelete={onDeleteSpy?.handler} />,
                );
            },
        });
    },
    When: async ({ page }, use) => {
        await use({
            ...
            "the user edits a cell in the table": async () => {
                ...
                ).then((textOfCells) => textOfCells.findIndex((text) => /notes/i.test(text ?? "")));

                await table.getByRole("row").nth(1).getByRole("gridcell").nth(indexOfNotesColumn).dblclick();
        ...
        await use({
            ...
            "a POST request should have been made to create the stoichiometry table": () => {
                const postRequest = networkRequests.find(
                    (request) => request.url.pathname.includes("/api/v1/stoichiometry") && request.postData !== null,
                );
            ...
            "a PUT request should have been made to update the stoichiometry table": () => {
                const putRequest = networkRequests.find(
                    (request) => request.url.pathname.includes("/api/v1/stoichiometry") && request.method === "PUT",
                );
                expect(putRequest).toBeDefined();
            },
            "a DELETE request should have been made to delete the stoichiometry table": () => {
                const deleteRequest = networkRequests.find(
                    (request) => request.url.pathname.includes("/api/v1/stoichiometry") && request.method === "DELETE",
                );
