...
function StandaloneDialogInner({
    ...
    const [showTable, setShowTable] = React.useState(stoichiometryId !== undefined);
    ...
    }, [open, stoichiometryId, chemistryStatus, addAlert]);
    ...
    const handleSave = () => {
        ...
        setSaving(true);
        if (!stoichiometryId) throw new Error("stoichiometryId is required to save");
        doNotAwait(async () => {
            try {
    ...
    return (
        <Dialog open={actuallyOpen} onClose={doNotAwait(handleClose)} aria-labelledby={titleId} maxWidth="xl" fullWidth>
            <AppBar
                variant="dialog"
            ...
            <DialogContent>
                {actuallyOpen && stoichiometryId === undefined && (
                    <Box display="flex" flexDirection="column" alignItems="center" gap={2} py={4}>
                        ...
                        <Button variant="contained" color="primary" onClick={handleCalculate} disabled={loading}>
                            {loading ? "Calculating..." : "Calculate Stoichiometry"}
                        </Button>
                    </Box>
                )}
                {actuallyOpen && stoichiometryId !== undefined && stoichiometryRevision !== undefined && (
                    <Stack spacing={2} flexWrap="nowrap">
                        <Box>
                            <Typography variant="body2">
                                Double-click to edit Equivalent, Mass, Moles, Actual Mass, Actual Moles, or Notes.
                                Yield/Excess values are calculated automatically, as are each pairing of moles and mass.
                            </Typography>
                            <StoichiometryTable
                                ref={tableRef}
                                editable
            ...
            <DialogActions>
                {showTable && hasTableChanges && (
                    <ValidatingSubmitButton onClick={handleSave} loading={saving} validationResult={IsValid()}>
                        Save Changes
