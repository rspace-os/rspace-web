...
describe("StoichiometryAddReagentDialog", () => {
  ...
  it("shows validation warnings and blocks submission when required fields are missing", async () => {
    ...
    expect(await screen.findByText("stoichiometry.addReagent.validation.smilesRequired")).toBeVisible();
    expect(screen.getByText("stoichiometry.addReagent.validation.nameRequired")).toBeVisible();
  });
  ...
  it("submits the SMILES string and trimmed name, then closes the dialog", async () => {
    ...
    await user.type(screen.getByRole("textbox", { name: "stoichiometry.addReagent.name" }), "  Water  ");
    await user.type(screen.getByRole("textbox", { name: "stoichiometry.addReagent.smilesString" }), "O");
    await user.click(screen.getByRole("button", { name: "stoichiometry.addReagent.addChemical" }));
  ...
  it("clears form state when cancelled", async () => {
    ...
    const nameInput = screen.getByRole("textbox", { name: "stoichiometry.addReagent.name" });
    const smilesInput = screen.getByRole("textbox", { name: "stoichiometry.addReagent.smilesString" });
