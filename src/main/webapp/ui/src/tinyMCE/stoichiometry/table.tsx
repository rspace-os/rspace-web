...
function calculateActualYieldOrExcess(molecule: EditableMolecule, limitingReagentMoles: number): number | null {
    ...
    } else if ((molecule.role === "REACTANT" || molecule.role === "AGENT") && !molecule.limitingReagent) {
...
function updateYieldAndExcess(molecules: ReadonlyArray<EditableMolecule>): ReadonlyArray<EditableMolecule> {
    ...
    const limitingReagentMoles =
        (calculateMoles(limitingReagent.actualAmount, limitingReagent.molecularWeight) ?? 0) /
    ...
    return molecules.map((molecule) => {
        const actualYield = calculateActualYieldOrExcess(molecule, limitingReagentMoles);
...
export function calculateUpdatedMolecules(
    ...
    function applyChanges(newProperties: Partial<EditableMolecule>, molecules = allMolecules) {
        return molecules.map((molecule) =>
            molecule.id === editedRow.id ? { ...molecule, ...newProperties } : molecule,
        );
    }

    const beforeMolecule = allMolecules[allMolecules.findIndex((m) => m.id === editedRow.id)];

    if (beforeMolecule.id !== editedRow.id)
        throw new Error("ID is an intrinsic property of the chemical and cannot be modified");
    ...
    if (beforeMolecule.role !== editedRow.role) throw new Error("Modifying the role of a molecule is not supported");
    ...
    if (editedRow.actualMoles !== null) {
        return updateYieldAndExcess(
            applyChanges({
                actualAmount:
                    editedRow.actualMoles === null ? null : editedRow.actualMoles * beforeMolecule.molecularWeight,
            }),
        );
    }
...
function LoadingDialog({
    ...
    return (
        ...
        >
            <DialogContent>
                <Box display="flex" flexDirection="column" justifyContent="center" alignItems="center" py={3} gap={2}>
...
>(function StoichiometryTable({ editable = false, onChangesUpdate, stoichiometryId, stoichiometryRevision }, ref) {
    const { getStoichiometry, updateStoichiometry, deleteStoichiometry, getMoleculeInfo } = useStoichiometry();
    const { trackEvent } = React.useContext(AnalyticsContext);
    const theme = useTheme();
    const [data, setData] = React.useState<StoichiometryResponse | null>(null);
    const [loading, setLoading] = React.useState(true);
    const [error, setError] = React.useState<string | null>(null);
    const [allMolecules, setAllMolecules] = React.useState<ReadonlyArray<EditableMolecule>>([]);
    ...
    }, [stoichiometryId, getStoichiometry, stoichiometryRevision]);
    ...
    React.useImperativeHandle(
            ...
            save: async () => {
                ...
                const updatedData: StoichiometryRequest = {
                    ...
                    molecules: allMolecules.map((m) => {
                        ...
                        if (!smiles) throw new Error("New reagents must have a SMILES string");
        ...
        [data, allMolecules, updateStoichiometry, deleteStoichiometry, onChangesUpdate],
    );
    ...
    const handleAddReagent = useCallback(
        async (smilesString: string, name: string, source: string) => {
            ...
            try {
                ...
                const limitingReagent = allMolecules.find((m) => m.limitingReagent);
                if (!limitingReagent) throw new Error("No limiting reagent defined");
                const limitingReagentMoles = calculateMoles(limitingReagent.mass, limitingReagent.molecularWeight);
                const ratio = limitingReagentMoles === null ? null : limitingReagentMoles / limitingReagent.coefficient;
    ...
    const limitingReagent = allMolecules.find((m) => m.limitingReagent && m.role.toLowerCase() === "reactant");
    ...
    const columns = [
        ...
        DataGridColumn.newColumnWithFieldName<"limitingReagent", EditableMolecule>("limitingReagent", {
        ...
        DataGridColumn.newColumnWithFieldName<"coefficient", EditableMolecule>("coefficient", {
        ...
        DataGridColumn.newColumnWithFieldName<"molecularWeight", EditableMolecule>("molecularWeight", {
        ...
        DataGridColumn.newColumnWithValueGetter<"moles", EditableMolecule, number | null>(
            "moles",
        ...
        DataGridColumn.newColumnWithFieldName<"actualAmount", EditableMolecule>("actualAmount", {
            ...
            editable,
            type: "number",
            renderCell: (params) => roundToThreeDecimals(params.value) ?? <>&mdash;</>,
        }),
        DataGridColumn.newColumnWithValueGetter<"actualMoles", EditableMolecule, number | null>(
            "actualMoles",
            (row: EditableMolecule) => calculateMoles(row.actualAmount, row.molecularWeight),
            {
                ...
                type: "number",
                renderCell: (params) => {
                    return params.value !== null ? roundToThreeDecimals(params.value) : <>&mdash;</>;
                },
            },
        ),
        DataGridColumn.newColumnWithFieldName<"actualYield", EditableMolecule>("actualYield", {
            ...
            renderCell: (params) => {
                ...
                return value !== null && value !== undefined ? `${roundToThreeDecimals(value) * 100}%` : <>&#8212;</>;
            },
        }),
    ...
    if (error) {
        return (
            <Box display="flex" justifyContent="center" alignItems="center" minHeight={100} my={2}>
            ...
            <DataGrid
                ...
                processRowUpdate={(newRow, oldRow) => {
                    try {
                         ...
                         */
                        const numericalFields = ["coefficient", "mass", "moles", "actualAmount", "actualMoles"];
