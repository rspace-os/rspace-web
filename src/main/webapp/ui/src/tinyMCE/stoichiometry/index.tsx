...
class StoichiometryPlugin {
    constructor(editor: Editor) {
        ...
        ): Generator<void, void, Partial<React.ComponentProps<typeof StoichiometryDialog>>> {
        ...
        editor.addCommand("cmdStoichiometry", () => {
            ...
            if (!/chem/.test(node.className)) throw new Error("Selected node is not a chemical element.");
            ...
            if (Number.isNaN(chemId)) {
                throw new Error("Chemical element id attribute must be a valid number.");
            }

            let stoichiometryId: number | undefined;
            let stoichiometryRevision: number | undefined;
            try {
                const { id, revision } = JSON.parse(node.getAttribute("data-stoichiometry-table") ?? "{}");
                stoichiometryId = id;
            ...
            const markElementWithStoichiometry = (id: number, revision: number) => {
                ...
                if (!targetNode) throw new Error("Could not find the target node in the document.");
                targetNode.setAttribute("data-stoichiometry-table", JSON.stringify({ id, revision }));
