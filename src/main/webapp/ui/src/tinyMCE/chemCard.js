...
export default function ChemCard(props) {
    ...
    const fetchChemDetails = () => {
        ...
        const url = `${publicUrlPrepend}/chemical/ajax/getInfo?chemId=${props.item.id}`;
        axios
            .get(url)
            .then((response) => {
    ...
    const chemInfoTable = (chemical) => {
        return (
            <React.Fragment key={chemical.formula}>
                ...
                {Object.entries(JSON.parse(chemical.additionalMetadata ?? "{}")).map(([k, v]) => (
                    <TableRow>
                        <TableCell component="th" scope="row">
                            {k}
                        </TableCell>
    ...
    return (
        <Card
            ...
            style={
                props.inline
                    ? {
                          height: props.height < 200 ? "200px" : `${parseInt(props.height, 10) + 6}px`,
                          overflowY: "auto",
                          margin: "0px",
                      }
                    : {}
        ...
        >
            {!props.inline && (
                <CardHeader
                    ...
                    subheader={<img className={classes.avatar} src={props.item.imageSrc} height="48px" />}
                    action={
                        <IconButton
                            size="small"
                            aria-label="close"
            ...
            <CardContent className={classes.cardContent}>
                <Table size="small" className={classes.table}>
                    <TableBody>
                        ...
                        {Object.entries(JSON.parse(chem.additionalMetadata ?? "{}")).map(([k, v]) => (
                            <TableRow key={k}>
                                <TableCell component="th" scope="row">
                                    {k}
                                </TableCell>
