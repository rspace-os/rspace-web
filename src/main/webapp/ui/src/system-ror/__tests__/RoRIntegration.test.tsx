...
async function assertRoRDetailsText() {
    ...
    expect(screen.getByRole("link", { name: "https://ror.org/02mhbdp94" })).toHaveAttribute(
        "href",
        "https://ror.org/02mhbdp94",
    );
    expect(screen.getByRole("link", { name: "http://www.uniandes.edu.co/" })).toHaveAttribute(
        "href",
...
describe("Renders page with ROR data ", () => {
    ...
    it("displays page with no searchbar  and with Unlink button when RoR is linked", async () => {
        mockAxios.onGet("system/ror/existingGlobalRoRID").reply(200, "https://ror.org/02mhbdp94");
        setUpComponent();
        await screen.findByText("Research Organization Registry (ROR) Integration");
