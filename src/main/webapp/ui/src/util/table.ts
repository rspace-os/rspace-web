...
const transformObject = <T, U extends keyof T, V>(obj: T, map: { [K in U]: (t: T) => V }): { [K in U]: V } =>
...
export const getSortingSpecified =
    ...
    (a: T, b: T) =>
        (desc(transformObject(a, map), transformObject(b, map), orderBy) * (order === "desc" ? 1 : -1)) as -1 | 0 | 1;
