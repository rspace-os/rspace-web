...
export const layoutToLabel = (layout: Axis, n: number, position: number): string => {
    if (n > 24) {
...
export const layoutToLabels = (layout: Axis, n: number): Array<{ value: number; label: string }> => {
    if (n > 24) {
        throw new Error("grid larger than 24x24 are currently not supported.");
    }

    return Array.from({ length: n }).map((_, i) => ({
        label: layoutToLabel(layout, n, i + 1),
        value: i + 1,
    }));
};
