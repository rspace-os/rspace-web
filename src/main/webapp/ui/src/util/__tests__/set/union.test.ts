  ...
  test("Idempotence", () => {
    fc.assert(
      fc.property(fc.tuple(arbRsSet(fc.anything()), arbRsSet(fc.anything())), ([setA, setB]) => {
        expect(setA.union(setB).isSame(setA.union(setB).union(setB))).toBe(true);
      }),
    );
  });
  test("Commutativity", () => {
  ...
  test("Distributes over intersection", () => {
    fc.assert(
      fc.property(
        fc.tuple(arbRsSet(fc.anything()), arbRsSet(fc.anything()), arbRsSet(fc.anything())),
        ([setA, setB, setC]) => {
          expect(setA.union(setB.intersection(setC)).isSame(setA.union(setB).intersection(setA.union(setC)))).toBe(
            true,
          );
        },
      ),
