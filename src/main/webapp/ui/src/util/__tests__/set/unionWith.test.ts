    ...
    test("Idempotence", () => {
        fc.assert(
            fc.property(arbitraryMappableSets, ([mapFn, setA, setB]) => {
                expect(unionWith(mapFn, [setA, setB, setB]).isSame(unionWith(mapFn, [setA, setB]))).toBe(true);
            }),
        );
    });
    test("Associativity", () => {
        fc.assert(
            fc.property(arbitraryMappableSets, ([mapFn, setA, setB, setC]) => {
                const twoThenOne = unionWith(mapFn, [unionWith(mapFn, [setA, setB]), setC]);
                const oneThenTwo = unionWith(mapFn, [setA, unionWith(mapFn, [setB, setC])]);
    ...
    test("Identity element", () => {
        fc.assert(
            fc.property(arbitraryMappableSets, ([mapFn, setA]) => {
                expect(unionWith(mapFn, [setA, new RsSet<{ id: unknown }>()]).isSame(setA)).toBe(true);
                expect(unionWith(mapFn, [new RsSet<{ id: unknown }>(), setA]).isSame(setA)).toBe(true);
            }),
        );
    });
});
