...
describe("any", () => {
    ...
    test("Multiple Errors, should return Error.", () => {
        const actual = Result.any(Result.Error<unknown>([new Error("foo")]), Result.Error<unknown>([new Error("bar")]));
        expect(actual.isError).toBe(true);
        actual.orElseGet((errors) => {
    ...
    test("Mix of OK and Error, should return OK value.", () => {
        const actual = Result.any(Result.Ok<string>("foo"), Result.Error<string>([new Error("bar")]));
        expect(actual.isOk).toBe(true);
        actual.do((value) => {
            expect(value).toEqual(["foo"]);
        });
