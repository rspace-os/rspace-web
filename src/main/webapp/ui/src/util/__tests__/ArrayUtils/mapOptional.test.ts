...
describe("mapOptional", () => {
  test("Example", () => {
    const data: Array<{ tag: "hasNumber"; value: number } | { tag: "hasString"; value: string }> = [
    ...
    const justNumbers: Array<{ tag: "hasNumber"; value: number }> = ArrayUtils.mapOptional(
      (obj) => (obj.tag === "hasNumber" ? Optional.present(obj) : Optional.empty()),
      data,
    );
  ...
  test("Idempotence", () => {
    fc.assert(
      fc.property(fc.tuple(fc.array(fc.anything()), fc.func(fc.boolean())), ([array, func]) => {
        expect(ArrayUtils.mapOptional(presentIfTrue(func), ArrayUtils.mapOptional(presentIfTrue(func), array))).toEqual(
  ...
  test("A function that always returns Optional.present will always result in an unchanged array.", () => {
    fc.assert(
      fc.property(fc.array(fc.anything()), (array) => {
        expect(ArrayUtils.mapOptional((x) => Optional.present(x), array)).toEqual(array);
      }),
    );
  });
  ...
  test("Length of output will always be less than or equal to length of input.", () => {
    fc.assert(
      fc.property(fc.tuple(fc.array(fc.anything()), fc.func(fc.boolean())), ([array, func]) => {
        expect(ArrayUtils.mapOptional(presentIfTrue(func), array).length).toBeLessThanOrEqual(array.length);
      }),
    );
  });
});
