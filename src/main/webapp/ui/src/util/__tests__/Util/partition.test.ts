...
describe("partition", () => {
    ...
    test("The size of the two resulting arrays will always sum to the size of the input array.", () => {
        fc.assert(
            fc.property(fc.func<unknown[], boolean>(fc.boolean()), fc.array(fc.anything()), (predicate, list) => {
        ...
        fc.assert(
            fc.property(
                ...
                (predicate, listA, listB) => {
                    const [yes, no] = ArrayUtils.partition(predicate, [...listA, ...listB]);
    ...
    test("Order is maintained.", () => {
        fc.assert(
            fc.property(fc.func<unknown[], boolean>(fc.boolean()), fc.array(fc.string()), (predicate, list) => {
