...
describe("doNotAwait", () => {
    describe("When passed a function that takes multiple arguments", () => {
        test("all of the subsequent arguments should be passed through.", () => {
            ...
            const sumTwoAndSleep: (ms1: number, ms2: number) => void = doNotAwait(sumAndSleep);
            expect(sumTwoAndSleep(3, 4)).toBeUndefined();

            const sumThreeAndSleep: (ms1: number, ms2: number, ms3: number) => void = doNotAwait(sumAndSleep);
