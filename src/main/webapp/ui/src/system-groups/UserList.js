...
export default function UserList(props) {
    ...
    const handleSelect = (u) => {
        if (Array.isArray(u)) {
            props.onSelect(u.map((user) => (typeof user === "string" ? user : user.username)));
        } else {
    ...
    const isSelected = (u) => {
        return props.selected.indexOf(u.username) !== -1;
    };
    ...
    function filterVisibleUsers(search) {
        const visible = props.users.filter((u) =>
            stripDiacritics(userLabel(u)).toUpperCase().includes(stripDiacritics(search.toUpperCase())),
        );
    ...
    }, [props.users, filterVisibleUsers, searchTerm]);

    return (
        <FormControl fullWidth className="grow" data-test-id={`${props.listTitle.split(" ").join("-")}-column`}>
            ...
            {visibleUsers.length === 0 && (
                <div style={{ color: "grey" }} data-test-id={`${props.listTitle.split(" ").join("-")}-empty`}>
                    No users found
                </div>
            )}
            {visibleUsers.length > 0 && (
