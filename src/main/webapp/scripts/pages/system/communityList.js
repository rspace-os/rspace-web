...
function addRemoveCommunityHandler(){
	$('body').on('click', '#removeCommunity', function (e){
		...
		RS.createConfirmationDialog({
			...
			consequences: RS.msg("legacyjs.system.communityList.confirmDeletionConsequences", Object.values(data).length, RS.formatList(Object.values(data).map(RS.escapeHtml))),
			variant: "warning",
			callback: callback
		});
	});
