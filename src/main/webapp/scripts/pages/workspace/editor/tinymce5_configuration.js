var tinymcesetup = {
  ...
  mobile: {
	...
	relative_urls: false,
	content_css: ["/styles/simplicity/typoEdit.css", "/scripts/tinymce/tinymce5109/plugins/codesample/css/prism.css"],
	image_advtab: true,
	smart_paste: false,
	branding: false,
	icons: 'custom_icons',
		...
		$.ajax({
			...
			success: function (data) {
				...
				if (removed_files.length) {
					RS.confirm(RS.msg("legacyjs.workspace.coreEditor.chemFileIncompatibleExtension", removed_files.length, removed_files.map(f => `<li>${RS.escapeHtml(f.name)}</li>`).join('')), 'error', 5000 + removed_files.length * 1000);
				}
