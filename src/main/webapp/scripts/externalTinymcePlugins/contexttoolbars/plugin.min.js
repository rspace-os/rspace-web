...
function addKetcherChemContextToolbar(editor){
	function isStoichiometryTableOnly(node) {
		return !!(node && node.closest && node.closest('div[data-stoichiometry-table-only="true"]'));
	}

	editor.ui.registry.addContextToolbar('chemicalFileViewableKetcher', {
		predicate: function (node) {
			return isChemFileAttachment(editor.selection.getNode()) && editor.dom.is(node, 'img.chem');
		},
		items: 'attachmentinfopopup attachmentdownload | resizeImage' + (RS.chemistryEnabled && RS.chemistryProvider === 'indigo' ? ' | ketcherViewable | stoichiometry' : ''),
		position: 'node',
		scope: 'node'
	});
	editor.ui.registry.addContextToolbar('chemicalElementEditableKetcher', {
		...
		items: 'downloadImage | resizeImage' + (RS.chemistryEnabled && RS.chemistryProvider === 'indigo' ? ' | ketcherEditable | stoichiometry' : ''),
		position: 'node',
		scope: 'node'
	});

	if (RS.chemistryEnabled && RS.chemistryProvider === 'indigo') {
		editor.ui.registry.addContextToolbar('stoichiometryTableOnly', {
			predicate: function (node) {
				return isStoichiometryTableOnly(node);
			},
			items: 'stoichiometry',
			position: 'node',
			scope: 'node'
