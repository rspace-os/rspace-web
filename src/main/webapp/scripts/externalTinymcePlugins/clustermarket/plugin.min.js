        ...
        editor.windowManager.openUrl({
            ...
            onAction(dialogApi, button) {
                if (button.name == "insert") {
                    // Ask the dialog for the table. Building it there is destructive,
                    // so it cannot be prepared in advance; the reply arrives below.
                    dialogApi.sendMessage({ mceAction: "clustermarket-insert" });
                }
            },
            onClose() {
            },
            onMessage(dialogApi, details) {
                if (details.mceAction == "clustermarket-table") {
                    if (details.tableHtml) {
                        editor.execCommand("mceInsertContent", false, details.tableHtml);
