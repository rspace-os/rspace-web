...
// FIXME - using the owncloud html elements (eg owncloudDialog) - should add a new set of nextcloud elements and specific nextcloud.css?? (see below)
// Owncloud html is in structuredDocument.jsp - these are linked to the ownCloud.css files and the css is needed for functionality
...
tinymce.PluginManager.add('nextcloud', function (editor, url) {
	...
	$.when(propertiesRequest).then(function (propertiesResponse) {
		...
		editor.addCommand('cmdNextCloud', function () {
			authenticateNextCloudOAuth(importFromNextCloudToTinyMCE, "Choose", true, nextCloudURL, nextCloudServerName);
		});
