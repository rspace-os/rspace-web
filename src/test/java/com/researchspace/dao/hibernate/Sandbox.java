...
public class Sandbox extends SpringTransactionalTest {
  ...
  public void testConcat() {
    ...
    user =
        (User)
            session
                .createQuery("from User u where :constant like concat('%', u.username, '%')")
                .setParameter("constant", "'" + largeName + "'")
                .uniqueResult();
    assertNotNull(user);
    assertEquals(u, user);
  ...
  public void testFileProperty() {
    ...
    String sql =
        "from FileProperty fp where :value like concat('%',"
            + " replace(replace(fp.relPath,'/',''),'\\\\',''), '%')";
    FileProperty retrieved =
        (FileProperty)
            session.createQuery(sql).setParameter("value", "'" + altered + "'").uniqueResult();
    ...
    FileProperty retrieved3 =
        (FileProperty)
            session.createQuery(sql).setParameter("value", "'" + altered3 + "'").uniqueResult();
    ...
    List results =
        session
            .createQuery(
                "from FileProperty fp where replace(replace(fp.relPath,'/',''),'\\\\','') in"
                    + " (:paths)")
            .setParameterList("paths", List.of(altered, altered3))
            .list();
