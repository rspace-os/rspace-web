...
public class ExternalMessageHandlerImplTest extends SpringTransactionalTest {
  ...
  public void sendExternalMessageToSlack() {
    ...
    StructuredDocument doc2 = createBasicDocumentInRootFolderWithText(u1, "any2");
    StructuredDocument doc3 = createBasicDocumentInRootFolderWithText(u1, "any3");
    Long cfgSetId = setUpAppConfigForUser(u1, () -> getSlackDevDfg());
    logoutAndLoginAs(u1);

    ServiceOperationResult<ResponseEntity<String>> response =
        handler.sendExternalMessage(
            EXPECTED_MESSAGE, cfgSetId, toList(doc.getId(), doc2.getId(), doc3.getId()), u1);
    assertTrue(response.isSucceeded());
    ServiceOperationResult<ResponseEntity<String>> response2 =
        handler.sendExternalMessage(EXPECTED_MESSAGE, cfgSetId, toList(doc.getId()), u1);
  ...
  public void sendExternalMessageToMSTeams() {
    ...
    StructuredDocument doc2 = createBasicDocumentInRootFolderWithText(u1, "any2");
    StructuredDocument doc3 = createBasicDocumentInRootFolderWithText(u1, "any3");
    Long cfgSetId = setUpAppConfigForUser(u1, () -> getMsTeamsCfg());
    logoutAndLoginAs(u1);
  ...
  private Long setUpNonMessageAppConfig() {
    ...
    UserAppConfig appConfig = mgr.saveAppConfigElementSet(config, null, false, u1);
    Long cfgSetId = appConfig.getAppConfigElementSets().iterator().next().getId();
    return cfgSetId;
  }
  ...
  public void sendExternalMessageReturnsEmptyForNonMessageApp() {
    ...
    logoutAndLoginAs(u1);
    ServiceOperationResult<ResponseEntity<String>> response =
        handler.sendExternalMessage(EXPECTED_MESSAGE, cfgSetId, toList(-10L), u1);
  ...
  public void sendExternalMessageReturnsEmptyForUnknownId() {
    ServiceOperationResult<ResponseEntity<String>> response =
        handler.sendExternalMessage(EXPECTED_MESSAGE, -100L, toList(-10L), u1);
    assertFalse(response.isSucceeded());
  }
