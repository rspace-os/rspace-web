...
public class OmeroClientImplTest {
  ...
  public void setUp() {
    ...
    mockServer.reset();
    mockServer.get("/api").respond(withPort(versionJson));
    mockServer.get("/api/v0").respond(withPort(urlsJson));
    mockServer.get("/api/v0/servers/").respond(serversJson);
    mockServer.get("/api/v0/token/").respond(tokenJson);
    mockServer.post("/api/v0/login/").respond(fakeLoginResponseJson);
  }
  ...
  public void testListProjects() {
    ...
    List<? extends OmeroRSpaceView> projects =
        omeroClient.getProjectsAndScreens("public_,_public", "Projects");
  ...
  public void testListScreens() {
    ...
    List<? extends OmeroRSpaceView> screens =
        omeroClient.getProjectsAndScreens("public_,_public", "Screens");
  ...
  public void testListDatasetsForProject() {
    mockServer.get("/api/v0/m/projects/51").respond(projectJson);
    mockServer
        .get("/webclient/api/datasets/")
        .query("id", "51")
        .query("page", "0")
        .respond(datasetsForProjectJson);
    List<DataSetRSpaceView> datasets = omeroClient.getDataSets("public_,_public", 51L);
  ...
  public void testListPlatesForScreen() {
    mockServer.get("/api/v0/m/screens/51").respond(screenJson);
    mockServer
        .get("/webclient/api/plates/")
        .query("id", "51")
        .query("page", "0")
        .respond(platesForScreenJson);
    List<PlateRSpaceView> plates = omeroClient.getPlates("public_,_public", 51L);
  ...
  public void testGetPlateAcquisitions() {
    mockServer.get("/api/v0/m/plates/422").query("childCount", "true").respond(withPort(plateJson));
    mockServer
        .get("/api/v0/m/plates/422/plateacquisitions/")
        .query("offset", "0")
        .respond(plateAcquisitionsJson);

    List<PlateAcquisitionRSpaceView> plates =
        omeroClient.getPlateAcquisitions("public_,_public", 422L);
  ...
  public void testGetAnnotations() {
    mockServer
        .get("/webclient/api/annotations/")
        .query("screen", "102")
        .respond(screenAnnotationsJson);
    List<String> annotations = omeroClient.getAnnotations("public_,_public", 102L, "screen");
  ...
  public void testListImagesForDataset() {
    mockServer.get("/api/v0/m/datasets/51").respond(withPort(datasetJson));
    mockServer
        .get("/api/v0/m/datasets/51/images/")
        .query("offset", "0")
    ...
    List<ImageRSpaceView> images = omeroClient.getImages("public_,_public", 51L, false);
  ...
  public void testGetImage() {
    ...
    mockServer.get("/api/v0/m/images/1884838").respond(singleImageJson);
    mockServer.get("/webgateway/render_thumbnail/1884838/400/").respond(fakeRenderedImage);
    ImageRSpaceView anImage = omeroClient.getImage("public_,_public", 1884838L, 51L, false);
  ...
  public void testGetWells() {
    ...
    List<WellRSpaceView> wells = omeroClient.getWells("public_,_public", 422L, 422L, false, 0);
