...
class OAuthTokensV2ControllerTest {
  ...
  @Test
  void refusesToMintAnUndelegatedTokenDuringRunAs() {
    OAuthTokenManager tokenManager = mock(OAuthTokenManager.class);
    User user = mock(User.class);
    MockHttpServletRequest request = new MockHttpServletRequest();
    request.getSession().setAttribute(SessionAttributeUtils.IS_RUN_AS, Boolean.TRUE);

    assertThrows(
        AuthorizationException.class,
        () -> new OAuthTokensV2Controller(tokenManager).createToken(user, request));
