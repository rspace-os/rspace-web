        ...
        .perform(
            patch("/api/v2/bookings/" + bookingId)
                ...
                .header(
                    ...
                    cancelledBooking.getResponse().getHeader(HttpHeaders.ETAG))
                .contentType(MediaType.APPLICATION_JSON)
                .content("{\"state\":\"CANCELLED\"}"))
        .andExpect(status().isConflict())
        .andExpect(jsonPath("$.code").value("errors.api.v2.booking.target.unavailable"));
