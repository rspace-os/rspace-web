...
class SequencedRunnable implements Runnable {
  ...
  public SequencedRunnable(
      String name, CountDownLatch[] conds, Invokable[] actions, Integer[] sequence) {
  ...
  private void validateArgs(
      String name, CountDownLatch[] latches, Invokable[] actions, Integer[] sequence) {
      ...
      if (index >= 1) {
        ...
        if (sequence[index] - sequence[index - 1] == 1) {
          throw new IllegalArgumentException(
              "Thread "
                  + name
                  + " can't have consecutive indices ["
                  + sequence[index - 1]
                  + ","
                  + sequence[index]
  ...
  public void run() {
    try {
      for (int integer : sequence) {
        ...
        } else log.debug("{} executed last invokable!", name);
      }
      ok = true;
    } catch (Throwable e) {
      e.printStackTrace();
