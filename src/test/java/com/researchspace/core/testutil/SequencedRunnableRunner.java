 ...
 * -thread-to-be-the-next-one-to-enter-a-synchronized-block. But, this method uses sequential
 * CountDown latches to ensure only 1 thread operates at a time.
 */
public class SequencedRunnableRunner {
  ...
  /**
   * @param nameToSequence A Map of keys ( thread names) and values ( the sequence of actions to be
   *     invoked)
   * @param actions An array of {@link Invokable} actions.
   */
  ...
  public void runSequence() throws InterruptedException {
    ...
    for (String name : nameToSequence.keySet()) {
      ...
      SequencedRunnable seqRun =
          new SequencedRunnable(name, conditions, actions, nameToSequence.get(name));
