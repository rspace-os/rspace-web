...
public class StringAppenderForTestLogging extends AbstractAppender {
  ...
  protected StringAppenderForTestLogging(
      String name,
      Filter filter,
  ...
  public void append(LogEvent event) {
    logContents += event.getMessage().getFormattedMessage();
  }

  /**
   * Clears the log of all contents, useful when searching for content multiple times in one test
   */
